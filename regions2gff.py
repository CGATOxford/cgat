...
#
#   $Id: regions2gff.py 2781 2009-09-10 11:33:14Z andreas $
#
...
def getRestrictionOnPredictionIds( schema, prediction_ids  = "all" ):
    ...
    if prediction_ids == "all":
        ...
        xwhere = "AND p.prediction_id  IN ('%s')" % "','".join( map(str, prediction_ids) )
...
def getMRNAs( dbhandle, schema, options, prediction_ids, contigs ):
    ...
    ## retrieve: gene_id, prediction_id, sbjct_token, sbjct_strand, sbjct_genome_from, sbjct_genome_to
    statement = """
...
def getIntrons( dbhandle, schema, options, prediction_ids, contigs ):
    ...
    for gene_id, prediction_id, sbjct_token, sbjct_strand, sbjct_from, sbjct_to, frame in result:
        ...
        if last_to:
            locations.append( (gene_id, prediction_id, sbjct_token, sbjct_strand, last_to, sbjct_from, frame) )
        last_to = sbjct_to
...
def getIntronicRegions( dbhandle, schema, options, prediction_ids, contigs ):
    ...
    for gene_id, prediction_id, contig, strand, start, end, frame in result:
        if last_id != gene_id:
            processChunk( last_id, last_contig, last_strand, last_frame, regions )
            last_id = gene_id
            last_contig = contig
            last_strand = strand
            regions = []
...
def getExonsThirdCodons( dbhandle, schema, options, prediction_ids, contigs ):
    ...
    def processChunk( prediction_id, gene_id, contig, strand, regions ):
        ...
        if Genomics.IsNegativeStrand( strand ):
            ...
            regions = map( lambda x: (coordinate_offset - x[1], coordinate_offset - x[0]), regions )
            regions.sort()
        else: 
            coordinate_offset = 0
        ...
        for start, end in regions:
            ...
            for x in range( start + 2, end, 3 ):
                if coordinate_offset:
                    ...
                    c = 0
                locations.append( (prediction_id, gene_id, contig, strand, c, c+1) )
            offset = (end - start) % 3
    ...
    for gene_id, prediction_id, contig, strand, start, end in result:
        if last_prediction_id != prediction_id:
            processChunk( last_prediction_id, last_gene_id, last_contig, last_strand, regions )
    ...
    processChunk( last_prediction_id, gene_id, last_contig, last_strand, regions )
...
def getLocations( dbhandle, schema, options, prediction_ids, contigs, taboo_regions = None ):
    ...
    if options.loglevel >= 1:
        options.stdlog.write( "# %s: retrieving candidate regions.\n" % (schema ) )
        options.stdlog.flush()

    if options.type == "mrnas":
        locations = getMRNAs( dbhandle, schema, options, prediction_ids, contigs )
    elif options.type == "genes":
        locations = getGenes( dbhandle, schema, options, prediction_ids, contigs )
    elif options.type == "introns":
        locations = getIntrons( dbhandle, schema, options, prediction_ids, contigs )
    elif options.type == "intronic":
        locations = getIntronicRegions( dbhandle, schema, options, prediction_ids, contigs )
    elif options.type == "exons":
        locations = getExons( dbhandle, schema, options, prediction_ids, contigs )
    elif options.type == "exonic":
        locations = getExonicRegions( dbhandle, schema, options, prediction_ids, contigs )
    elif options.type == "exons-third-codons":
        locations = getExonsThirdCodons( dbhandle, schema, options, prediction_ids, contigs )
    elif options.type == "intergenic":
        locations = getIntergenicRegions( dbhandle, schema, options, prediction_ids, contigs )
    else:
...
def processPredictions( dbhandle, schema, options, prediction_ids, taboo_regions = None, map_feature2property = None ):
    ...
    locations = getLocations( dbhandle, schema, options, prediction_ids, contig_size, taboo_regions )
...
if __name__ == "__main__":
    ...
    parser = optparse.OptionParser( version = "%prog version: $Id: regions2gff.py 2781 2009-09-10 11:33:14Z andreas $", usage = USAGE)
    ...
    parser.add_option("-t", "--type", dest="type", type="choice",
                      choices = ("genes", "mrnas", "introns", "intronic", "exons", "exonic", "intergenic", "exons-third-codons" ), 
                      help="type to output."  )
