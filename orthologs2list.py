...
#
#   $Id: orthologs2list.py 2781 2009-09-10 11:33:14Z andreas $
#
...
if __name__  == "__main__":
    ...
    if options.combine_genes:
        ...
        if options.use_networkx:
            ...
            if options.loglevel >= 1:
                options.stdlog.write( "# before combining genes: %i clusters\n" % len(orthologs) )
            ...
            for transcripts1, transcripts2, genes1, genes2, weight in orthologs:
                ...
                for g in genes1.keys(): graph.add_node( (1,g) )
                for g in genes2.keys(): graph.add_node( (2,g) )
            ...
            if options.loglevel >= 1:
                options.stdlog.write("# calculated connected components in %i seconds\n" % (time.time() - tt2))
                options.stdlog.flush()
    ...
    for transcripts1, transcripts2, genes1, genes2, weight in orthologs:
        ...
        if options.format == "graph":
            ...
            if options.only_best:
                ## print only best match between each possible set of genes in ortholog pair
                for gg1,tt1 in genes1.items():
                    for gg2,tt2 in genes2.items():
                        ...
                        if best_pair:
                            ...
                            options.stdlog.write( "# missed link between: %s %s\n" % (str(genes1), str(genes2)) )
                            nmissed += 1
            else:
            ...
            if options.within:
                ...
                # If orphans, also add links for genes with a single transripts.
