...
#
#   $Id: analyze_clustering.py 2781 2009-09-10 11:33:14Z andreas $
#
...
parser = optparse.OptionParser( version = "%prog version: $Id: analyze_clustering.py 2781 2009-09-10 11:33:14Z andreas $")
...
if __name__ == "__main__":
    ...
    if options.map:
        map_id2component = {}
        lines = filter( lambda x: x[0] != "#", open(options.map, "r").readlines())
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            raise ValueError, "could not extract species from line: %s" % line[:-1]
        ...
        if key not in arrays: arrays[key] = []
        ...
        if method == "hists":
            ...
            for s1 in species.keys():
                ...
                if options.loglevel >= 2:
                    options.stdlog.write("# processing histograms for species %s\n" % s1 )
                    options.stdlog.flush()
            ...
            outfile.write( "species1\tspecies2\tcounts\tmin\tmax\tmean\tmedian\tstd\n")
