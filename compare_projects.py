...
#
#   $Id: compare_projects.py 2387 2009-01-07 16:33:07Z andreas $
#
...
if __name__ == "__main__":
    ...
    ## summary_fields: starting coordinates of field separators for summary calculation
    if param_associate_fields:
        ...
        summary_fields = range( 1, len(param_schemas) * (len(param_fields) + 1), len(param_schemas) )        
        if param_summary:
            ...
            for x in param_fields:
                ...
                for y in param_schemas: headers2 += [y]
            
    else:
        fields = string.join( map( lambda x,y: "%s.%s.%s" % (x,table_name,y),
                                   GetExpandedList( param_schemas, len(param_fields)),
                                   param_fields * len(param_schemas)
                                   ), ",")
        summary_fields = range( 1, (len(param_schemas) + 1) * len(param_fields), len(param_fields) )
        if param_summary:
            ...
            for x in param_schemas:
                headers1 += [x] + [""] * (len(param_fields) -1)
                for y in param_fields: headers2 += [y]
        ...
        for r in result:
            ...
            for f in funcs:            
                for x in range(0, len(summary_fields)-1):
                    sys.stdout.write( "\t" + str(f( r[summary_fields[x]:summary_fields[x+1]]) ))
            sys.stdout.write( "\n" )
    else:
