...
class ClassifierRNASeq(Counter):
	...
	def classify_overlap( self, exons, transcript_id ):
		...
		if is_intronic:
			...
			# IMS structure is an exact match if and only if all intron boundaries in both exons and transcripts are shared.
			#	  this allows extensions of the 3' or 5' UTRs.
			...
			# If there is a matched structure, i.e. all of the introns in the gene model are in an existing gene model,
			# then either we have a fragment of the complete transcript or some sort of retain intron (or both)
