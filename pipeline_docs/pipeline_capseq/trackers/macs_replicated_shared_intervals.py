...
class replicatedSharedIntervals( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getFirstRow( "SELECT COUNT(*) as number, round(AVG(stop-start),0) as length FROM %(track)s_replicated_shared_intervals" % locals() )
...
class replicatedsharedIntervalLengths( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getValues( "SELECT (stop-start) FROM %(track)s_replicated_shared_intervals" % locals() )
        return { "length" : data }
