...
class FoldChangeCounts( cpgTracker ):
    ...
    def __call__(self, track, slice = None ):
        ...
        downfold = 1.0 / upfold
        data.append( ("> %5.2f fold" % upfold, self.getValue( "SELECT COUNT(*) FROM %(track)s_macs_intervals WHERE fold > %(upfold)f " % locals() )) )
        data.append( ("unchanged", self.getValue( "SELECT COUNT(*) FROM %(track)s_macs_intervals WHERE fold between %(downfold)f and %(upfold)f" % locals() )) )
        data.append( ("< %5.2f fold" % downfold, self.getValue( "SELECT COUNT(*) FROM %(track)s_macs_intervals WHERE fold < %(downfold)f " % locals() )) )
