...
class MacsSummary( cpgTracker ):
    ...
    def __call__(self, track, slice = None ):
        ...
        if os.path.exists( resultsdir ):
            print resultsdir
            result["link"] = "`pdf <%(resultsdir)s/%(track)s.macs_model.pdf>`_" % locals()
        return result
...
class MacsSoloSummary( cpgTracker ):
    ...
    def __call__(self, track, slice = None ):
        ...
        fields = (
            "tag_treatment_total", 
            "ncandidates_positive", 
            "called_positive", "min_tags",
            "paired_peaks", "scan_window", "shift" )

        f = ",".join(fields)
        data = self.getFirstRow( '''SELECT %(f)s FROM macs_solo_summary WHERE track="%(track)s"''' % locals())
        result = odict( zip( fields,data) )
