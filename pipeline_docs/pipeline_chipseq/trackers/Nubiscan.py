...
class NubiscanSummary( Nubiscan ):
    ...
    def __call__(self, track, slice = None):
        ...
        n = self.getValue( "SELECT COUNT(DISTINCT id) FROM %(track)s_nubiscan WHERE motif = '%(slice)s'" % locals() )
...
class NubiscanArrangements( Nubiscan ):
    ...
    def __call__(self, track, slice = None):
        data = []
        data = self.get( "SELECT arrangement, COUNT(*) FROM %(track)s_nubiscan WHERE motif = '%(slice)s' GROUP BY arrangement" % locals() )
...
class NubiscanArrangementsPerInterval( Nubiscan ):
    ...
    def __call__(self, track, slice = None):
        data = []
        data = self.get( "SELECT arrangement, COUNT(DISTINCT id) FROM %(track)s_nubiscan WHERE motif = '%(slice)s' GROUP BY arrangement" % locals() )
        data.sort( key = lambda x: (x[0][:2], int(x[0][2:]) ) )
        return odict(data)
...
class NubiscanArrangementsPerIntervalWeighted( Nubiscan ):
    ...
    def __call__(self, track, slice = None):
        ...
        data = self.get( "SELECT arrangement, alternatives FROM %(track)s_nubiscan WHERE motif = '%(slice)s' ORDER by arrangement" % locals() )
        for arr, alt in data:
            all = [arr]
            if alt: all.extend( alt.split(",") )
            v = 1.0 / len(all)
            for x in all: result[x] += v
