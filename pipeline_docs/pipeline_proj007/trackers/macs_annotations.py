...
class Annotations(cpgReport.cpgTracker):
    ...
    pattern = "(.*)_merged_annotations$"
    mTable = "merged_annotations"
    mSelect = None
    mColumns = None
    mWhere = "1"
...
class TSSProfile(cpgReport.cpgTracker):
    ...
    mPattern = "_merged_tss$"
    def __call__(self, track, slice = None ):
        statement1 = """SELECT (closest_dist*-1) as d from %(track)s_merged_tss where closest_dist=dist5 """
        statement2 = """SELECT closest_dist as d from %(track)s_merged_tss where closest_dist=dist3 """
...
class TTSProfile(cpgReport.cpgTracker):
    ...
    mPattern = "_merged_tts$"
    def __call__(self, track, slice = None ):
        statement1 = """SELECT (closest_dist*-1) as d from %(track)s_merged_tts where closest_dist=dist5 """
        statement2 = """SELECT closest_dist as d from %(track)s_merged_tts where closest_dist=dist3 """
