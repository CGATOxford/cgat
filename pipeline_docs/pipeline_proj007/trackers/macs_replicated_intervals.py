...
class replicatedIntervalSummary( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getRow( "SELECT COUNT(*) as Intervals, round(AVG(length),0) as Mean_length, round(AVG(nprobes),0) as Mean_reads FROM %(track)s_replicated_intervals" % locals() )
        return data
...
class replicatedIntervalLengths( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getAll( "SELECT length FROM %(track)s_replicated_intervals" % locals() )
        return data
...
class replicatedIntervalPeakValues( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getAll( "SELECT peakval FROM %(track)s_replicated_intervals" % locals() )
        return data
...
class replicatedIntervalAverageValues( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getAll( "SELECT avgval FROM %(track)s_replicated_intervals" % locals() )
        return data
...
class replicatedIntervalFoldChange( cpgTracker ):
    ...
    def __call__(self, track, slice = None ):
        data = self.getAll( "SELECT fold FROM %(track)s_replicated_intervals" % locals() )
        return data
...
class replicatedIntervalPeakLocation( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data1 = self.getValues( "SELECT (PeakCenter - start) / CAST( Length as FLOAT) - 0.5 FROM %(track)s_replicated_intervals" % locals() )
        data2 = self.getValues( "SELECT (end - PeakCenter) / CAST( Length as FLOAT) - 0.5 FROM %(track)s_replicated_intervals" % locals() )
...
class replicatedIntervalPeakDistance( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data1 = self.getValues( "SELECT PeakCenter - start FROM %(track)s_replicated_intervals" % locals() )
        data2 = self.getValues( "SELECT end - PeakCenter FROM %(track)s_replicated_intervals" % locals() )
        return { "distance" : data1 + data2 }
...
class replicatedIntervalCpGDensity( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data1 = self.getValues( "SELECT pCpG FROM %(track)s_replicated_capseq_composition" % locals() )
        data2 = self.getValues( "SELECT pCpG FROM %(track)s_replicated_composition_control" % locals() )
        data3 = self.getValues( "SELECT pCpG FROM %(track)s_replicated_composition_flanking5" % locals() )
        data4 = self.getValues( "SELECT pCpG FROM %(track)s_replicated_composition_flanking3" % locals() )
...
class replicatedIntervalCpGObsExp1( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data1 = self.getValues( "SELECT CpG_ObsExp1 FROM %(track)s_replicated_capseq_composition" % locals() )
        data2 = self.getValues( "SELECT CpG_ObsExp1 FROM %(track)s_replicated_composition_control" % locals() )
        data3 = self.getValues( "SELECT CpG_ObsExp1 FROM %(track)s_replicated_composition_flanking5" % locals() )
        data4 = self.getValues( "SELECT CpG_ObsExp1 FROM %(track)s_replicated_composition_flanking3" % locals() )
        return odict( zip(("CAPseq composition","Control composition","5` Flank Composition", "3` Flank Composition"), (data1, data2, data3, data4)) )
...
class replicatedIntervalCpGObsExp2( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data1 = self.getValues( "SELECT CpG_ObsExp FROM %(track)s_replicated_capseq_composition" % locals() )
...
class replicatedIntervalGCContent( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data1 = self.getValues( "SELECT pGC FROM %(track)s_replicated_capseq_composition" % locals() )
        data2 = self.getValues( "SELECT pGC FROM %(track)s_replicated_composition_control" % locals() )
        data3 = self.getValues( "SELECT pGC FROM %(track)s_replicated_composition_flanking5" % locals() )
        data4 = self.getValues( "SELECT pGC FROM %(track)s_replicated_composition_flanking3" % locals() )
        return odict( zip(("CAPseq composition","Control composition","5` Flank Composition", "3` Flank Composition"), (data1, data2, data3, data4)) )
