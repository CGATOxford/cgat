...
class cgiIntervals( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getFirstRow( "SELECT COUNT(*) as number, round(AVG(stop-start),0) as length FROM %(track)s" % locals() )
...
class cgiIntervalLengths( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data1 = self.getValues( "SELECT (stop-start) FROM %(track)s_replicated_predicted_cgi_and_cap" % locals() )
        data2 = self.getValues( "SELECT (stop-start) FROM %(track)s_replicated_cap_not_predicted_cgi" % locals() )
        data3 = self.getValues( "SELECT (stop-start) FROM %(track)s_replicated_predicted_cgi_not_cap" % locals() )
