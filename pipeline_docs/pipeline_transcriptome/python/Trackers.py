import os, sys, re, types, collections
import numpy
import numpy.ma
...
class TranscriptLengths(DefaultSlicer, TrackerSQL):
    ...
    def __call__(self, track, slice = None ):
        ...
        table = sqlalchemy.Table( "%s_annotation" % track, self.metadata, autoload = True )
        if slice == None or slice == "all":
            data = [ x[0] for x in self.execute( sqlalchemy.select( columns = [table.c.exons_sum,] )) ]
        elif slice == "linc":
            data = [ x[0] for x in self.execute( "SELECT exons_sum FROM %s_annotation AS a, %s_%s AS s WHERE s.gene_id = a.gene_id" % (track, track,slice)) ]
        else:
            data = [ x[0] for x in self.execute( "SELECT exons_sum FROM %s_annotation WHERE is_%s" % (track, slice)) ]
...
class Annotations(DefaultSlicer, TrackerSQL):
    ...
    def __call__(self, track, slice = None ):
        ...
        if slice in trackers_derived_slices:
            ...
            data = self.getFirstRow( "%(select)s FROM %(track)s_annotation WHERE %(where)s" % locals() )
        else:
            data = self.getFirstRow( "%(select)s FROM %(track)s_annotation WHERE %(where)s AND is_%(slice)s" % locals() )
...
class UTRTranscripts( TrackerSQL ):
    ...
    def __call__(self, track, slice = None):
        ...
        data = []
        data.append( ("utr5", self.getValue( "SELECT COUNT(*) FROM %(track)s_annotation WHERE nover1_utr5 > 0" % locals() ) ))
        data.append( ("utr3", self.getValue( "SELECT COUNT(*) FROM %(track)s_annotation WHERE nover1_utr3 > 0" % locals()) ))
        data.append( ("utr", self.getValue( "SELECT COUNT(*) FROM %(track)s_annotation WHERE nover1_utr > 0" % locals()) ))
        return odict( data )
...
class IntronBoundaries(Annotations):
    ...
    """
    pattern = "(.*)_annotation$"
    mColumns = ("total", "found", "missed", "both", "one", "none", "exon_skipping")
    ...
    def getSlices( self, subset = None ): return [ "all", ]
...
class IntronOverrunLengths(TrackerSQL):
    ...
    def getXLabel( self ): return "bases"

    def __call__(self, track, slice = None ):
        data = self.getValues( """SELECT nover_intronic from %s_overrun WHERE nover_exonic > 0""" % (track,) )
...
class RatesKs(AnnotationsAssociated):
    ...
    def getXLabel( self ): return self.mColumns
...
class RatesAll(DefaultSlicer, TrackerSQL):
    ...
    pattern = "(.*)_evol$"
    mColumns = set( ("aligned","ar_aligned","ir_aligned","ka","ki","ks","kska","kski","length","meancoverage","nreads","pgc","repeats_gc") )
    ...
    def __call__(self, track, slice = None ):
        ...
        table = sqlalchemy.Table( "%s_evol" % track, self.metadata, autoload = True )
        columns = [ x.name for x in table.columns if x.name in self.mColumns ]
        
        if not slice or slice == "all":
            data = list( self.execute( """SELECT %s FROM %s_evol AS e""" % (",".join( columns), track,)) )
        elif slice == "linc":
...
class ClosestDistance(DefaultSlicer, TrackerSQL):
    ...
    def __call__(self, track, slice = None ):
        ...
        if not slice or slice == "all":
            data = self.getValues( """SELECT %(column)s FROM %(track)s_distances AS d WHERE %(where)s""" % locals() )
        elif slice == "linc":
...
class TranscriptionDirectionIntergenic( TrackerSQL ):
    ...
    def __call__(self, track, slice = None ):
        ...
        for key, val in counts.iteritems():
            if "." in key: k = "unknown"
            elif key[0] == key[1]: k = "same-%s" % key[2]
            else: k = "different-%s" % key[2]
            print key,val
            counts2[k] += val
...
class TranscriptionDirectionIntronic( TrackerSQL ):
    ...
    def __call__(self, track, slice = None ):
        ...
        counts = collections.defaultdict( int )
        for x in data: counts[ "".join(x[:2]) ] += 1
...
class Segments(TrackerSQL):
    ...
    def __call__(self, track, slice = None ):
        ...
        column = self.mColumn
        if not slice or slice == "all":
            data = self.getValues( """SELECT d.%(column)s FROM %(track)s%(table)s AS d""" % (locals()) )
        elif slice == "linc":
            data = self.getValues( """SELECT d.%(column)s FROM %(track)s%(table)s AS d, %(track)s_%(slice)s as s WHERE s.gene_id = d.gene_id""" % (locals()) )
        else:
            data = self.getValues( """SELECT d.%(column)s FROM %(track)s%(table)s AS d, %(track)s_annotation as a WHERE d.gene_id = a.gene_id and a.is_%(slice)s""" % (locals()) )
        return odict( ((column,data),))
...
class IntronsPerTranscript(DefaultSlicer, TrackerSQL):
    ...
    def __call__(self, track, slice = None ):
        if not slice or slice == "all":
            data = self.getValues( """SELECT introns_nval FROM %s_annotation AS a""" % (track,) )
        elif slice == "linc":
            data= self.getValues( """SELECT introns_nval FROM %s_annotation AS a, %s_%s as s WHERE s.gene_id = a.gene_id""" % (track,track,slice) )
        else:
            data = self.getValues( """SELECT introns_nval FROM %s_annotation AS a WHERE a.is_%s""" % (track,slice) )
...
class Overlaps( TrackerSQL ):
    ...
    mColumn = None
    pattern = "(.*)_annotation$"
    def getSlices( self, subset = None ): return trackers_default_slices

    def getData(self, track, slice = None ):
        if slice == "None": slice == "all"
        tablename = self.mTableName % slice
        if self.mColumn == None: raise NotImplementedError( "mColumn not set in derived class." )
        column = self.mColumn
        data = self.get( "SELECT set2, %(column)s1 FROM %(tablename)s WHERE set1 = '%(track)s'" ) +\
            self.get( "SELECT set1, %(column)s2 FROM %(tablename)s WHERE set2 = '%(track)s'" )
        return odict(data)
...
class SharedGenes( TrackerSQL ):
    ...
    def getTracks( self, subset = None ): return ["all",]
    def getSlices( self, subset = None ): return trackers_default_slices
    ...
    def __call__(self, track, slice = None ):
        ...
        for table in alltables:
            a = self.getValue( "SELECT COUNT(DISTINCT gene_id) FROM %(table)s_annotation" % locals() )
            try:
                b = self.getValue( "SELECT COUNT(DISTINCT gene_id2) FROM %(master)s_vs_%(table)s_ovl" % locals() )
            except SQLError:
                continue
...
#         if not self.mTableName: raise NotImplementedError("table not specified")
#         return self.getValues( "SELECT DISTINCT slice || ':' || subset || ':' || workspace FROM %s" % self.mTableName )
...
#     def getTracks( self, subset = None ):
#         if not self.mTableName: raise NotImplementedError("table not specified")
#         return self.getValues( "SELECT DISTINCT track || ':' || slice || ':' || subset || ':' || workspace FROM %s ORDER BY track,slice,subset,workspace" % self.mTableName )
    ...
    def __call__(self, track, slice = None ):
        ...
        if slice:
            ...
            for pair in pairs:
                key,value = pair.split(":")
                o[key] = value
            if "background" in o and background not in o["background"]: return []
            if "subset" in o and subset not in o["subset"]: return []
            if "slice" in o and xslice not in o["slice"]: return []
            if "track" in o and xtrack not in o["track"]: return []
...
class ReadStatsCounts(AnnotationsAssociated):
    ...
    def __call__(self, track, slice = None ):
        data = []
        data.append( ( "known", self.getValue( self.getStatement( track, slice = "known" ) ) ) )
        data.append( ( "ambiguous", self.getValue( self.getStatement( track, slice = "ambiguous" ) ) ) )
        data.append( ( "novel", self.getValue( self.getStatement( track, slice = "unknown" ) ) ) )
        return odict(data)
