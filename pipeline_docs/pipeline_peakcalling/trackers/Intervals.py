...
class PeakLocation( DefaultTracker ):
    ...
    def __call__(self, track, slice = None):
        data1 = self.getValues( "SELECT (PeakCenter - start) / CAST( Length as FLOAT) - 0.5 FROM %(track)s_%(slice)s_%(suffix)s" )
...
class PeakDistance( DefaultTracker ):
    ...
    def __call__(self, track, slice = None):
        data1 = self.getValues( "SELECT PeakCenter - start FROM %(track)s_%(slice)s_%(suffix)s" )
