...
class ReadCountSummary ( TrackerSQL ):
    ...
    def __call__(self, row, col ):
        data = self.getValue( '''SELECT %(col)s FROM reads_under_peaks WHERE track = '%(row)s' ''' )
        return data
...
class NormalisedTable ( ReadCountSummary ):
    ...
    def __call__(self, row, col):
        ...
        data = self.getValue('''SELECT %(col)s FROM reads_under_peaks WHERE track = '%(row)s' ''') / float(norms[row])
        return data
