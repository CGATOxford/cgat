...
class EncodeQualityMetrics( Status ):
    ...
    def testNSC( self, track ):
        ...
        value = self.getValue( """SELECT nsc FROM %(tablename)s WHERE track = '%(track)s'""" )
        if value >= 1.1: status= "PASS"
        elif value >= 1.05: status= "WARNING"
        else: status= "FAIL"
    ...
    def testRSC( self, track ):
        ...
        value = self.getValue( """SELECT rsc FROM %(tablename)s WHERE track = '%(track)s'""" )
        if value >= 1.0: status= "PASS"
    ...
    def testMappedReads( self, track ):
        ...
        FAIL : <10000000
        '''
        value = self.getValue( """SELECT mapped_reads FROM %(tablename)s WHERE track = '%(track)s'""" )
        if value >= 2e7: status= "PASS"
        elif value >= 1e7: status= "WARNING"
        else: status= "FAIL"
