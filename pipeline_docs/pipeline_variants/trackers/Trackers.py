...
class VariantTypeCounts( VariantsTracker ):
    ...
    def __call__(self, track, slice = None):
        ...
        data = self.get( "SELECT variant_type, COUNT(*) FROM %(track)s_annotations GROUP BY variant_type" % locals() )
...
class SNPQuality( VariantsTracker ):
    ...
    def __call__(self, track, slice = None):
        ...
        data = self.getValues( "SELECT %(quality)s FROM %(track)s_annotations WHERE variant_type in ('%(snpcodes)s')" % locals() )
...
class SubstitutionMatrixAminoAcids( VariantsTracker ):
    ...
    def __call__(self, track, slice=None): 
        data = self.get( "SELECT variant_aa, COUNT(*) FROM %(track)s_annotations WHERE reference_aa='%(slice)s' AND (variant_type='E' OR variant_type='O') and variant_aa != NULL GROUP BY variant_aa" % locals() )
...
class TranscriptCounts( VariantsTracker ):
    ...
    def __call__(self, track, slice = None):
        r = odict()
        r["genes"] = self.getValue("SELECT COUNT(DISTINCT gene_id) FROM %(track)s" % locals())
        r["transcripts"] = self.getValue("SELECT COUNT(DISTINCT transcript_id) FROM %(track)s" % locals())
        return r
