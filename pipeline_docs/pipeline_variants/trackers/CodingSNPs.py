...
class ExonEssentiality( VariantsTracker ):
    ...
    def __call__(self, track, slice = None):
        ...
        data = self.get( "SELECT exons_nused, exons_ntranscripts FROM %(track)s_annotations WHERE code IN ('%(codes)s')" % locals() )
...
class EffectCounts( VariantsTracker ):
    ...
    def __call__(self, track, slice = None):
        ...
        data.append( ( "synonymous", 
                       self.getValue( "SELECT COUNT(*) FROM %(track)s_annotations WHERE variant_type = 'S' AND reference_aa = consensus_aa" % locals() )
                       ) )
