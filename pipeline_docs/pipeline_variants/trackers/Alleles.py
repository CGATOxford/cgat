...
class AlleleCounts( TrackerAlleles ):
    ...
    def __call__(self, track, slice = None ):
        ...
        data = odict()
        data["total"] = self.getValue("SELECT COUNT(*) FROM %(track)s_alleles" % locals())
        data["wildtype"] = self.getValue("SELECT SUM(is_wildtype) FROM %(track)s_alleles" % locals())
        data["stop"] = self.getValue("SELECT SUM(is_stop_truncated) FROM %(track)s_alleles" % locals())
        data["nmd"] = self.getValue("SELECT SUM(is_nmd_knockout) FROM %(track)s_alleles" % locals())
        data["splice"] = self.getValue("SELECT SUM(is_splice_truncated) FROM %(track)s_alleles" % locals())
...
class TranscriptsCounts( TrackerAlleles ):
    ...
    def __call__(self, track, slice = None ):
        ...
        data = odict()
        data["total"] = self.getValue("SELECT COUNT(*) FROM %(table)s" % locals())
...
class GenesNMDKnockoutsSummary( VariantsTracker ):
    ...
    def getTracks( self, subset = None ): return ["view_genes"]
    ...
    def __call__(self, track, slice = None ):
        ...
        d = odict(zip( ["gene_id", "gene_name", "nmd_knockout_total" ] + columns + info, zip(*data )))
...
class GenesNMDKnockoutsWithOMIM( VariantsTracker ):
    ...
    def __call__(self, track, slice = None ):
        ...
        d["omim_gene_id"] = [ "`%s <%s>`_" % (x, self.url_omim % x) for x in d["omim_gene_id"] ]
        d["omim_morbid_id"] = [ "`%s <%s>`_" % (x, self.url_omim % x) for x in d["omim_morbid_id"] ]
        c = []
