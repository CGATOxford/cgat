...
#
#   $Id: orthologs2genes.py 2889 2010-04-07 08:57:57Z andreas $
#
...
def MapTranscripts2Genes( transcripts, map_transcript2location ):
    ...
    for transcript in transcripts:
        map_id2info[transcript.mTranscript] = (transcript.mSchema,transcript.mQuality)
        token1, strand1, from1, to1 = map_transcript2location[transcript.mTranscript]
...
def Write( old_transcripts, map_transcript2location, fix, options ):
    ...
    if fix:
        ...
        new_transcripts, genes = MapTranscripts2Genes( old_transcripts, map_transcript2location )
...
if __name__  == "__main__":
    ...
    if options.filename_exons1:
        rschema1, map_transcript2location1 = GetPrediction2LocationFromFile( open(options.filename_exons1, "r"), options )        
    else:
        map_transcript2location1 = GetPrediction2Location( dbhandle, schema1,
                                                           tablename_predictions = options.tablename_predictions)

    E.info( "collected %i for map_transcript2location1" % len(map_transcript2location1 ) )

    if options.filename_exons2:
        rschema2, map_transcript2location2 = GetPrediction2LocationFromFile( open(options.filename_exons2, "r"), options )
    else:
        map_transcript2location2 = GetPrediction2Location( dbhandle, schema2,
                                                           tablename_predictions = options.tablename_predictions)

    E.info( "collected %i for map_transcript2location2" % len(map_transcript2location2 ) )

    ## swop maps, if schemas have become confused (can happen if files are supplied)
