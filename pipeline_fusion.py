...
def prepare_directory():
    ...
    tophat_fusion_path = os.open('which tophat-fusion').read()
    tophat_fusion_path = P.snip(tophat_fusion_path,'tophat-fusion')

    for file in os.listdir(tophat_fusion_path + 'annotations'):
        if not os.exists(file):
            E.info('Linking file %(tophat_fusion_path)sanotations/%(file)s -> %(file)s' % locals())
            os.symlink(tophat_fusion_path + 'annotations/' + file, file)
        else:
            E.info('File %s already exists, no need to link' % file)

    if not os.exists('blast_human'):
        E.info('Linking blast_db from %s -> blast_human' % PARAMS('blast_dbpath'))
        os.link(PARAMS('blast_dbpath'),'blast_human')
    else:
...
def mapReadsWithTophatFusion( infiles, outfile ):
    ...
    if not PARAMS('tophatfusion_referenceJunctions')==none:
        reffile = PARAMS('tophatfusion_referenceJunctions')
...
@merge(mapReadsWithTophatFusion,'tophatfusion_out/results.html')
...
def doCleanUp(infile,outfile):
    ...
    to_cluster = USECLUSTER
    indir = P.snip(infile,'accepted_hits.ban')
