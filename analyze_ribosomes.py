...
#
#   $Id: analyze_ribosomes.py 2781 2009-09-10 11:33:14Z andreas $
#
...
if __name__ == "__main__":
    ...
    parser = optparse.OptionParser( version = "%prog version: $Id: analyze_ribosomes.py 2781 2009-09-10 11:33:14Z andreas $", usage = USAGE)
        ...
        for schema in options.schemas:
            if options.output_fields:
                for field in options.output_fields:
                    options.stdout.write( "\t" + options.output_pattern % (schema, field) )
            else:
    ...
    if identifiers:
            ...
            for schema in options.schemas:
                ...
                if row[f_compare] != "na":
                    ...
                    if options.filter_min_ratio or options.filter_max_ratio:
                        ...
                        f = lambda x: x[f_compare] != "na" and float(x[f_compare]) >= mi and float(x[f_compare]) <= ma and x[0] not in identifiers and x[f_extract] != "na"
                    else:
                        f = lambda x: x[0] not in identifiers and x[f_extract] != "na"
                    ## extract values: filter by minimum and maximum range and remove
                    ## positive identifiers.
                    v = float(row[f_extract])
                    values = map( lambda x: float(x[f_extract]), filter( f, table))
                ...
                for field in options.output_fields:
                    ...
                    if v != None:
                        if field == "zscore":
                            ...
                            f = options.format % ((v - stats["mean"]) / stats["mean"])
                        elif field == "val":
                            f = options.format % v 
                        else:
        ...
        for schema in options.schemas:
            extract_columns.append( map_fields2column[options.field_extract % schema] )
        ...
        for row in table:
                    ...
                    if r < mi or r > ma:
                        ...
                        if options.loglevel >= 3:
                            if options.format == "table":
                                ...
                                options.stdout.write( "\t".join([ row[y] for y in extract_columns] ))
                                options.stdout.write("\n")                                         
                        break
            ...
            if options.output_format == "table":
                ...
                if has_na and options.remove_na: continue
