...
#
#   $Id: exons2clusters.py 2464 2009-02-02 10:28:09Z andreas $
#
...
def CollectCluster( map_transcript2transcript, seed ):
    ...
    if seed not in map_transcript2transcript: return []
...
def ClusterByExonCorrespondence( lengths = {} , peptide_sequences = None):
    ...
    for t in exons:
        ...
        for e in exons[t]:
            r = "%s-%s-%i-%i" % (e.mSbjctToken, e.mSbjctStrand, e.mGenomeFrom, e.mGenomeTo )
        ...
        if last_strand == e.mSbjctStrand and \
           ...
           max(o1,o2) >= param_min_max_overlap:
            ...
            if e.mQueryToken not in map_cluster2transcripts[last_cluster]:
                ...
                if join:
                    map_cluster2transcripts[last_cluster] += map_cluster2transcripts[e.mQueryToken]
...
if __name__ == "__main__":
    ...
    for t in map_cluster2transcripts:
        if not map_cluster2transcripts[t]: continue
