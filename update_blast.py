...
#
#   $Id: update_blast.py 2781 2009-09-10 11:33:14Z andreas $
#
...
""" program $Id: update_blast.py 2781 2009-09-10 11:33:14Z andreas $

take a blast output file and rerun with BLAST
...
if __name__ == "__main__":
    ...
    new_query_sequences = Genomics.ReadPeptideSequences( open( options.filename_new_query_sequences, "r"))
    old_query_sequences = Genomics.ReadPeptideSequences( open( options.filename_old_query_sequences, "r"))    

    if options.loglevel >= 1:
        print "# read %i/%i sequences for old/new query" % (len(old_query_sequences),len(new_query_sequences))
        
    common, unique_old, unique_new = CompareSequenceSets( old_query_sequences, new_query_sequences)
    ...
    WriteUnique( open(options.filename_pattern % "unique_query_old", "w"), unique_old )
    WriteUnique( open(options.filename_pattern % "unique_query_new", "w"), unique_new )    
