...
#
#   $Id: check_blast_run.py 2781 2009-09-10 11:33:14Z andreas $
#
...
if __name__ == "__main__":
    ...
    for x in range(len(options.vertices)):
	    ...
	    vv = map( lambda x: x[:-1].split("\t")[0], filter( lambda x: x[0] != "#", open(f, "r").readlines()))
	    nvertices[x] = len(vv)
    ...
    for v, vv in vertices.items():
	    ...
	    if nsbjct: found_sbjcts[index] += 1	    

    headers = ("set", "name", "tvertex", "nmissed", "pmissed", "nquery", "pquery", "nsbjct", "psbjct" )
    ...
    for x in range(len(options.vertices)):
	    print "%i\t%s\t%i\t%i\t%5.2f\t%i\t%5.2f\t%i\t%5.2f" % ( x, options.vertices[x] ,
								    nvertices[x],									  
								    len(missed_queries[x]), 
								    100 * float(len(missed_queries[x])) / nvertices[x],
								    found_queries[x],
								    100 * float(found_queries[x]) / nvertices[x],
								    found_sbjcts[x], 
