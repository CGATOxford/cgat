...
###################################################
@follows( mkdir("exons") )
@transform( "*.gtf", regex(r"(\S+).gtf"), r"exons/\1.exons.gtf" )
def getExonGtf( infile, outfile ):
    '''Copy replicated Bed files generated by capseq pipline to geneset-specific output directory'''
    track = P.snip( os.path.basename(infile), ".gtf" )
    species, tissue = track.split("-")
    statement = '''cat %(infile)s | grep "exon" > %(outfile)s'''
    P.run()
    
###################################################
@merge(getExonGtf, "exons/all_tissues.exons.bed")
def mergeExonGTFsAcrossTissues( infiles, outfile ):
    '''merge GTF files from all species'''
    inlist = " ".join(infiles)
    statement = ''' cat %(inlist)s | mergeBed -i stdin -s -n > %(outfile)s;'''
    P.run()
...
@transform( "anoCar2*.gtf", regex(r"(\S+).gtf"), r"transcripts/\1.transcripts.gtf" )
def getGtfStrandedTranscripts( infile, outfile ):
    ...
    track = P.snip( os.path.basename(infile), ".gtf" )
    species, tissue = track.split("-")
    statement = '''cat %(infile)s | awk '$3 == "transcript"' | awk '$7 != "."' | sort -k1,1 -k4,4n > %(outfile)s'''
    P.run()
...
def addMissingEnsemblTranscripts( infile, outfile ):
    ...
    statement = '''cat %(infile)s 
                   | intersectBed -a %(ensembl_transcripts)s -b stdin -v -s -f 1 -r > transcripts/missing_ensembl_transcripts.gtf;
...
@transform( addMissingEnsemblTranscripts, regex(r"transcripts/all_transcripts.gtf"), "geneset/transcripts.gtf.gz" )
...
def annotateNoncodingTranscripts( infile, outfile ):
    ...
    statement = '''intersectBed -a %(infile)s  -b %(ensembl_noncoding)s -wa -wb -s 
                   | awk -F "\\t" 'OFS="\\t" {print $1,$2,$3,$4,$5,$6,$7,$8,"gene_id \\""$13"\\"; "$9}' > transcripts/known_noncoding_exons.gtf;  
                   intersectBed -a %(infile)s -b %(ensembl_noncoding)s -v -s
                   | awk -F "\\t" 'OFS="\\t" {print $1,$2,$3,$4,$5,$6,$7,$8,"gene_id \\"novel_gene_"NR"\\"; "$9}' > transcripts/novel_noncoding_transcripts.gtf; 
                   cat transcripts/known_noncoding_exons.gtf transcripts/novel_noncoding_transcripts.gtf | sort -k1,1 -k4,4n
                   > %(outfile)s;'''
    P.run()
...
@follows( mkdir("geneset") )
@transform(getEnsemblCodingGeneset, regex(r"transcripts/(\S+).coding.gtf"), r"geneset/\1.genes.bed" )
...
############################################################
## Annotate exons
@follows( annotateGeneIntervals )
@transform(mergeExonGTFsAcrossTissues, suffix(".bed"), ".anno.bed" )
def annotateExons( infile, outfile ):
    ''' Add ensembl gene id to GTF file'''
    genes = "geneset/genes.bed"
    statement = '''cat %(infile)s 
                   | awk 'OFS="\\t" {print $1,$2,$3,"exon_"NR,$4,$5}'
                   | intersectBed -a stdin -b %(genes)s -wa -wb -s
                   | awk -F "\\t" 'OFS="\\t" {print $1,$2,$3,$10,$5,$6}'
                   | sort -k1,1 -k4,4 -k2,2n > %(outfile)s;'''
    P.run()

###################################################
@transform( annotateExons, suffix(".bed"), ".gtf" )
def convertExonsToGTF( infile, outfile ):
    '''convert bed to gtf'''
    statement = """cat %(infile)s 
                   | python %(scriptsdir)s/bed2gff.py --as-gtf  --log=%(outfile)s.log 
                   > %(outfile)s """
