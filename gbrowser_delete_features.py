...
#
#   $Id: gbrowser_delete_features.py 2781 2009-09-10 11:33:14Z andreas $
#
...
if __name__ == "__main__":
    ...
    parser = optparse.OptionParser( version = "%prog version: $Id: gbrowser_delete_features.py 2781 2009-09-10 11:33:14Z andreas $", usage = USAGE)
    ...
    if "ftype" not in tables:
        raise "table ftype not found in %s - is it a gbrowser database?" % (self.database)
    ...
    if options.source_type:
        ...
        if len(source_types) == 1 and "%" in source_types[0]:
            statement = "SELECT ftypeid FROM ftype WHERE fsource LIKE '%s'" % source_types[0]
        else:
            statement = "SELECT ftypeid FROM ftype WHERE fsource IN ('%s')" % "','".join(source_types)
            
        if options.dry_run: print statement
        ...
        ## find annotations
        statement = "SELECT DISTINCTROW a.fid FROM fattribute_to_feature AS a, fdata AS b WHERE b.ftypeid IN (%s) AND a.fid = b.fid" % ",".join( typeids)
        if options.dry_run: print statement
        ...
        statement = "SELECT DISTINCTROW b.gid FROM fdata AS b WHERE b.ftypeid IN (%s)" % ",".join( typeids )
        if options.dry_run: print statement
        ...
        statement = "DELETE FROM fattribute_to_feature WHERE fid IN (%s)" % ",".join( fids )
        if options.dry_run:
            print statement
        else:
            dbhandle.cursor().execute(statement)
        ...
        statement = "DELETE FROM fdata WHERE ftypeid IN (%s)" % ",".join( typeids)
        if options.dry_run:
            print statement
        else:
            dbhandle.cursor().execute(statement)
        ...
        statement = "DELETE FROM ftype WHERE ftypeid IN (%s)" % ",".join( typeids)
        if options.dry_run:
            print statement
        else:
            dbhandle.cursor().execute(statement)
