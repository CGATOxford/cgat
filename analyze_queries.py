...
#
#   $Id: analyze_queries.py 2781 2009-09-10 11:33:14Z andreas $
#
...
def ClusterPeptidesByFragment( peptides ):
    ...
    for x in range(len(keys)):
        ...
        if kx in map_peptide2cluster: continue
...
def GetQueryInfo( dbhandle, genome, options, subset=None ):
    ...
    if subset or options.filter_quality:
        ...
        data = {}
        for x in r: data[x[1]] = [x[0], x[1], 0, 0]
...
def writeListMissed( outfile,
    ...
    for transcript, missed_genomes in output.items():
        outfile.write( transcript + "\t" + "%i\t" % len(missed_genomes) + "\t".join(missed_genomes) + "\n" )
...
if __name__ == "__main__":
    ...
    parser = optparse.OptionParser( version = "%prog version: $Id: analyze_queries.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    if options.filter_quality: options.filter_quality = options.filter_quality.split(",")        
    ...
    if options.infile_clusters:
        map_peptide2cluster, map_cluster2peptide = IOTools.ReadMap( open(options.infile_clusters, "r"), both_directions = True)
    elif peptides:
        if options.clustering_method == "fragment":
            map_cluster2peptide, map_peptide2cluster = ClusterPeptidesByFragment( peptides )            
        elif options.clustering_method == "hid":
            map_cluster2peptide, map_peptide2cluster = ClusterPeptidesByHid( peptides )
    else:
        map_cluster2peptide = {}
        map_peptide2cluster = {}
        ...
        if method == "stats":
            ...
            for genome in options.genomes:
                ...
                options.stdout.write("%s\t%i\t%i\t%i\t%5.2f\t%5.2f\t%i\t%i\t%5.2f\t%5.2f\t%i\t%i\t%i\t%5.2f\t%5.2f\t%i\t%i\t%5.2f\t%5.2f\n" % (\
                    ...
                    f2(nfound_transcripts), f2(ntranscripts - nfound_transcripts),
            ...
            for genome in options.genomes:
                ...
                options.stdout.write( "%s\t%i\t%i\t%i\t%i\t%s\t%s\n" % (genome,
                                                                        ...
                                                                        options.format_percent % (100.0 * float(len(missed_genes)) / len(genes)),
