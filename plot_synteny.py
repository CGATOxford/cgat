...
#
#   $Id: plot_synteny.py 2781 2009-09-10 11:33:14Z andreas $
#
...
class PlotSynteny:
    ...
    def initializePlot( self ):
        ...
        self.mDataWidth,  self.mMapContig2Start1, self.mMapContig2Size1 = self.getDiagonal( 0, self.mContentsContig1, self.mSortedContigs1 )
        self.mDataHeight, self.mMapContig2Start2, self.mMapContig2Size2 = self.getDiagonal( 1, self.mContentsContig2, self.mSortedContigs2 )        

        ## set the page size
        self.mPageWidth  = self.mHeaderWidth + self.mDataWidth + self.mFooterWidth
        self.mPageHeight = self.mHeaderHeight + self.mDataHeight + self.mFooterHeight

        ## set the font size factor
        self.mFontSizeFactor = int(max(1.0, float(min(self.mPageWidth, self.mPageHeight)) / 1000))
...
class PlotSyntenyGenomic( PlotSynteny ):
    ...
    def assignCoordinates( self ):
        ...
        def getTotalContigLength( contig_sizes, contents_contig ):
            ...
            min_length = max( contig_sizes.values() ) * self.mMinRelativeContigSize
            total = 0
        ...
        self.mScaleFactor1 = float(self.mDataWidth) / getTotalContigLength( self.contigSizes1, self.mContentsContig1 ) * self.mDotRadius
        self.mScaleFactor2 = float(self.mDataHeight) / getTotalContigLength( self.contigSizes2, self.mContentsContig2 ) * self.mDotRadius
...
if __name__ == "__main__":
    ...
    parser = optparse.OptionParser( version = "%prog version: $Id: plot_synteny.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    sorted_contigs1, sorted_contigs2, contigs1, contigs2 = Synteny.SortAssignOrthologs( orthologs1, orthologs2 )
    ...
    if options.coordinates == "genes":
        ...
        contig_sizes1 = IndexedFasta.IndexedFasta( options.genome_file1 ).getContigSizes()
        contig_sizes2 = IndexedFasta.IndexedFasta( options.genome_file2 ).getContigSizes()
