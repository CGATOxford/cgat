...
#
#   $Id: plot_multiple_synteny.py 2781 2009-09-10 11:33:14Z andreas $
#
...
class PlotMultipleSynteny:
    ...
    def buildColourWheel( self ):
        ...
        for n in range(0, nPalSize):
            ...
            if nFactor <= 256:
                ...
                nRed = int( 255.0 * (math.cos( ( (nFactor - 256.0 ) /256.0) * pi / 2 ) ) );
                nGreen =  int( 255.0 * (math.sin( ( (nFactor - 256.0 ) /256.0) * pi / 2 ) ) );
                nBlue = 0;

            elif nFactor > 512 :
                # Red 0, green decreasing, blue increasing
                nRed = 0;
                nGreen =  int( 255.0 * (math.cos( ( (nFactor - 512 ) /256.0) * pi / 2 ) ) );
                nBlue = int( 255.0 * (math.sin( ( (nFactor - 512 ) /256.0) * pi / 2 ) ) );
    ...
    def buildColourLinear( self ):
        ...
        d = map( lambda x, y: (x - y) / num_steps, self.mStopColour, self.startColour)
    ...
    def buildColourMap( self ):
        ...
        nsymbol = 0
        for group in groups:
            self.mMapGroup2Colour[group] = (self.mColours[x], self.mSymbols[nsymbol] )
            x += increment
    ...
    def initializePlot( self ):
        ...
        for species in self.mOrthologs.keys():
            max_width = max( max_width, self.getRowWidth( self.getGenes(species) ))

        ## calculate size of diagonal
        self.mDataWidth  = max_width
        self.mDataHeight = self.mNumSpecies * (self.mBlockSize + self.mHorizontalSeparator)
    ...
    def getSortedGenes( self, genes, last_genes ):
        ...
        for x in range(len(blocks)):
            for y in range(len(last_blocks)):
                ...
                best_matches.append( (y, -len(groups1.intersection(groups2)), x) )
        ...
        if self.mPlotLinks == "all":
            for x in range(len(self.mSpeciesList)-1):
                for y in range(x+1, len(self.mSpeciesList)):
                    if abs(x-y) == 1:
                        self.plotLinksForPair( self.mSpeciesList[x], self.mSpeciesList[y], BLACK, self.mPlaneLinks )
                    else:
                        self.plotLinksForPair( self.mSpeciesList[x], self.mSpeciesList[y], GREEN, self.mPlaneLinks + 1 )
                        
        elif self.mPlotLinks == "pairs":
            for x in range(len(self.mSpeciesList)-1):
                self.plotLinksForPair( self.mSpeciesList[x], self.mSpeciesList[x+1] )
