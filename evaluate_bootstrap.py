    ...
    def printSummary( self, format = "tab" ):
        ...
        if format == "pretty":
            return string.join( map(str, (
                ...
                "notus=%i" % self.notus,
                "ptest=%5.4f" % \
                (float(self.nconsistent_internal + self.nconsistent_external) / t1),
                "ptotal=%5.4f" % \
                (float(self.nconsistent_internal + self.nconsistent_external) / t2),
                "ftotal=%5.4f" % \
                (float(self.nfixed_consistent_internal + self.nfixed_consistent_external) / t2),
                )), "\t")
        elif format == "tab":
            return string.join( map(str, (
                "%i" % self.norgs,
...
def SelectMasks( masks, patterns, norgs, map_id2org, min_report_support = 90 ):
    ...
    ## parse masks and select those that have no inconsistencies and are not overlapping.
    for mask in masks:
        if mask.isOk():
            is_overlap = False
    ...
    ## parse masks and select those that have inconsistencies and are not overlapping.
    for mask in masks:
        if not mask.isOk():
            is_overlap = False
            for new_mask in new_masks:
    ...
    if cover_mask.getSize() > 0:
        ...
        reference_tree = GetPrunedReferenceTree( cover_mask, GetOrgs(map_id2org), param_reference_tree )
        cover_mask.setOrgs( norgs )

        AnalyseMask( cover_mask, patterns, norgs, reference_tree, map_id2org, min_report_support )
...
def AnalyseMask( mask, patterns, norgs, reference_tree, map_id2org, min_report_support = 90 ):
    ...
    for support, pattern in patterns:
        ...
        if param_loglevel >= 4: print "# pattern=", pattern
        ...
        for x in range(len(pattern)):
            ...
            if mask.mask[x]:
                if pattern[x] == "*":
                    t1[org] = 1
                    p1[nid] = 1
                    if param_loglevel >= 4: print "# *", name, nid
                else:
                    t2[org] = 1
                    p2[nid] = 1
                    if param_loglevel >= 4: print "# .", name, nid
        ...
        if key1 in tested or key2 in tested: continue
        
        tested[key1] = 1
        tested[key2] = 1
        
        if len(t1) == 0 or len(t2) == 0: continue
        is_monophyletic1, is_subtree1, is_monophyletic2, is_subtree2 = IsMonophyletic( reference_tree, t1, t2 )

        AddResults( mask, support, min_report_support, pattern, is_monophyletic1, is_subtree1, t1, p1 ) 
        AddResults( mask, support, min_report_support, pattern, is_monophyletic2, is_subtree2, t2, p2 )
...
def AnalysePatterns( patterns,
    ...
    for mask in masks:
        reference_tree = GetPrunedReferenceTree( mask, GetOrgs(map_id2org), param_reference_tree )
        AnalyseMask( mask, patterns, norgs, reference_tree, map_id2org, min_report_support )
...
def WriteInconsistencies( outfile, mask, map_id2org, cluster_id ):
    ...
    if len(mask.inconsistencies) == 0 or mask.mask_id == 0: return
...
if __name__ == "__main__":
    ...
    for o,a in optlist:
        if o in ( "-v", "--verbose" ):
            ...
            param_reference_tree = filter( lambda x: x[0] != "#", open( a, "r").readlines())
...
"""
    ...
    while 1:
        ...
        if line[0] == ">":
            ...
            map_id2org, map_otu2id = ReadMapId2Org( sys.stdin, rx_species, map_taxon2id )
            continue
            ...
            for mask in masks:
                WriteInconsistencies( outfile_inconsistencies, mask, map_id2org, cluster_id )
            ...
            print string.join(map(str, ("summary", cluster_id, len(map_id2org), num_samples,
                                        len(filter(lambda x: x[0] >= param_min_report_support, patterns)),
