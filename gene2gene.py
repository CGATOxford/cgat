################################################################################
#
#   $Id: gene2gene.py 2781 2009-09-10 11:33:14Z andreas $
#
...
if __name__ == '__main__':
    ...
    parser = optparse.OptionParser( version = "%prog version: $Id: gene2gene.py 2781 2009-09-10 11:33:14Z andreas $", usage = USAGE)
	...
	for x in range(len(lines)-1):
	    for y in range(x+1, len(lines)):
                options.stdout.write( options.separator.join( lines[x] ) + "\t" + options.separator.join( lines[y] ) + "\t0\n" )
                noutput += 1
    ...
    for line in sys.stdin:
        try:
            schema, prediction_id, gene_id, quality = line[:-1].split(options.separator)
        except ValueError:
            nerrors += 1
