...
#################################################################################
'''
optic_analyze_orthology_pairwise.py - 
======================================================
...
Example::
   ...
   python optic_analyze_orthology_pairwise.py --help
...
"""program $Id: optic_analyze_orthology_pairwise.py 2781 2009-09-10 11:33:14Z andreas $
...
if __name__ == "__main__":
    ...
    parser = E.OptionParser( version = "%prog version: $Id: optic_analyze_orthology_pairwise.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    all_identifiers = Genomics.ReadPeptideSequences( open( options.filename_cds, "r" ) )
    ...
    for line in gzip.open( options.filename_blast, "r" ):
        ...
        coverage = 100.0 * (link.mQueryTo - link.mQueryFrom + 1) / link.mQueryLength
        if b["coverage"] < coverage: b["coverage"] = coverage
        if b['pid'] < link.mPercentIdentity: b['pid'] = link.mPercentIdentity

        if options.test and ninput > options.test: break
    ...
    if options.loglevel >= 1:
        options.stdlog.write( "# read best matches for %i genes.\n" % (len(best_matches_blast)))
    ...
    for line in gzip.open( options.filename_bitscore_distances, "r" ):
        ...
        if t1.mGene not in best_matches_bitdist: best_matches_bitdist[t1.mGene] = 100.0
    ...
    if options.loglevel >= 1:
        options.stdlog.write( "# read bitscore distances for %i genes.\n" % (len(best_matches_bitdist)))
    ...
    for line in open( options.filename_kaks, "r" ):
        if line[0] == "#": continue
        if line.startswith( "seq1\tseq2" ): continue
        ninput += 1
        ...
        if t1.mGene not in best_matches_ks: best_matches_ks[t1.mGene] = 999.0

        if best_matches_ks[t1.mGene] > ks:
            best_matches_ks[t1.mGene] = ks

        if t1.mGene not in best_matches_ka: best_matches_ka[t1.mGene] = 999.0
    ...
    if options.loglevel >= 1:
        options.stdlog.write( "# read ka and ks distances for %i genes.\n" % (len(best_matches_ks)))
    ...
    for filename_orphans in options.filename_orphans:
        ...
        for line in open( filename_orphans, "r"):
            ...
            for t in tt:
                if t.mSchema != options.schema: continue
                if t.mGene not in oo: oo[t.mGene] = []
                oo[t.mGene].append( t )
        orphans.append( oo )

        if options.loglevel >= 1:
            options.stdlog.write( "# read orphans for %i genes from %s.\n" % (len(oo), filename_orphans))
    ...
    for filename_orthologs in options.filename_orthologs:
        ...
        for line in open( filename_orthologs, "r"):
            if line[0] == "#" : continue
            if line[0] == ">" : continue            
            tt = map( lambda x: Orthologs.Transcript(x), line[:-1].split("\t")[1:])
            for t in tt:
                if t.mSchema != options.schema: continue
                if t.mGene not in oo: oo[t.mGene] = []
                oo[t.mGene].append( t )
        ...
        if options.loglevel >= 1:
            options.stdlog.write( "# read orthologs for %i genes from %s.\n" % (len(oo), filename_orthologs))
...
""")
    ...
    options.stdout.write("gene\tntranscripts\tstage\tblast_pid\tblast_cov\tbitdist\tka\tks" )
    ...
    for g in genes:
        ...
        stage = "%i" % stages[g]
        if g in best_matches_blast:
            blast_pid, blast_cov = "%5.2f" % best_matches_blast[g]["pid"], "%5.2f" % best_matches_blast[g]["coverage"]

        if g in best_matches_bitdist: bitdist = "%5.2f" % best_matches_bitdist[g]
        if g in best_matches_ka: ka = "%6.4f" % best_matches_ka[g]
        if g in best_matches_ks: ks = "%6.4f" % best_matches_ks[g]
        ...
        for x in range(len(options.filename_orphans)):
            ...
            if g in orphans[x]: norphans = "%i" % len(orphans[x][g])
            if g in orthologs[x]: northologs = "%i" % len(orthologs[x][g])
