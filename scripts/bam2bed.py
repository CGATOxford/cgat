...
Convert aligned reads in :term:`BAM` formatted files into intervals in
:term:`bed` formatted files.

At its simplest, the script outputs intervals for each read or read
pair. In more complex processing, bam2bed outputs intervals derived
from properties of reads.
...
This script takes a :term:`bam` file as input and counts the number of
reads in windows across each chromsome.  Output is a :term:`bed`
formatted file for each interval with the relevant counts of reads
mapping to each interval.  Reads can be selected on the basis of
:term:`tags`, such as NH or NM and a threshold for the proportion of
reads mapping to an interval before it is output.  Read counting is
done using mapping position within an interval.

The :term:`bam` file must have an :term:`index` file (.bai) in the
same directory. A contig file can be provided with the --contig-file
option, or a new one can be generated using a specific genome and
directory with the --genome and --genome-dir.  The default action is
to create a dictionary of contig sizes from the bam file header info,
which requires no additional user input.

The original bam2bed.py functionality was to convert BAM files to BED
files over a user-specified interval.  This functionality has been
...
For example to output only 1000bp intervals that have >10% of reads
that are multi-mapping::
...
The output files are a :term:bedGraph format file with each interval
that has >0 reads aligned to it and a tab-delimtted per-contig
:term:summary file that contains::
...
def contigFileMaker(genome_dir, genome):
    ...
    This code is ripped from pipeline_annotations function
    buildContigSizes
    ...
    with open("contig.tsv", "w") as outs:
        ...
        for contig, size in fasta.getContigSizes(
                with_synonyms=False).iteritems():
            outs.write("%s\t%i\n" % (contig, size))
...
def summaryInfo(bedfile):
    ...
    for interval in Bed.iterator(bedfile):
        if interval.contig == cur_chr:
            res_dict[
                "%s:%i-%i" % (interval.contig,
                              interval.start,
                              interval.end)] = float(interval.fields[0])
    ...
    # summary info:
    # Number of intervals per contig, highest number
    # of reads across intervals
    # lowest number of reads across intervals, average number of
    # reads across intervals
    # Total number of reads across intervals, median number of
    # reads across intervals
...
# over a bam file and check each read against a condition to see
# if it should be counted.  The initial simple implementation is
# counting per contig.  Will generalize out to include user
# defined options


class Result(object):
    ''' The result class wraps up useful read
    information for the Aggregator class functions '''
...
class ReadCounter(object):
    ...
    def checkCount(self, item, condition=None):
        '''Checks whether the read meets a categorical
    ...
    def checkRange(self, item, condition=None):
        ...
        Range is defined by passing in a tuple of (`lower limit`,
        `upper limit`) Returns True or False
        '''

        if condition is None:
            return True
        elif (self.__getattr__(item) >= condition[0] and
              self.__getattr__(item) <= condition[1]):
            return True
        else:
            return False
...
class Profiler(object):
    '''A class that takes a list of ReadCounter instances and a sam file
    and yields profiles of counts
    ...
    def __iter__(self):
        ...
        return self.count(self.samfile,
                          self.condition,
                          self.tag,
                          self.k,
                          self.threshold)
        ...
        # need specific implementations for `cat`
            ...
            if self.condition == "cat":
                ...
                tocount = ReadCounter(read).checkCount(
                    read.tid, current_tid)

            # check reads are on the same contig, if not change
            # the contig variable and reinitialise the variables
            # pass results to Aggregator class functions

            if current_tid != last_tid:
                if self.condition is "range" and self.tag is None:
                    yield Aggregator(res).sumInterval(
                        res,
                        self.k,
                        contig_dict[samfile.getrname(last_tid)])
                elif self.condition == "cat":
                    yield (samfile.getrname(current_tid),
                           1,
                           contig_dict[samfile.getrname(current_tid)],
                           running_total)
                elif self.condition is "range" and self.tag is not None:
                    E.info("%s: Checking for intervals with > %0.2f reads "
                           "with tag %s" %
                           (samfile.getrname(current_tid), threshold, tag))
                    yield Aggregator(res).tagsInterval(
                        res,
                        self.k,
                        contig_dict[samfile.getrname(last_tid)],
                        self.threshold)
...
class Aggregator(object):
            ...
            if record.start <= interval:
                ...
                while record.start > interval:
                    if interval <= contig_size:
                        last_interval = interval
                        interval += k
                    # check the incremented interval isn't larger
                    # than the contig size reset to contig size if
                    # it is
                ...
                # return total count for each new interval that contains
                # aligned reads
                if interval > last_interval and intsum > 0:
                    yield (
                        samfile.getrname(record.contig),
                        last_interval,
                        interval,
                        intsum)
                    intsum = 0
    ...
    def tagsInterval(self, results, k, contig_size, threshold):
        '''Counts the number of reads that match a tag within a region and
        highlights regions > threshold tagged reads E.g. output
        regions with a large proportion of multi-mapping reads.
        ...
        for record in results:
            ...
            if interval > contig_size:
                ...
                pass
            # only yield results if the proportion of reads in an interval
            # exceed a threshold
            if counter > 1 and interval > last_interval and \
               float(counter) / float(reads) >= threshold:
                yield (samfile.getrname(record.contig),
                       last_interval,
                       interval,
                       counter)
                reads = 0
...
def main(argv=None):
    ...
    parser.add_option("--condition", dest="condition", type="string",
                      help="either `range` or `cat`, `range` is used "
                      "with --interval.")

    parser.add_option("--interval", dest="interval", type="int",
                      help="the interval size to count reads in "
                      "[default=%default]")

    parser.add_option("--tag", dest="tag", type="string",
                      help="outputs regions with >:term:`threshold` reads "
                      "containing `tag` [default=%default]")
    ...
    parser.add_option("--features", dest="features",
                      ...
                      help="compute one or more features "
                      "[default=%default]. ")

    parser.add_option("--merge-pairs", dest="merge_pairs",
                      action="store_true",
                      help="merge paired-end reads and output interval for "
                      "entire fragment [default=%default]. ")

    parser.add_option("--max-insert-size", dest="max_insert_size", type="int",
                      help="only merge paired-end reads if they are less "
                      "than # bases apart."
                      " 0 turns off this filter [default=%default]. ")

    parser.add_option("--min-insert-size", dest="min_insert_size", type="int",
                      help="only merge paired-end reads if theya re at "
                      "least # bases apart. "
                      " 0 turns off this filter [default=%default]. ")

    parser.add_option("--bed-format", dest="bed_format", type="choice",
                      choices=('3', '4', '5', '6'),
                      help="bed format to output. Only used with "
                      "--merge-pairs option. "
                      " [default=%default]")
    ...
    if options.contig is None and options.gDir is not None:
        ...
        if options.contig and not os.path.exists(options.contig):
            ...
            E.info("Making contig dictionary from input file %s header" %
                   sys.argv[1])
            contig_it = itertools.izip(samfile.references, samfile.lengths)
            contig_dict = {}
            for i in contig_it:
    ...
    # Are these the options to do feature computation?
    if options.features:
        # Needs to be set from -output-pattern, see Experiment.py
        file_prefix = ""
        # Write out a summary file
        # TODO: have as a user defined option to switch on/off

        if options.interval and options.tag:
            bedfileOut = IOTools.openFile(
                "%s-%s-interval.bed" % (file_prefix, options.tag), "w")
            bedfileOut.write(
                'track type=bedGraph name="%s intervals" '
                'description="Intervals with %s > %0.2f%%" '
                'visibility=full priority=20\n' %
                (options.tag, options.tag, round(options.threshold * 100, 2)))
            summaryfile = IOTools.openFile(
                "%s-%s.summary.txt" % (file_prefix, options.tag), "w")
            summaryfile.write(
                "Contig:\tIntervals:\tTotal:\tMin:\tMax:\tMean:"
                "\tMedian:\t25th_quartile:\t75th_quartile:\n")

            E.info("Writing bedGraph file %s-%s-interval.bed" %
                   (file_prefix, options.tag))
            prof1 = Profiler(
                samfile,
                options.condition,
                options.interval,
            ...
            bedfileOut.close()
            bedfileIn = IOTools.openFile(
                "%s-%s-interval.bed" % (file_prefix, options.tag), "r")
            for result in summaryInfo(bedfileIn):
                summaryfile.write("%s\t%i\t%i\t%i\t%i\t" % (
            ...
            bedfileOut = IOTools.openFile(
                "%s-interval.bed" % (file_prefix), "w")
            bedfileOut.write('track type=bedGraph name="%s intervals" description="Intervals with %s > %0.2f%%" visibility=full priority=20\n' % (
                options.tag, options.tag, round(options.threshold * 100, 2)))
            summaryfile = IOTools.openFile(
                "%s.summary.txt" % (file_prefix), "w")
            summaryfile.write(
                "Contig:\tIntervals:\tTotal:\tMin:\tMax:\tMean:\tMedian:\t25th_quartile:\t75th_quartile:\n")
            E.info("Writing bedGraph file %s-interval.bed" % (file_prefix))
            prof1 = Profiler(
                samfile, options.condition, options.interval, threshold=options.threshold)
            prof2 = prof1
            for res in prof1:
                for i in res:
                    bedfileOut.write("%s\t%i\t%i\t%0.2f\n" %
            ...
            bedfileOut.write(
                'track type=bedGraph name="category" description="Categorical" visibility=full priority=20\n')
        ...
        # default - convert reads to intervals
