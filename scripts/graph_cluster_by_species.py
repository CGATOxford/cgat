...
def clusterBySpecies(links, options):
    ...
    for transcript1, transcript2, weight in links:
        ...
        if transcript1 in map_to_synonyms:
            transcript1 = map_to_synonyms[transcript1]
        if transcript2 in map_to_synonyms:
            transcript2 = map_to_synonyms[transcript2]
        ...
        if transcript1 not in reciprocal_best:
            reciprocal_best[transcript1] = {}
        ...
        if transcript2 not in reciprocal_best:
            reciprocal_best[transcript2] = {}
    ...
    if options.loglevel >= 1:
        options.stdlog.write("# component sizes before pruning: %s\n" % ",".join(
            map(lambda x: str(len(x)), components)))

    # prune components: eliminate sequences from the same species in each
    # component
    new_components = []

    # sort by species and quality, take first entry, the rest are synonyms
    for component in components:
        component = map(lambda x: (
            x.split(options.separator)[0], x.split(options.separator)[1], x), component)
        component.sort()
        new_component = []
        last_species = None
        rep_transcript = None
        for species, quality, transcript in component:
            ...
            if last_species == species:
                AddSynonym(
                    rep_transcript, transcript, map_synonyms, map_to_synonyms)
            else:
...
def updateGraphs(transcript1, transcript2,
    ...
    if id1 not in map_id2transcripts:
        map_id2transcripts[id1] = {}
    if id2 not in map_id2transcripts:
        map_id2transcripts[id2] = {}
...
def clusterByReciprocity(links, map_transcript2id, options):
    ...
    components = getReciprocalBestComponents(
        reciprocal_best, map_transcript2id)
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: graph_cluster_by_species.py 2782 2009-09-10 11:40:29Z andreas $")
    ...
    if options.mode == "species":
        ...
        components, map_synonyms = clusterByReciprocity(
            links, map_transcript2id, options)
    ...
    if options.filename_summary:
        ...
        for component in components:
            ...
            nsynonyms = 0
            for c in component:
                if c in map_synonyms:
                    nsynonyms += len(map_synonyms[c])
            outfile.write("%i\t%i\t%i\t%i\t%i\n" % (n,
                                                    len(component),
                                                    len(species),
                                                    len(filter(
                                                        lambda x: x == options.genome_master, species)),
                                                    nsynonyms))
