...
def getSampledMali(mali, sample_size, block_size=1):
    ...
    if sample_size >= l:
        raise ValueError(
            "sample size (%i) larger than number of columns (%i) to sample from" % (sample_size, l))
...
def getMali(mali, columns, block_size=1):
    ...
    for id, val in mali.items():
        ...
        new_mali.addSequence(
            id, 0, mali.countCharacters(new_sequence), new_sequence)
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: mali2bootstrap.py 2782 2009-09-10 11:40:29Z andreas $", usage=globals()["__doc__"])
