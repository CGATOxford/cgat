...
various sequence properties (length, information content, codon bias, ...) and
outputs them as a tab-separated table.
...
Example::
   ...
   python fasta2table.py --type=aa --section=length --section=hid
...
def main( argv = None ):
    ...
    parser.add_option("-w", "--filename-weights", dest="filename_weights", type="string",
                      help="filename with codon frequencies. Multiple filenames can be separated by comma [default=%default]." )
    
    parser.add_option("-s", "--sections", dest="sections", type="choice", action="append",
                      choices = ("length", "hid", "na", "aa", "degeneracy", "bias", "codons", "codon-usage", "codon-translator" ),
    ...
    parser.add_option("-e", "--regex-identifier", dest="regex_identifier", type="string",
                      help="regular expression to extract identifier from fasta description line [default=%default]." )
    ...
    if options.filename_weights:
        ...
        for x in range(0, len(reference_codons)):
            for y in range(0, len(reference_codons)):
                ...
                E.info("tablediff\t%s\t%s\t%f" % (options.filename_weights[x],
