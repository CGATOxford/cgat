...
class Builder(Processor):
    ...
    def buildPileUp( self, sbjct_id, sbjct_start, sbjct_end, matches, map_genome2transcript = None ):
        ...
        if map_genome2transcript:
            mapped = alignlib.py_makeAlignmentBlocks()
        ...
        for match in matches:
            ...
            if self.options.loglevel >= 4:
                ...
                a = str(alignlib.py_AlignmentFormatExplicit( match.mMapTarget2Query,
                                                          alignlib.py_makeSequence( genome ),
                                                          alignlib.py_makeSequence( query ) ))

                # self.options.stdlog.write( str( alignlib.py_AlignmentFormatExplicit( match.mMapTarget2Query, 
#                                                                                  alignlib.py_makeSequence( genome ),
#                                                                                  alignlib.py_makeSequence( query ) ) ) + "\n" )
            
            if map_genome2transcript:
                alignlib.py_combineAlignment( mapped, 
...
class BuilderTranscribedLocus(Builder):
    ...
    def getTranscriptMap( self, sbjct_id, sbjct_start, sbjct_end, matches ):
        ...
        map_transcript2genome = alignlib.py_makeAlignmentBlocks()
...
class BuilderConsensus(Builder):
    ...
    def process( self, sbjct_id, sbjct_start, sbjct_end, matches ):
        ...
        map_genome2transcript = alignlib.py_makeAlignmentVector()
        ...
        if options.loglevel >= 3:
            options.stdlog.write( "# %s\n" % str(alignlib.py_AlignmentFormatEmissions( map_genome2transcript)) )
    ...
    def filter( self, sbjct_id, sbjct_start, sbjct_end, matches ):
        ...
        for match_without_introns in matches_without_introns:
            for match_with_introns in matches_with_introns:
                d = alignlib.py_getAlignmentShortestDistance( \
                ...
                for intron_start, intron_end in match_with_introns.iterator_introns():
                    ...
                    intron_map = alignlib.py_makeAlignmentBlocks()
                    ...
                    overlap = alignlib.py_getAlignmentOverlap( match_without_introns.mMapTarget2Query, 
                                                            intron_map,
                                                            alignlib.py_RR )
    ...
    def filter( self, sbjct_id, sbjct_start, sbjct_end, matches ):
        ...
        for x in range(len(matches)):
            ...
            for y in range(x+1, len(matches)):
                ...
                overlap = alignlib.py_getAlignmentOverlap( \
                    mx.mMapTarget2Query, 
                    my.mMapTarget2Query, 
                    alignlib.py_RR )
