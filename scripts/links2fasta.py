...
class Map:
    ...
    def expand( self ):
        if not self.mMapOld2New:
            self.mMapOld2New = alignlib.py_makeAlignmentVector()
        
            alignlib.py_AlignmentFormatEmissions( 
...
def Write( map_row2col, row_seq, col_seq, link,
    ...
    if not status:
        ...
        f = alignlib.py_AlignmentFormatExplicit( map_row2col, row_seq, col_seq )
...
if __name__ == '__main__':
    ...
    map_row2col = alignlib.py_makeAlignmentVector()
    tmp1_map_row2col = alignlib.py_makeAlignmentVector()
    counts = {}
    ...
    for link in BlastAlignments.iterator_links( sys.stdin ):
        ...
        row_seq = alignlib.py_makeSequence( sequences[link.mQueryToken] )
        col_seq = alignlib.py_makeSequence( sequences[link.mSbjctToken] )
        ...
        if link.mQueryToken in map_old2new:
            ...
            if options.loglevel >= 3:
                options.stdlog.write( "# combining in row with %s\n" %\
                                      str(alignlib.py_AlignmentFormatEmissions(map_old2new[link.mQueryToken].mMapOld2New ) ))

            alignlib.py_combineAlignment( tmp1_map_row2col,
                                      map_old2new[link.mQueryToken].mMapOld2New,
                                      map_row2col,
                                      alignlib.py_RR )
            map_old2new[link.mQueryToken].clear()
            alignlib.py_copyAlignment( map_row2col, tmp1_map_row2col )
        ...
        if link.mSbjctToken in map_old2new:
            ...
            if options.loglevel >= 3:
                options.stdlog.write( "# combining in col with %s\n" %\
                                      str(alignlib.py_AlignmentFormatEmissions(map_old2new[link.mSbjctToken].mMapOld2New ) ))
        ...
        if dr < 0 or dc < 0:
            raise ValueError("out of bounds alignment: %s-%s: alignment out of bounds. row=%i col=%i ali=%s" %\
                                          (link.mQueryToken,
                                           ...
                                           row_seq.getLength(),
                                           col_seq.getLength(),
                                           str(alignlib.py_AlignmentFormatEmissions(map_row2col))))
            

        if options.loglevel >= 2:
            options.stdlog.write( str( alignlib.py_AlignmentFormatExplicit( map_row2col, 
                                                                         row_seq, 
                                                                         col_seq )) + "\n" )
        ...
        if options.require_codons:
            ...
            if naligned % 3 != 0:
                ...
                options.stdlog.write( "#\n%s\n" % alignlib.py_AlignmentFormatExplicit( map_row2col, 
                                                                                    row_seq,
                                                                                    col_seq ) )
        ...
        if exons:
            ...
            for a,b in segments:
                ...
                alignlib.py_copyAlignment( tmp1_map_row2col, map_row2col,
