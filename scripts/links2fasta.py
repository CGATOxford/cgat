...
def Write(map_row2col, row_seq, col_seq, link,
    ...
    if not status:
        ...
        f = alignlib_lite.py_AlignmentFormatExplicit(
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: links2fasta.py 2446 2009-01-27 16:32:35Z andreas $", usage=globals()["__doc__"])
    ...
    for link in BlastAlignments.iterator_links(sys.stdin):
        ...
        if options.require_codons:
            ...
            if naligned % 3 != 0:
                ...
                raise ValueError("incomplete codons %i in pair %s - %s" %
    ...
    if options.loglevel >= 1:
        options.stdlog.write("# %s\n" % ", ".join(
            map(lambda x, y: "%s=%i" % (x, y), counts.keys(), counts.values())))
        options.stdlog.write(
