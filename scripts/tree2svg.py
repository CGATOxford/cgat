    ...
    def __init__(self, tree, filename, *args, **kwargs):
        ...
        for line in infile:
            ...
            if line.startswith("table="):
                if table_id:
                    self.mTables[table_id] = table
                table_id = re.search("table=(\d+)", line).groups()[0]
                if int(table_id) == 0:
                    raise ValueError("table id 0 is invalid")
                table = []
                continue
...
def main(argv=None):
    ...
    parser.add_option("-a", "--annotations", dest="annotations", type="choice", action="append",
                      choices=(
                          "support", "error", "kaks", "master", "value", "tables"),
            ...
            if annotation == "support":
                ...
                if options.support_style == "pie":
                    plot.setDecoratorInternalNodes(
                        NodeDecoratorSupportPieChart(nexus.trees[current_tree]))
                ...
                b = BranchDecoratorTable(
                    tree, filename=options.filename_tables)
                plot.setDecoratorHorizontalBranches(b)
