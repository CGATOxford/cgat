...
def search_hierarchy(ID,hierarchy, options):
    ...
    for parent in entry['Parent']:
        ...
        new_gene_id, new_transcript_id, new_possible_transcript_id = search_hierarchy(parent, hierarchy, options)
    ...
    if options.missing_gene:
         possible_transcript_id = [entry['transcript_id'] if x is None else x for x in possible_transcript_id]
    ...
    if len(gene_id) == 0 and options.missing_gene:
        ...
        raise ValueError("Root found without finding an object of type %s" % options.gene_type)
    
    if entry['type'] == options.transcript_type:
        transcript_id = [entry['transcript_id'] if x is None else x for x in transcript_id]


    assert len(gene_id) == len(transcript_id) and len(transcript_id) == len(possible_transcript_id)
...
def main( argv = None ):
    ...
    if options.read_twice:
        ...
        second_gff = GFF3.flat_file_iterator(IOTools.openFile(options.stdin.name))
    ...
    for chunk in gffs:
        ...
        if options.method == "hierarchy":
            ...
            convert_set(chunk,options.gene_field_or_pattern, options.transcript_field_or_pattern,options)
