...
def filterJoiningTranscripts(links, options):
    ...
    for transcript1, transcript2, weight in links:
        ...
        if species1 not in transcripts:
            transcripts[species1] = []
        if species2 not in transcripts:
            transcripts[species2] = []

        transcripts[species1].append(transcript1)
        transcripts[species2].append(transcript2)

        if transcript1 not in matrix:
            matrix[transcript1] = {}
        if transcript2 not in matrix:
            matrix[transcript2] = {}
...
def isSynchronous(key1, key2, mali, exons):
    ...
    return (mstart1 < mstart2 and mend1 < mend2 and gstart1 < gstart2 and gend1 < gend2) or \
           (mstart2 < mstart1 and mend2 <
            mend1 and gstart2 < gstart1 and gend2 < gend1)
        ...
        for t1 in range(len(transcripts) - 1):
            ...
            for t2 in range(t1 + 1, len(transcripts)):
                ...
                for t3 in range(t2 + 1, len(transcripts)):
                    ...
                    overlap12 = getPercentOverlap(
                        mali[transcript1], mali[transcript2])
                    overlap13 = getPercentOverlap(
                        mali[transcript1], mali[transcript3])
                    overlap23 = getPercentOverlap(
                        mali[transcript2], mali[transcript3])
                    ...
                    if options.loglevel >= 2:
                        ...
                        joining_transcripts.add(
                            (transcript1, transcript2, "joining transcript"))
        ...
        for t1 in range(len(transcripts) - 1):
            ...
            for t2 in range(t1 + 1, len(transcripts)):
                ...
                if exons and hasGenomicOverlap(transcript1, transcript2, exons):
                    if options.loglevel >= 1:
                        options.stdlog.write(
                            "# transcript: %s removed because genomic overlap with %s\n" % (transcript2, transcript1))
                    removed_transcripts.add(
                        (transcript2, transcript1, "genomic overlap"))
                    continue

                if exons:

                    genomic_distance = getGenomicDistance(
                        transcript1, transcript2, exons)
                    is_synchronous = isSynchronous(
                        transcript1, transcript2, mali, exons)
                    ...
                    if genomic_distance > 0 and genomic_distance < options.min_genomic_distance and \
                        ...
                        removed_transcripts.add(
                            (transcript2, transcript1, "genomic proximity: %i/%5.2f%%" % (genomic_distance, overlap12)))
                        continue

                if overlap12 < options.max_percent_overlap:
                    if options.loglevel >= 1:
                        options.stdlog.write(
                            "# transcript: %s removed because no overlap with %s\n" % (transcript2, transcript1))
                    removed_transcripts.add(
                        (transcript2, transcript1, "no mali overlap"))
                    continue
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: mali2cleaned_mali.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    if options.mode == "joining":
        mapped_transcripts = removeJoiningTranscripts(
            mali, exons, map_species2transcripts, options)

    elif options.mode == "split":
        mapped_transcripts = removeSplitTranscripts(
            mali, exons, map_species2transcripts, options)
    ...
    for t1 in range(len(all_identifiers) - 1):
        ...
        if transcript1 in removed_transcripts:
            continue

        for t2 in range(t1 + 1, len(all_identifiers)):
            transcript2 = all_identifiers[t2]
            if transcript2 in removed_transcripts:
                continue
