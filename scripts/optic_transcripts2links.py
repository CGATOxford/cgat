...
Example::
   ...
   python optic_transcripts2links.py --help
...
""" program $Id: optic_transcripts2links.py 2781 2009-09-10 11:33:14Z andreas $
...
if __name__ == "__main__":
    ...
    parser = E.OptionParser( version = "%prog version: $Id: optic_transcripts2links.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    if options.method == "genes":
        ...
        transcripts = map(lambda x: x[:-1].split("\t")[0], filter( lambda x: x[0] != "#", sys.stdin.readlines()))

        if subset: transcripts = filter( lambda x: x in subset, transcripts )

        transcripts = map( lambda x: x[:-1].split(options.separator), transcripts )
