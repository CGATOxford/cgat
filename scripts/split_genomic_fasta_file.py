...
def processSequence(key, description_rest, sequence, options, mask_regions=None):
    ...
    while written < lsequence:
        ...
        chunk_size = min(
            options.chunk_size - global_written2file, lsequence - written)
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: split_genomic_fasta_file.py 1225 2007-04-10 15:13:11Z andreas $")
    ...
    for line in infile:
        ...
        if line[0] == ">":
            if fragments:
                nwritten, nmasked = processSequence(
                    key, description_rest, string.join(fragments, ""), options, mask_regions)
