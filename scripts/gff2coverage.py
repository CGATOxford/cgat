...
def processChunk(contig, chunk, options, fasta=None):
    ...
    if options.window_size:
        ...
        assert max_coordinate <= contig_length, "maximum coordinate (%i) larger than contig size (%i) for contig %s" % (
            max_coordinate, contig_length, contig)
        max_coordinate = contig_length
        window_size = int(math.floor(float(contig_length) / options.num_bins))
        num_bins = options.num_bins
    else:
        raise ValueError(
            "please specify a window size of provide genomic sequence with number of bins.")
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: gff2coverage.py 2781 2009-09-10 11:33:14Z andreas $", usage=globals()["__doc__"])
