...
Example::
   ...
   python gpipe_export_aaa.py --help
...
Version: $Id: gpipe_export_aaa.py 2781 2009-09-10 11:33:14Z andreas $
...
def WriteGeneTrack( outfile, dbhandle,
    ...
    if options.loglevel >= 2:
        options.stdlog.write( "statement for selecting genes:\n%s\n%s\n%s\n" % ("#" * 20, statement, "#"*20))
    ...
    for sbjct_token, sbjct_strand, gene_id, feature_from, feature_to in rr:
        ...
        if gene_id not in map_gene_id:
            map_gene_id[gene_id] = "%s_%s_%s_%s" % ( options.species_id, options.group_id, options.method_id, current_id)
            current_id += 1
...
def WriteMRNATrack( outfile, dbhandle,
    ...
    if options.loglevel >= 2:
        options.stdlog.write( "statement for selecting mRNAs:\n%s\n%s\n%s\n" % ("#" * 20, statement, "#"*20))
    ...
    for sbjct_token, sbjct_strand, gene_id, prediction_id, feature_from, feature_to in rr:
        ...
        if gene_id not in map_gene_id:
            options.stderr.write("# warning: gene_id %i not in set\n" % gene_id)
            continue

        if prediction_id not in map_prediction_id:
            map_prediction_id[prediction_id] = "%s_%s_%s_%s" % ( options.species_id, options.group_id, options.method_id, current_id)
            current_id += 1
...
def WriteCDSTrack( outfile, dbhandle,
    ...
    if options.loglevel >= 2:
        options.stdlog.write( "statement for selecting cds:\n%s\n%s\n%s\n" % ("#" * 20, statement, "#"*20))
    ...
    for sbjct_token, sbjct_strand, gene_id, prediction_id, feature_from, feature_to in rr:
        ...
        if prediction_id not in map_prediction_id:
            options.stderr.write("# warning: prediction_id %i not in set\n" % prediction_id)
            continue
        ...
        outfile.write( "\t".join((sbjct_token,
                                  ...
                                  sbjct_strand,
                                  ".",
                                  ";".join( ("PARENT=%s" % map_gene_id[gene_id],)),
                                  )) + "\n")
...
def WriteGFF( outfile, dbhandle, options ):
    ...
    parameters = {}
    parameters['tablename_predictions'] = options.schema + "." + options.tablename_predictions
    parameters['tablename_genes'] = options.schema + "." + options.tablename_genes
    parameters['tablename_redundant'] = options.schema + "." + options.tablename_redundant
    parameters['tablename_contigs'] = options.schema + "." + options.tablename_contigs
    parameters['tablename_exons'] = options.schema + "." + options.tablename_exons
    parameters['tablename_quality'] = options.schema + "." + options.tablename_quality    

    if options.tablename_orthologs:
        parameters['extra_tables'] = ",%s AS oo" % options.tablename_orthologs
        parameters['extra_where'] = "AND oo.prediction_id1 = p.prediction_id AND oo.schema1='%s' " % (options.schema)
...
def WriteMap( options, category, map_id):
    ...
    outfile = open(options.filename_pattern_map % (options.assembly_id, category), "w")
    ids = map_id.keys()
    ids.sort()
    for id in ids:
        outfile.write( "%i\t%s\n" % (id, map_id[id]) )
...
if __name__ == "__main__":
    ...
    parser = E.OptionParser( version = "%prog version: $Id: gpipe_export_aaa.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    if options.filename_batch:
        ...
        for line in infile:
            ...
            map_gene_id, map_prediction_id = WriteGFF( outfile, dbhandle, options )
