...
def doColumnInTables(dbhandle, error, options):
    ...
    for x in tables:
        try:
            heads.add(reg_head.search(x).groups()[0])
        except AttributeError:
            pass
        try:
            tails.add(reg_tail.search(x).groups()[0])
        except AttributeError:
            pass
    ...
    for table in tables:
        ...
        try:
            x = map_head2index[reg_head.search(table).groups()[0]]
        except AttributeError:
            continue
        try:
            y = map_tail2index[reg_tail.search(table).groups()[0]]
        except AttributeError:
            continue
...
def main(argv=None):
    ...
    if options.method == "column-in-tables":
        ...
        counts, row_names, column_names = doColumnInTables(
            dbhandle, error, options)

    elif options.method == "columns-in-table":
        if not options.table:
            raise ValueError("please supply a table name or pattern")
        counts, row_names, column_names = doColumnsInTable(
            dbhandle, error, options)
