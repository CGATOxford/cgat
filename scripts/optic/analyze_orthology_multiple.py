...
"""program $Id: optic/analyze_orthology_multiple.py 2781 2009-09-10 11:33:14Z andreas $
...
def readAnnotationInterpro( infile ):
    ...
    for line in infile:
        ...
        map_id2annotation[id].append(Annotation( identifier, short, description ))
...
class Duplication:
    ...
    def __str__(self):
        ...
        return "\t".join( (self.mGeneTree,
                           ...
                           ",".join([ self.format_branch_length % x for x in self.mDistance2Children ]),
                           str(self.mNGenes),
    ...
    def readFromString(self, line):
        ...
        self.mDistance2Root, self.mDistance2Min, self.mDistance2Max, self.mRelativeHeight, self.mTotalDistance, self.mRelativeDistance, self.mDistance2Parent = \
                      map( float, (d2root, d2min, d2max, relative_height, total_distance, rel_distance, distance_to_parent ) )
...
class BranchDecoratorHorizontalDistributions( SVGTree.BranchDecoratorHorizontal ):
    ...
    def getValues( self, node_id, contents ):
        ...
        if node_id not in self.mMapNode2Counts: return []
        vals = []
        for c in contents:
    ...
    def getElements( self, node_id, x1, x2, y ):
        ...
        if self.mMaxDistance == None:
            ...
            max_value = min( max( map (lambda x: max(x), values) ), self.mMaxDistance )
        else:
...
class DistributionBox:
    ...
    def getElements( self, x1, y1, x2, y2, values,
        ...
        bins = [ min_value + increment_values * x for x in range( 0, num_bins + 1) ] 
...
class NodeDecoratorDuplications(SVGTree.NodeDecorator):
    ...
    def getElements( self, node_id, x, y):
        ...
        if t not in self.mMapType2Colour:
            self.mMapType2Colour[t] = SVGTree.COLOURS[len(self.mMapType2Colour) % len(SVGTree.COLOURS) ]

        e = []
        e.append( SVGdraw.circle( x, y,
                                  self.mRadius,
                                  stroke = "rgb(%i,%i,%i)" % self.mMapType2Colour[t],
    ...
    if options.output_pattern:
        ...
        if section in opened_files:
            ...
            if options.loglevel >= 1:
                options.stdout.write("# output for section %s goes to %s\n" % (section, fn))
...
def extractSubtrees( tree, extract_species, options ):
    ...
    def update_groups( node_id ):
        ...
        if node.succ == []:
            ...
            # print node_id, a, b, "oa=",oa, "ob=", ob, "ia=", ia, "ib=",ib, "ovl=",overlap, "merge=",merge
...
def printSummary( counts_categories, counts_trees, counts_nodes, duplications,
    ...
    * 1:1 orthologs?
    ...
    for s in species_list: lineage_specific_duplications[s] = 0
    ...
    nproblems = sum( [ len(counts_categories[x]) for x in options.nodetypes_inconsistency ] )
...
def printCounts( counts_categories, counts_trees, counts_nodes,
    ...
    for k,c in counts_categories.items():
        outfile.write("%s%s\t%i\t%s\n" % (prefix_row,k,len(c), ",".join(map(lambda x: options.format_branch_length % x, c))))
        ...
        for node,types in counts_nodes[tree].items():
            for type, heights in types.items():
                ...
                outfile.write("%s%i\t%i\t%s\t%s\t%s\t%s\n" % (prefix_row,
                                                              ...
                                                              type,
                                                              str(s),
                                                              ",".join(map(lambda x: options.format_branch_length % x, heights)),
...
def printBestNodes( best_nodes, options, prefix_header, prefix_row ):
    ...
    if is_new:
        outfile.write( "%sgenenode\tspeciesnode\ttree\ttype\theight\tspecies\totus\n" % prefix_header )

    for best in best_nodes:

        if best == None: continue
        if best.mType == "Leaf": continue
        ...
        outfile.write("\t" + ",".join( species_nexus.trees[best_tree].get_taxa( best.mSpeciesNode) ) )
...
def printClusters( clusters, options, prefix_prefix, otus ):
    ...
    if options.loglevel >= 1:
        options.stdlog.write("# written: %i/%i subtrees (%5.2f%%) %i/%i (%5.2f%%) taxa\n" % (written_subtrees,len(clusters),
                                                                                             100.0 * written_subtrees / len(clusters),
                                                                                             nclustered_taxa, len(otus),
...
def printSVGSpeciesTrees( trees, all_counts_nodes, options ):
    ...
    for ntree in range(len(species_nexus.trees)):
        ...
        plot.setDecoratorHorizontalBranches( BranchDecoratorHorizontalDistributions(tree, counts_nodes) )                
        ...
        if options.output_pattern_svg:
            ...
            if options.loglevel >= 1:
                options.stdlog.write( "# svg output for tree %s goes to %s.\n" % (tree.name, fn))
...
def printSVGGeneTree( tree, options, extract_species ):
    ...
    if options.filename_species2url:
        if not map_species2url:
            map_species2url = IOTools.ReadMap( open(options.filename_species2url, "r"))
    ...
    if options.filename_species2colour:
        if not map_species2colour:
            map_species2colour = IOTools.ReadMap( open( options.filename_species2colour, "r") )
    else:
        map_species2colour = None

    if options.filename_species2name:
        if not map_species2name:
            map_species2name = IOTools.ReadMap( open( options.filename_species2name, "r") )
    else:
        map_species2name = None
...
##------------------------------------------------------------------------------------------
##------------------------------------------------------------------------------------------        
def getReferenceHeight( distance_to_root, gene_tree, input_set, options, extract_species = str, method = "median" ):
    """return height of tree.

    applies filtering opions from options. Method describes how the gene tree
    is computed from the leaf distances. Possible values are median, mean, min, max
    """
    
    ## get maximum height - only genes that are not outgroups
    if options.outgroup_species:
        max_length_set = filter( lambda x: extract_species(gene_tree.node(x).data.taxon) not in options.outgroup_species, input_set )
    else:
    ...
    if method == "median":
        reference_height = numpy.median( map(lambda x: distance_to_root[x], max_length_set ) )
    elif method == "min":
        reference_height = min( map(lambda x: distance_to_root[x], max_length_set ) )
    elif method == "max":        
        reference_height = max( map(lambda x: distance_to_root[x], max_length_set ) )                
    elif method == "mean":        
        reference_height = numpy.mean( map(lambda x: distance_to_root[x], max_length_set ) )
    else:
...
def analyzeDuplicationData( duplications, species_nexus, options, extract_gene = None, extract_species = None,
    ...
    def getDuplicationCounts( duplications ):
        ...
        for duplication in duplications:
            ...
            if duplication.mSpeciesTree != 0: continue

            if mode == "genes":
                if duplication.mNPseudogenes or duplication.mNOthers: continue
            elif mode == "pseudogenes":
                if duplication.mGenes or duplication.mNOthers: continue                
                
            try:
                counts_per_node[duplication.mSpeciesNode].append( duplication.mRelativeDistance )
            except KeyError:
                counts_per_node[duplication.mSpeciesNode] = [ duplication.mRelativeDistance ]
    ...
    def printDuplicationHistograms( outfile, counts, species_tree, options, cumulate = False, normalize = False ):
        ...
        for t in sorted(counts.keys()):
            ...
            if cumulate: Histogram.cumulate( h )
            if normalize: Histogram.normalize( h )
    ...
    def printDuplicationsLocal( outfile, duplications, species_tree, options, extract_gene, extract_species, gene_nexus, map_id2location ):
        ...
        for duplication in duplications:
            ...
            otus.sort( lambda x, y: cmp( map_id2location[x].mId, map_id2location[y].mId ))

            def printOTU( last_otu ):
                try:
                    outfile.write("%s\t%s\t%s\t%i\t%s\n" % (extract_species(last_otu),
                                                            extract_gene(last_otu),
                                                            last_otu,
                                                            map_id2location[last_otu].mId,
    ...
    def printDuplicationsSVG( duplications, species_tree, options, extract_gene, extract_species, gene_nexus, map_id2location ):
        ...
        duplications.sort( lambda x, y: cmp( (x.mGeneTree, x.mNChildren), (y.mGeneTree, y.mNChildren) ) )
        ...
        for duplication in duplications:
            ...
            if options.loglevel >= 3:
                options.stdlog.write("# processing duplication %s\n" % (str(duplication)) )
            ...
            for species in set(map(extract_species, otus )):
                ...
                data[species].append( (len(c), len(new_otus), min_pos, max_pos, new_otus, tree) ) 
        ...
        for species in species_list:
            ...
            if len(data[species]) == 0: continue
            ...
            for cis, ndups, min_pos, max_pos, otus, tree in data[species]:
                ...
                for children, height, branchlength in s:
                    ...
                    if options.loglevel >= 5:
                        options.stdlog.write("# species %s - %s: adding duplication to plot:\n" % ( species, tree.name ))                        
            ...
            if options.output_pattern_svg:
                ...
                if options.loglevel >= 1:
                    options.stdlog.write( "# svg output for wheel %s goes to %s.\n" % (tree.name, fn))
                    options.stdlog.flush()
                outfile = open(fn, "w")
            else:
                outfile = options.stdout
    ...
    def annotateDuplications( outfile, duplications, species_tree, options, extract_gene, gene_nexus, map_id2location ):
        ...
        """
        map_id2annotations = readAnnotationInterpro( open( options.filename_map_annotation_interpro, "r") )
        ...
        for duplication in duplications:
            ...
            if len(taxa) > 1: continue
            if taxa[0] != "dmel_vs_dmel4": continue
        ...
        if options.loglevel >= 1:
            options.stdlog.write("# analysing duplications in %i clusters - %i skipped.\n" % (len(dups_per_tree), nskipped) )
        ...
        for gene_tree_name, dups in dups_per_tree.items():
            ...
            if options.loglevel >= 2:
                options.stdlog.write("# %s: %i duplications\n" % ( gene_tree.name, len(dups) ) )
            ...
            for t in taxa:
                ...
                if gene in found_genes: continue

                genes.append(gene)
                if map_id2location:
                    l = map_id2location[t]
                    locations.append( ":".join( map( str, (l.contig, l.strand, l.mFrom, l.mTo) ) ) )
    ...
    for method in options.analyze_duplication_data:
        ...
        if method == "cumul-norm-hist":
            ...
            annotateDuplications( outfile, duplications, species_nexus.trees[0], options, extract_gene, gene_nexus, map_id2location )

        elif method == "members":
            printDuplicationsMembers( outfile, duplications, species_nexus.trees[0], options, extract_gene, extract_species, gene_nexus, map_id2location )

        elif method == "svg-wheel-plot":
            printDuplicationsSVG( duplications, species_nexus.trees[0], options, extract_gene, extract_species, gene_nexus, map_id2location )
...
def analyzeResolutionData( infile, options ):
    ...
    def printResolutionCounts( outfile, data, field, f, options, prefix_header = "", prefix_row = "" ):
        ...
        outfile.write( "%s%s\t%s\n" % (prefix_header,  "type", "\t".join(Stats.DistributionalParameters().getHeaders())))
        for t in sorted(counts.keys()):
    ...
    ##------------------------------------------------------------------------------------------
    def middle( *args ): return sorted(args)[1]
    ...
    for line in infile:
        if line[0] == "#": continue
        if line[:4] == "type": continue

        cluster, gene_node_id, type, tree, d_np, d_nl, d_nr = line[:-1].split("\t")[:7]

        d_np, d_nl, d_nr = map(float, (d_np, d_nl, d_nr) )

        total_resolution.append( (cluster, gene_node_id, type, tree, d_np, d_nl, d_nr ) )
        ...
        if method == "stats":
            ...
            for field, name in ( (2, "category"), (3, "tree") ):
                for f, method in ( ( min, "min" ), (middle, "middle" ) ):
                    ...
                    outfile, is_new = getFile( options, section )
                    printResolutionHistograms( outfile, total_resolution, field, f, options )
                    if outfile != options.stdout:
                        outfile.close()
...
def annotateDuplicationEvents( best, gene_tree, species_nexus, best_nodes, counts_duplications, duplications,
    ...
    parent, distance_to_parent = TreeTools.GetParentNodeWhereTrue( best.mGeneNode, gene_tree, stop_function )

    ## find child speciation
    children = TreeTools.GetChildNodesWhereTrue( best.mGeneNode, gene_tree, stop_function ) 
    ...
    if False:
        ...
        if best.mType != "DuplicationLineage":
            ...
            ## nodes of children will differ). Turned off, might add option later.
            if False:
...
def readSpeciesTrees( options ):
    ...
    if options.filename_species_tree:
        species_tree_lines = open(options.filename_species_tree, "r").readlines()
...
def processGeneTrees( gene_nexus, options ):
    ...
    for gene_tree in gene_nexus.trees:
        ...
        if len(otus) <= 1:
            nfiltered += 1
            if options.loglevel >= 1:
                options.stdlog.write("# tree %s: less than two species (%i) after filtering - skipped.\n" % (gene_tree.name, len(otus) ) )
                options.stdlog.flush()                    
            continue
        ...
        if options.outgroup_species and not set(map( extract_species, otus )).difference( options.outgroup_species):
            nfiltered += 1
            if options.loglevel >= 1:
                options.stdlog.write("# tree %s: only outgroups after filtering - skipped.\n" % gene_tree.name )
            continue
...
""" )
    ...
    parser.set_defaults(
        ...
        nodetypes_duplication = ("Duplication", "DuplicationLineage", "DuplicationDeletion" ),
        ...
        svg_wheel_plot_quality2symbol = { 'CG' : "circle", 'PG' : "circle", 'SG' : "circle" },
        svg_wheel_plot_quality2mask = ("RG", "CP", "PP", "SP", "RP", "CF", "PF", "SF", "UG", "UP", "UF", "BF", "UK" ),
        svg_template = "screen",
        svg_add_legend = True,
        )
    ...
    if options.filename_species_list:
        species_list, nerrors = IOTools.ReadList( open( options.filename_species_list, "r") )
        ## prune species trees 
    ...
    if options.filename_filter_positives:
        filter_positives, nerrors = IOTools.ReadList( open(options.filename_filter_positives, "r") )
    ...
    if options.filename_locations:
        ...
        if options.loglevel >= 1:
            options.stdlog.write("# reading locations from %s\n" % options.filename_locations )
            options.stdlog.flush()
            
        map_id2location = readLocations( open(options.filename_locations, "r"), extract_species )
    ...
    #########################################################################
    if not options.skip_preprocessing and gene_nexus != None and len(gene_nexus.trees) > 0:
    ...
    for x in options.priority.keys(): total_counts_categories[x] = []
    ...
    if not options.filename_node_types:
        ...
        for gene_tree in gene_nexus.trees:
            ...
            ## get analysis set
            #########################################################################                    
            analysis_set, gene_set, pseudogene_set, other_set = getAnalysisSets( gene_tree, extract_quality, options )
            
            if len(analysis_set) == 0:
                nskipped_analysis_set += 1
                if options.loglevel >= 1:
                    options.stdlog.write("# tree %s: empty analysis set - skipped.\n" % gene_tree.name )
                continue
            ...
            #########################################################################
            min_branch_lengths, max_branch_lengths = TreeTools.GetBranchLengths( gene_tree )
            ...
            if reference_height == None:
                if options.loglevel >= 1:
                    options.stdlog.write("# tree %s: reference height not computable or 0 - skipped.\n" % gene_tree.name )
            ...
            if options.filter_quality != "all":
                ...
                TreeTools.PruneTree( gene_tree, set( [ gene_tree.node(x).data.taxon for x in analysis_set] ))
                ...
                if len(otus) <= 1:
                    nfiltered += 1
                    if options.loglevel >= 1:
                        options.stdlog.write("# tree %s: empty after filtering by analysis set - skipped.\n" % gene_tree.name )
                        options.stdlog.flush()                    
                    continue
            ...
            for n in range( len(node_types) ):
                ...
                if options.loglevel >= 2:
                    ...
                    for s in gene_tree.node(node_type.mGeneNode).succ:
                        options.stdout.write("#" + "\t" * 5 + ",".join( tuple(set(map(extract_species, gene_tree.get_taxa( s )))) ) + "\n" )
            ...
            for x in range( nspecies_trees ):
                a = {}
                for y in options.priority.keys(): a[y] = 0
            ...
            counts_categories = {}
            for x in options.priority.keys(): counts_categories[x] = []
            ...
            for best in best_nodes:
                ...
                counts_nodes[best_tree][best.mSpeciesNode][best.mType].append( height )
                ...
                if node.succ:
                    total_resolution.append( ( gene_tree.name, best.mGeneNode,
                                               ...
                                               gene_tree.node(node.succ[0]).data.branchlength,
        ...
        if len(gene_nexus.trees) > 0:
            ...
            if total_resolution and options.print_totals:
                printResolution( total_resolution, options, prefix_header, prefix_row )
        ...
        if options.filename_node_types:
            if options.loglevel >= 1:
                options.stdlog.write("# reading previous counts from %s.\n" % options.filename_node_types)
                
            total_counts_nodes = readCountsNodes( open( options.filename_node_types, "r" ), nspecies_trees )

        elif options.filename_duplications:

            total_duplications = readDuplications( open(options.filename_duplications, "r") )
