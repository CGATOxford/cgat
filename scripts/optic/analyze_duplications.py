...
def fit( histogram, parameters, fresiduals = residuals_decay ):
    ...
    try:
        result = scipy.optimize.leastsq(fresiduals, parameters, args=(in_y, in_x), full_output = True)
...
if __name__ == "__main__":
    ...
    parser = E.OptionParser( version = "%prog version: $Id: optic/analyze_duplications.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    if options.method in ("counts", "medians"):
        ...
        for location in options.locations:
            ...
            for function in options.functions:
                matrix = numpy.zeros( (len(options.species), len(options.species)), numpy.Float )
                ...
                for species1, species2, cluster_id, l, f, height, locations, tree in data:
                    ...
                    if last_species1 != species1 or last_species2 != species2:
                        ...
                        if len(values) > 0:
                            matrix[map_species2pos[last_species1],map_species2pos[last_species2]] = func(values)                            
        ...
        for location in options.locations:
            ...
            for function in options.functions:
                ...
                values = [ [ [] for y in range(len(options.species)) ] for x in range(len(options.species)) ]
                    ...
                    if last_species1 != species1 or last_species2 != species2:
                        ...
                        if options.filename_output:
                            if options.method == "lists":
                                ...
                                dict = { "f": function,  "l": location, "s": species1, "o": species2}
                                written = {}
                                outfile = open(options.filename_output % dict, "w" )                                
                            elif options.method == "lists-union":
                                if last_species1 != species1:
                                    if outfile: outfile.close()                                    
                                    dict = { "f": function,  "l": location, "s": species1 }                                    
                                    written = {}
                                    outfile = open(options.filename_output % dict, "w" )
                        else:
                            outfile = sys.stdout                    
                            if options.method == "lists":
                                ...
                                if last_species1 != species1:
                                    outfile.write( "location: %s, function: %s, species1: %s\n" % (location, function, species1))
                                    written = {}
        ...
        for location in options.locations:
            ...
            for function in options.functions:
                ...
                for species1, species2, cluster_id, l, f, height, locations, tree in data:
                    try:
                        values[map_species2pos[species1]][map_species2pos[species2]].append(float(height))
                    except KeyError:
                        continue
                ...
                for s in options.species:
                    ...
                    if options.method == "hists":
                        combined_histogram = Histogram.Combine( histograms, missing_value = "-" )
        ...
        for location in options.locations:
            ...
            for function in options.functions:
                ...
                if options.loglevel >= 2:
                    options.stdlog.write("#   processing function %s " % function )
        ...
        for location in options.locations:
            ...
            for function in options.functions:
                ...
                for species1, species2, cluster_id, l, f, height, locations, tree in data:
                    values[map_species2pos[species1]][map_species2pos[species2]].append( (cluster_id, -len(locations), locations, tree ) )
                ...
                for s in options.species:
                    ...
                    for x in range(len(options.species)):
                        ...
                        ## write trees per cluster
                        if options.filename_output:
                            dict = { "f": function,  "l": location, "s": s, "o": map_pos2species[x]}
                            outfile = open(options.filename_output % dict, "w" )
                        else:
                            outfile = sys.stdout                    
