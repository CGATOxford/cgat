...
def AnalyseOrphans( orphans, outfile,
    ...
    sums = {}
    for x in categories: sums[x] = {'all' : 0 } 
    ...
    if map_query2best:
        ...
        if best_match:
            ...
            if is_alternative:
                ...
                if not status:
                    # check if best prediction overlaps with assignments to other genes
                    overlaps = []
...
def PrintCrossAssignments( outfile,
    ...
    for g in found.keys():
        ...
        if l > 1:
            ...
            if is_alternative_transcripts:
                ...
                if map_contig2junk and map_transcript2location:
                    ...
                    # only one gene without junk location, all others with junk location
        ...
        if l > 1:
            ...
            # check transcripts for genes in other genome and see if they overlap.
...
def GetAssignments( genes, map_transcript2location, separator = "|" ):
    ...
    for g, vv in genes.items():
        ...
        for v in vv:
            ...
            ( sbjct_token, sbjct_strand, sbjct_genome_from, sbjct_genome_to ) = map_transcript2location[t]
            if min_from == 0:
...
def RemoveRedundancy( outfile,
    ...
    for tt in transcripts:
        try:
            ...
            raise "key %s not in map_transcript2location, examples are: %s" % (tt.mTranscript, map_transcript2location.keys()[:10])
        
        assignments.append( (sbjct_token, sbjct_strand, sbjct_genome_from, sbjct_genome_to, tt ) )
                ...
                for tt3, score in graph[tt1]:
                    ...
                    if tt1 in cds1 and tt2 in cds1 and \
                       (cds1[tt1] == cds1[tt2] or \
                        ...
                        within_scores.append( score )
                        if options.loglevel >= 3:
                            outfile.write( "# %s\t%s\t%6.4f\tsame sequence\n" % (tt1, tt2, score) )
                    else:
                        ## check if same gene
                        ...
                        if xg != yg:
                            if options.loglevel >= 3:                            
                                outfile.write( "# %s\t%s\t%6.4f\tlink not found\n" % (tt1, tt2, score) )                        
                            nmissed_links_within += 1
                        else:
                            if options.loglevel >= 3:                            
                                outfile.write( "# %s\t%s\t%6.4f\tsame gene\n" % (tt1, tt2, score) )
            # between links
            for tt2 in t2:
                for tt3, score in graph[tt1]:
                    if tt3 == tt2:
        ...
        for y in range(x+1,len(gg)):
            for tx in genes[gg[x]]:
                for ty in genes[gg[y]]:
                    ...
                    p = AlignedPairs.AlignedPair( sequence1=cds[tx], sequence2=cds[ty] )
                    p.Align( MyAlignFunction, verbose = 0 )

                    if tx not in pairs: pairs[tx] = {}
                    if ty not in pairs: pairs[ty] = {}                    
                    pairs[tx][ty] = p
                    pairs[ty][tx] = p
...
def CheckLocations( genes,
    ...
    for g, vv in genes.items():
        ...
        for v in vv:
            ...
            ( sbjct_token, sbjct_strand, sbjct_genome_from, sbjct_genome_to ) = map_transcript2location[v.mTranscript]
        ...
        if sbjct_token not in contigs: contigs[sbjct_token] = []
...
def UpdateCountsForCluster( outfile,
    ...
    for tree in trees:
        ...
        is_monophyletic = TreeTools.IsMonophyleticForTaxa( tree, str_transcripts )
        ...
        if is_monophyletic:
            ...
            if options.loglevel >= 4:
                options.stdlog.write( "# counted branchpoints in %5.2f seconds.\n" % (time.time() - time_t1) )
                options.stdlog.flush()
            ...
            if branchpoints:
                ...
                for str_children, height, branchlength in branchpoints:
                    ...
                    if len(str_children) <= 1: continue

                    children = map( lambda x: Orthologs.Transcript(x), str_children)
                    ...
                    duplications.append ( (cluster_id, this_status, duplication_status, height, children) )
                    ...
                    for child in children:
                        if child.mGene in this_gene2location:
                            locs.append( "%s:%s:%s:%i:%i" % ((child.mGene,) + this_gene2location[child.mGene] ))
                ...
                if ortholog_distance > 0:
                    rel_heights = map(lambda x: x/ortholog_distance, distances)
                else:
                    rel_heights = "NaN"
    ...
    for b in range(0, num_bins):
        ...
        for x in categories:
            if x in histograms1 and x in histograms2:
                outfile.write( "\t%i\t%i" % (histograms1[x][b], histograms2[x][b] ))
            else:
                outfile.write("\t0\t0")
                
        outfile.write("\n")
    outfile.write("total")
    outfile.write( "\t%i\t%i" % (reduce(lambda x,y: x+y, h0), reduce(lambda x,y: x+y, h0)) )
    for x in categories:
        if x in histograms1 and x in histograms2:        
    ...
    for member in members:
        outfile.write( "%s\t%i\t%s\t%i\t%s\t%s\t%s\n" % \
                       ("pairs",
                        ...
                        "\t".join(map(str, [ results_locations[x] for x in GetLocationKeys() ] )),
...
def AnalyseDuplications( outfile,
    ...
    for cluster_id in range(len(orthologs)):
        ...
        for tree in trees[cluster_id]:
            ...
            # print "distances2", TreeTools.GetDistancesBetweenTaxa( tree, t1, t2 )            
            orthologs_distances += TreeTools.GetDistancesBetweenTaxa( subtree, t1, t2 )
            
        if options.loglevel >= 4:
            options.stdlog.write( "# collected distances between orthologs in %5.2f seconds.\n" % (time.time() - time_t1) )
            options.stdlog.flush()

        orthologs_distances = map(lambda x: x[2], orthologs_distances )
        
        if len(orthologs_distances) == 0:
            outfile.write("# skipped %i: no ortholog distances.\n" % (cluster_id))
            nskipped += 1
            continue
        ...
        if key not in histogram_data: histogram_data[key] = [[],[]]
        ...
        time_t1 = time.time()
        PrintResultsDuplicationsPairs( outfile, cluster_id, schema1, transcripts1, duplications2, ortholog_distance )
        PrintResultsDuplicationsPairs( outfile, cluster_id, schema2, transcripts2, duplications1, ortholog_distance )        

        if options.loglevel >= 4:
            options.stdlog.write( "# %i: Output: %5.2f seconds.\n" % (cluster_id, time.time() - time_t1) )
            options.stdlog.flush()
    ...
    for key in kk:
        ...
        outfile.write("duplication\t%s\t%s\t%s\t%s\t%i\t%i\t%s\t%s\n" %\
                      (schema1, schema2,
                       ...
                       ",".join( map(lambda x: "%6.4f" % x, histogram_data[key][0])),
                       ",".join( map(lambda x: "%6.4f" % x, histogram_data[key][1])),                      
                       ) )
...
def GetPrediction2LocationFromFile( infile, schema, options, use_genes = False ):
    ...
    for line in infile:
        if line[0] == "#": continue
        id, sbjct_token, sbjct_strand, phase, n, p1, p2, sbjct_genome_from, sbjct_genome_to = line[:-1].split("\t")

        sbjct_genome_from, sbjct_genome_to = int(sbjct_genome_from), int(sbjct_genome_to)
        if use_genes:
        ...
        if prediction_id not in map_transcript2location:
            map_transcript2location[prediction_id] = ( sbjct_token, sbjct_strand, sbjct_genome_from, sbjct_genome_to )
        else:
            o_sbjct_token, o_sbjct_strand, o_sbjct_genome_from, o_sbjct_genome_to = map_transcript2location[prediction_id]
            map_transcript2location[prediction_id] = (sbjct_token, sbjct_strand,
                                                      min(sbjct_genome_from, o_sbjct_genome_from),
...
def GetPrediction2Location( dbhandle, schema,
    ...
    for prediction_id, sbjct_token, sbjct_strand, sbjct_genome_from, sbjct_genome_to in rr:
        map_transcript2location[str(prediction_id)] = ( sbjct_token, sbjct_strand, sbjct_genome_from, sbjct_genome_to )
...
def GetRepeats( dbhandle, schema, repeats_queries ):
    ...
    for r in rr: repeats_list[r[0]] = 1
...
def GetBestMatches( dbhandle, schema ):
    ...
    for r in rr:
        ...
        if last_query_token != query_token:
            if best_match:
                map_query2best[last_query_token] = (nmatches, best_pide, best_coverage) + tuple(best_match)
        ...
        if query_coverage > best_coverage: best_coverage = query_coverage
        if pidentity      > best_identity: best_identity = pidentity        
        nmatches += 1
...
def ReadOrphans( infile, options ):
    ...
    for line in infile:
        ...
        transcripts = map( lambda x: Orthologs.Transcript(x), line[:-1].strip().split("\t"))
...
def main( argv = None ):
    ...
    parser = E.OptionParser( version = "%prog version: $Id: optic/analyze_orthology.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    if options.repeats_list:
        data = map( lambda x: x[:-1].split("\t")[0], filter( lambda x: x[0] != "#", open(options.repeats_list, "r").readlines()))
    ...
    if options.filename_links:
        ...
        for line in infile:
            ...
            if key1 not in graph_genes: graph_genes[key1] = {}
            x = graph_genes[key1]
            if key2 not in x:
                x[key2] = score
            else:
            ...
            for t in (transcript1, transcript2):
                ...
                if t.mSchema == options.schema1:
                    ...
                    if t.mGene not in all_genes_schema1: all_genes_schema1[t.mGene] = 0
                    all_genes_schema1[t.mGene] += 1
                elif t.mSchema == options.schema2:
                    map_transcript2gene_schema2[t.mTranscript] = t.mGene
                    if t.mGene not in all_genes_schema2: all_genes_schema2[t.mGene] = 0
            ...
            if g not in aligned_genes_schema1: aligned_genes_schema1[g] = {}
            aligned_genes_schema1[g][ss[options.schema2]] = score

            g = ss[options.schema2]
            if g not in aligned_genes_schema2: aligned_genes_schema2[g] = {}
        ...
        if options.loglevel >= 1:
            print "# graph: read %i/%i vertices, ninput=%i, nskipped=%i" %\
                  (len(all_genes_schema1), len(all_genes_schema2), ninput, nskipped)        
    ...
    if options.filename_interpretation:
        ...
        for t1, t2, g1, g2, w in orthologs:
            ...
            for t in t1: map_transcript2cluster1[t.mTranscript] = cluster_id
            for t in t2: map_transcript2cluster2[t.mTranscript] = cluster_id   
            cluster_id += 1
            ...
            for gg in g1.keys():
                x = (True, gg)
                if x not in graph_ortholog_genes: graph_ortholog_genes[x]=[]
                for g in g1.keys() : graph_ortholog_genes[x].append( (True, g) )
                for g in g2.keys() : graph_ortholog_genes[x].append( (False, g) )
    ...
    if "orphans" in options.methods and options.filename_orphans:
        ...
        orphans1, orphans2 = ReadOrphans( open(options.filename_orphans,"r"), options )
    ...
    if not options.skip_locations and \
       ("duplications" in options.methods or "crossassignments" in options.methods):
        ...
        if options.loglevel >= 1:
            options.stdlog.write( "# locations: read %i/%i locations." % (len(map_transcript2location2), len(map_transcript2location2)) + "\n" )
            options.stdlog.flush()
    ...
    if options.filename_cds1 and options.filename_cds2 and \
        ...
        for x,y in cds1.items(): cds1[x] = y.upper()
        for x,y in cds2.items(): cds2[x] = y.upper()        
    ...
    if options.filename_kaks:
        ...
        if not os.path.exists( options.filename_kaks ):
            if options.loglevel >= 1:
                options.stdlog.write("# filename %s not found - kaks analysis skipped.\n" % (options.filename_kaks ))
            ...
            for line in infile:
                ...
                if a not in graph_kaks: graph_kaks[a] = []
                if b not in graph_kaks: graph_kaks[b] = []            
        ...
        for line in infile:
            ...
            for tt in TreeTools.GetTaxa( tree ):
                ...
                if transcript.mTranscript in map_transcript2cluster1:
                    clusters[map_transcript2cluster1[transcript.mTranscript]] = 1
                elif transcript.mTranscript in map_transcript2cluster2:
                    clusters[map_transcript2cluster2[transcript.mTranscript]] = 1
...
##                 continue
        ...
        #########################################################################################            
            ...
            for t1, t2, g1, g2, w in orthologs:
                ...
                if len(g1) == 1 and len(g2) > 1: row['hook'] = g1.keys()[0]
                if len(g2) == 1 and len(g1) > 1: row['hook'] = g2.keys()[0]                
        ...
        #########################################################################################
            ...
            for xt1, xt2, xg1, xg2, w in orthologs:
                ...
                if dg not in h_dgenes: h_dgenes[dg] = 0
...
# all: percentage based on all genes in genomes 1 and 2.\n""")
                ...
                if x[0] > 0 and x[1] > 0:
                    ...
                    outfile.write("%i\t%i\t%i\t%6.4f\t%6.4f\t%6.4f\t%6.4f\t%6.4f\t%i\t%i\n" %\
                                  (x[0], x[1], cnts,
                                   ...
                                   float(100 * cnts * x[0] ) / len(assigned_genes_schema1),
                                   float(100 * cnts * x[1] ) / len(assigned_genes_schema2),
                                   float(100 * cnts * x[0] ) / len(all_genes_schema1),
                                   float(100 * cnts * x[1] ) / len(all_genes_schema2),
                                   x[0] * cnts, x[1] * cnts ) )
        ...
        ## 1:1, 0:0, 1:m, m:1, m:m 
            ...
            for t1, t2, g1, g2, w in orthologs:
                ...
                for x in g1.keys(): gene_status1.append( (x, dgenes) )
                for x in g2.keys(): gene_status2.append( (x, dgenes) )
                for x in t1: transcript_status1.append( (x, dtranscripts) )
                for x in t2: transcript_status2.append( (x, dtranscripts) )                
                
            for g, t in orphans.items():
                for x in g: gene_status1.append( (x, "0:0" ) )                                
                for x in t: transcript_status1.append( (x, "0:0" ) )                

            outfile.write("# ortholog status for transcripts/genes\n")
            outfile.write("schema\tlevel\tgene\tstatus\n")            
            
            WriteOrthologs( outfile, options.schema1, gene_status1, transcript_status1 )
            WriteOrthologs( outfile, options.schema2, gene_status2, transcript_status2 )            
        ...
        #########################################################################################
            ...
            for vals in (vals_all, 
                ...
                hists.append( scipy.stats.histogram2( vals, range(min_bin, max_bin) ))
