...
def ProcessResult( result, options, mali = None, prefix = None, p_value = None ):
    ...
    if options.method == "summary-slr":
        ...
        options.stdout.write("\t".join( map(lambda x: "%i" % result.mNPositiveSites[x][0], thresholds) ) )
        options.stdout.write("\t")
        options.stdout.write("\t".join( map(lambda x: "%i" % result.mNNegativeSites[x], thresholds) ) )            
        options.stdout.write("\n")
        ...
        if len(result.mSites) * 3 != mali_width:
            raise "mali (%i) and # of sites (%i) do not correspond." % (mali_width, len(result.mSites) )
        ...
        if options.method == "summary-filtered":
            ...
            counts = Result( nfiltered, ntotal, nsynonymous, nnegative, npositive )

        elif options.method in ( "positive-site-table", "negative-site-table", "neutral-site-table",
                                 "positive-site-list", "negative-site-list", "neutral-site-list",                                 
                                 ):

            select_positive_sites = options.method in ("positive-site-table", "positive-site-list")
            select_negative_sites = options.method in ("negative-site-table", "negative-site-list")            
...
if __name__ == "__main__":
    ...
    parser = E.OptionParser( version = "%prog version: $Id: optic/analyze_sites_slr.py 2781 2009-09-10 11:33:14Z andreas $" )
    ...
    ## write headers
    if "%s" in options.filename_sites: options.prefix = True
...
""")
            ...
            options.stdout.write("ltree\tomega\tkappa\tlnL\tnsites\tnsyn\tngap\t")
            options.stdout.write("\t".join( map( lambda x: "npos_" + x.replace( " ", "_"), thresholds)))
            options.stdout.write("\t")                
            options.stdout.write("\t".join( map( lambda x: "nneg_" + x.replace( " ", "_"), thresholds)))                
            options.stdout.write("\n")
...
""")
            ...
            if options.prefix: options.stdout.write("prefix\t")
            options.stdout.write("ltree\tomega\tkappa\tlnL\tnsites\tnfiltered\tntotal\tnsyn\tnneg\tnpos\n")
...
""" % options.significance_threshold)
            ...
            options.stdout.write("sequence\tn\taa\tseq_pos\tmali_pos\tcontext\n")
    ...
    if options.method in ("summary-slr", "summary-filtered",
        ...
        if "%s" in options.filename_sites:
            ...
            for row in table:
                ...
                if options.filename_log:
                    lines_log = open(re.sub("%s", id, options.filename_log),"r").readlines()
                ...
                if options.method in ("summary-filtered", "positive-site-table", "negative-site-table", "neutral-site-table" ):
                    mali = Mali.Mali()
                    mali.readFromFile( open(re.sub("%s", id, options.filename_mali), "r") )
                else:
                    mali = None
        ...
        if options.method == "over-representation":
            ...
            for line in sys.stdin:
                ...
                results.append( Result( data[0], int(data[6]), int(data[7]), int(data[8]), int(data[9]), int(data[10])) )
            ...
            for result in results:
                if result.mNTotal == 0: continue

                # use -1, because I need P( x >= X)
                # sf = 1 - cdf and cdf = P( x <= X ), thus sf = 1 - P( x <= X ) = P (x > X ).
                r = scipy.stats.binom.sf( result.mNPositive - 1, result.mNTotal, p )
            ...
            new_results.sort( lambda x,y: cmp(x.mSignificance,y.mSignificance) )
