...
def CheckBenchmark( p, kept_p = None ):
    ...
    if not options.benchmarks.has_key( key ): return
...
def EvaluateBenchmark( predictions, loglevel = 2 ):
    ...
    if not options.benchmarks: return
    
    ## build hash of all benchmarking regions
    ## hash is indexed by sbjct_token;sbjct_strand;sbjct_genome_from;sbjct_genome_to;query_token
    benchmark_regions = {}
    for key in options.benchmarks.keys():
        for sbjct_genome_from, sbjct_genome_to, query_token in options.benchmarks[key]:
            benchmark_regions["%s;%i;%i;%s" % (key, sbjct_genome_from, sbjct_genome_to, query_token)] = []
    ...
    for p in predictions:
        ...
        if not options.benchmarks.has_key( key ): continue
    ...
    for big_key in benchmark_regions.keys():
        ...
        if nfound > 1: total_nredundant += 1
        if nnonsyn > 0: total_nnonsyn += 1
...
def CombinePredictions( predictions ):
    ...
    covered = reduce( lambda x, y : x + y, map(lambda x: x[1] - x[0], new_intervalls))
...
def JoinRegions( old_predictions, new_predictions ):
    ...
    if isinstance( old_predictions, PredictionFile.PredictionFile):
        old_predictions.sort( ('mQueryToken', 'mSbjctToken', 'mSbjctStrand', 'mSbjctGenomeFrom') )
    else:
...
def ReadBenchmarkingRegions( infile ):
    ...
    for line in infile:
        ...
        (query_token, sbjct_token, sbjct_strand, sbjct_genome_from, sbjct_genome_to ) = line[:-1].split("\t")
        sbjct_genome_from, sbjct_genome_to = int(sbjct_genome_from), int(sbjct_genome_to)

        key = "%s;%s" % (sbjct_token, sbjct_strand)
        if not benchmarks.has_key( key ): benchmarks[key] = []
        benchmarks[key].append( (sbjct_genome_from, sbjct_genome_to, query_token) )
...
def PrintEdges( region_id, region, predictions ):
    ...
    for p in predictions:
        if p.mQueryToken in queries: continue
...
def ExchangeStreams( old_predictions, new_predictions ):
    ...
    os.system( "mv %s %s" % (new_predictions.GetFileName(), old_predictions.GetFileName()))
...
def main( argv = None ):
    ...
    if options.filename_benchmark:
        options.benchmarks = ReadBenchmarkingRegions( open(options.filename_benchmark, "r") )
        if options.loglevel >= 1:
            options.stdlog.write( "# read benchmarking regions for %i tokens\n" % len(options.benchmarks) )
            sys.stdout.flush()
    ...
    for line in sys.stdin:
        ...
        if entry.score < options.min_total_score:
            if options.loglevel >= 3:            
                options.stdlog.write( "# PRUNING: reason: score below minimum: removing: %s\n" % str(entry) )
            continue
        elif entry.mQueryCoverage < options.min_coverage_query:
            if options.loglevel >= 3:                        
                options.stdlog.write( "# PRUNING: reason: coverage below minimum: removing: %s\n" % str(entry))
            continue
        elif entry.mPercentIdentity < options.min_percent_identity:
            if options.loglevel >= 3:                        
                options.stdlog.write( "# PRUNING: reason: percent identity below minimum: removing: %s\n" % str(entry))
            continue
        elif entry.mSbjctTo - entry.mSbjctFrom < options.min_length:
            if options.loglevel >= 3:                        
                options.stdlog.write( "# PRUNING: reason: length of transcript below minimum: removing: %s\n" % str(entry) )
            continue
    ...
    if options.loglevel >= 10:
        ...
        options.stdlog.write( "############## start: predictions after input ###################################\n" )
        for x in old_predictions:
            options.stdlog.write( "# %s\n" % str(x) )
        options.stdlog.write( "############## end: predictions after input #####################################\n" )
        sys.stdout.flush()
    ...
    if options.join_regions and options.join_regions_max_coverage:
        ...
        if options.loglevel >= 10:
            options.stdlog.write( "############## start: predictions after joining ###################################\n")
            for x in old_predictions:
                options.stdlog.write( "# %s" % str(x) )
            options.stdlog.write( "############## end: predictions after joining #####################################\n" )
            sys.stdout.flush()
    else:
        if options.loglevel >= 1:
    ...
    if options.filter_regions:
        ...
        for filename in filenames:
            ...
            for line in infile:
                ...
                for exon in exons:
                    filter_regions[key].append( (exon.mGenomeFrom, exon.mGenomeTo) )
    ...
    for this_prediction in old_predictions:
        ...
        if this_prediction.mQueryToken in best_predictions:
            ...
            if float(this_prediction.mQueryCoverage) / best_prediction.mQueryCoverage < options.min_relative_coverage:
                if options.loglevel >= 2:
                    options.stdlog.write( "# PRUNING: reason: coverage below best: removing %s\n" % str(this_prediction) )
                continue

            if float(this_prediction.score) / best_prediction.score < options.min_relative_score:
                if options.loglevel >= 2:
                    options.stdlog.write( "# PRUNING: reason: score below best: removing %s\n" % str(this_prediction) )
                continue

            if float(this_prediction.mPercentIdentity) / best_prediction.mPercentIdentity < options.min_relative_percent_identity:
                if options.loglevel >= 2:
                    options.stdlog.write( "# PRUNING: reason: percent identity below best: removing %s\n" % str(this_prediction) )
                continue

            neliminated_suboptimal -= 1

        ## Filter 2: remove predictions overlapping with certain segments
        key = "%s-%s" % (this_prediction.mSbjctToken, this_prediction.mSbjctStrand)
        ...
        if key in filter_regions:
            ...
            if CheckOverlap( map( lambda x: (x.mGenomeFrom, x.mGenomeTo), exons), filter_regions[key] ):
                if options.loglevel >= 2:
                    options.stdlog.write( "# PRUNING: reason: overlapping with taboo region: removing %s\n" % str(this_prediction) )
    ...
    if last_prediction:
        ...
        region_id, nxoutput, nxfiltered = ProcessRegion( predictions, region_id, region, peptide_sequences, filter_queries )
        noutput += nxoutput
        nfiltered += nxfiltered
