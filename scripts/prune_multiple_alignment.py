        ...
        for y in range( x+1, ninc):
            ...
            if s2 == s1 and g1 == g2:
                ...
                if options.loglevel >= 4:
                    options.stdlog.write("# weighting: %s\t%s\t%5.2f\n" % (t1, t2, weight))
...
def SplitExons( mali, exons, options, separator = "|", gap_char = "-", masters = None):
    ...
    pairs = {}
    if masters: masters = set(masters)
    genes = map( lambda x: (x[0], x[2]), map( lambda x: x.split(separator), identifiers ))
    for t1 in range(len(genes)-1):
        if masters and identifiers[t1] not in masters: continue
        for t2 in range(t1+1, len(genes)):
            if genes[t1] == genes[t2]:
                if genes[t1] not in pairs: pairs[genes[t1]] = []
    ...
    if not inconsistent_columns: return

    if options.loglevel >= 5:        
        for column, mm, ic in inconsistent_columns:
            options.stdlog.write("# inconsistent column: %i\t%i: %s\n" % (column, mm, str(ic)))
    ...
    if options.loglevel >= 1:
        options.stdlog.write("# found %i inconsistent segments.\n" % len(inconsistent_segments) )
    ...
    for first_res, last_res, num_clusters, inconsistencies in inconsistent_segments:
        ...
        clusters = GetInconsistentClusters(mali, num_clusters, first_res, last_res, inconsistencies, options )
        
        ## assign remaining entries to clusters
        incs = set(map( lambda x: x[0], inconsistencies))
        clusters = Assign2Clusters( mali, set(identifiers).difference( incs ), clusters, first_res, last_res, options )
...
def GetFrameColumns( mali, master, gap_chars = "-." ):
    ...
    while x < len(sequence):
        ...
        while x < len(sequence) and c < 3:
            if  sequence[x] not in gap_chars: #  and sequence[x] in string.uppercase:
                codon.append( x )
                c += 1
                t += 1
            x += 1
    ...
    if t % 3 != 0:
        raise "master %s has length %i, which is not divisible by 3" % (master, t)
...
if __name__ == '__main__':
    ...
    if options.target == "paml":
        ...
        if options.loglevel >= 1:
            options.stdlog.write("# setting output to paml : removing frameshifts, masking stops with '%s'.\n" % ( options.mask_char ) )
    ...
    if exons:
        for id in identifiers:
            if id in exons:
                mali.getSequence(id).mString = AddExonInformation( mali[id], exons[id], mask_char = options.mask_char )
    ...
    if options.remove_frameshifts:
        ...
        for column in frame_columns:
            for c in column: frame_set.add( c )
        ...
        if options.loglevel >= 1:
            ...
            if options.loglevel >= 5:
                options.stdlog.write( "# Out-of frame columns with residue of masters: %i\n" % (len(out_of_frame_set) ))
                options.stdlog.write( "# %s" % ",".join(map(str, out_of_frame_columns) ) )
                
        mask_chars = ( string.upper( options.mask_char ), string.lower( options.mask_char ) )
        ...
        for id in identifiers:
            ...
            if options.loglevel >= 7:
                options.stdlog.write("# processing sequence %s of length %i with gaps\n" % (id, len(sequence) ))
...
##                                                                                                  str(codon_is_aligned) ))
            ...
            if naligned == 0:
                options.stdlog.write( "# sequence: %s removed because there are no aligned nucleotides.\n" % id)
                to_delete.append(id)
            elif ncodons == 0:
                options.stdlog.write( "# sequence: %s removed because there are no aligned codons.\n" % id)
                to_delete.append(id)
            else:
