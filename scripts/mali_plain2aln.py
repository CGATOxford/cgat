...
def main(argv=None):
    ...
    while "" in aliLines:
        aliLines.remove("")
    while len(aliLines) > 2:
        del aliLines[2]

    # Sort data and get alignment length
    qArray = aliLines[0].split("\t")
    queryStart, queryAli, queryEnd, queryId = int(
        qArray[0]), qArray[1], int(qArray[2]), qArray[3]
    queryAli = queryAli.replace("-", ".")
    sArray = aliLines[1].split("\t")
    sbjctStart, sbjctAli, sbjctEnd, sbjctId = int(
        sArray[0]), sArray[1], int(sArray[2]), sArray[3]
    ...
    while (CURR_POS < length):
        if length - CURR_POS < 60:
            LINE_WIDTH = length - CURR_POS
        qGaps, sGaps = 0, 0
