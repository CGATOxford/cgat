        ...
        for start, end in small_intervals:
            ...
            for e in exons:
                ...
                if e.mQueryToken in map_peptide2cds:
                    map_p2c = lambda x: MapP2CRight(
                        map_peptide2cds[e.mQueryToken], x)
                    map_c2p = lambda x: MapC2PRight(
                        map_peptide2cds[e.mQueryToken], x)
                else:
                    map_p2c = nomap
                    map_c2p = nomap
                ...
                if peptide_start % coordinate_factor != 0:
                    start_frame = coordinate_factor - \
                        peptide_start % coordinate_factor
                else:
                    start_frame = 0
        ...
        for x in range(len(small_intervals)):
            ...
            for key, exons_in_intervall_and_frame in frames[x].items():
                ...
                for e in exons_in_intervall_and_frame:
                    ...
                    # assert that boundaries work, but ignore errors in last
                    # exon or intervals that are smaller than a codon
                    if x < len(input[e.mQueryToken]) and end - start > 3:
                        assert x <= y, "sanity check x <= y failed: x=%i, y=%i, l=%i" % (
                            x, y, len(input[e.mQueryToken]))
                    ...
                    if x < y and x < len(input[e.mQueryToken]) and x >= 0:
                        ...
                        if cds_sequences and e.mQueryToken in cds_sequences:
                            ...
                            for peptide_pos in range(x, y):
                                ...
                                # do not add codons that are incomplete in the
                                # peptide sequence
                                if -1 in pos:
                                    s.append("NNN")
                                    continue
...
sequences will be aligned to the cds sequences. This produces better coordinates."""  )
            ...
            for key, ee in exons.items():
                ...
                if ee[-1].mPeptideTo != map_p2c.getColTo():
                    ...
                    if d == 3:
                        ...
                        # if the exon information extends to higher residues
    ...
    for key, val in unpacked.items():
        ...
        if len(val.mString) != width:
            raise ValueError("incompatible mali lengths in %s: %i should be %i." % (
                key, len(val.mString), width))
        ...
        if len(sold) != len(snew):
            if key in pseudogenes:
                ...
                E.warn(
                    "sequence %s changed in length - last residue lost" % (key))
                E.warn("%s:\nold=%s\nnew=%s" % (key, sold, snew))
                nwarnings_length += 1
            elif not options.force_map:
                raise ValueError("sequence changed in length %s:\nold=(%i)\n%s\nnew=(%i)\n%s" % (
                    key, len(sold), sold, len(snew), snew))
            ...
            # should be >=, but was too strict for one fly sequence, so I
            # relaxed it.
            if nchanged > len(exons[key]):
                raise ValueError(
                    "more codons fixed than intron-exon boundaries in sequence %s: %i>=%i" % (key, nchanged, len(exons[key])))
        ...
        if sold.upper() != snew.upper():
            ...
            if options.force_map:
                ...
                alignator = alignlib_lite.py_makeAlignatorDPFull(
                    alignlib_lite.py_ALIGNMENT_GLOBAL, -10.0, -1.0)
        ...
        if "final_na" in options.output:
            ...
            for key in unpacked.getIdentifiers():
                ...
                try:
                    ...
                    E.warn(
                        "final alignment not mappable for %s -skipped" % key)
                    nskipped += 1
                    continue
                ...
                alignlib_lite.py_combineAlignment(
                    map_pos2genome, map_pos2cds, map_c2g, alignlib_lite.py_CR)
                ...
                info.append(
                    (key, exons[key][0].mSbjctToken, exons[key][0].mSbjctStrand))
                ...
                if len(s) != len(p) * 3:
                    raise ValueError(
                        "incomplete aligned string for %s: %s, cds=%s" % (key, s, c))
            ...
            if "coords" in options.output:
                E.info("output genomic coordinates to %s" %
                       options.output_filename_coords)
