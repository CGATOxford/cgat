...
def getOrthologNodes(tree, positive_set, options, selector="strict", outgroups=None):
    ...
    if selector == "strict":
        ...
        check_outgroup_function = lambda genes: 0 < sum(
            [len(genes[x]) for x in outgroups]) < sum(map(lambda x: len(x), genes))
        negative_set = set()
    else:
...
def rerootTree(gene_tree, extract_species, options):
    ...
    try:
        outgroup_taxa = filter(
            lambda x: extract_species(x) in options.outgroups, otus)
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: trees2sets.py 2782 2009-09-10 11:40:29Z andreas $", usage=globals()["__doc__"])
    ...
    parser.add_option("-e", "--enumeration", dest="enumeration", type="choice",
                      choices=(
                          "monophyletic", "full", "pairwise", "exhaustive", "explicit", "lineage"),
    ...
    parser.add_option("-m", "--method", dest="methods", type="choice", action="append",
                      choices=(
                          "strict", "degenerate", "any", "outgroup", "lineage"),
    ...
    if options.loglevel >= 1:
        options.stdlog.write(
            "# reference tree after pruning has %i taxa.\n" % len(reference_tree.get_taxa()))
