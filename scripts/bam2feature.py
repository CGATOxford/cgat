        ...
        for read in samfile:
            ...
            # pass results to Aggregator class functions
                ...
                if current_tid != last_tid:
                    ...
                    elif (self.condition == "range" and clip is not None):
                        yield Aggregator(samfile=samfile,
                                         results=res).clipInterval(results=res,
                                                                   k=self.k,
                                                                   contig_size=contig_dict[last_contig],
                                                                   threshold=threshold,
                                                                   clip=clip)

                    elif (self.condition == "cat" and clip is not None):
                        yield Aggregator(samfile=samfile,
                                         results=res).clipInterval(results=res,
                                                                   k=contig_dict[last_contig],
                                                                   contig_size=contig_dict[last_contig],
                                                                   threshold=threshold,
                                                                   clip=clip)
...
class Aggregator(object):
    ...
    def clipInterval(self,
        ...
        '''
        Counts the number of clipped reads in a given interval
        and outputs those regions > threshold in the same
        fashion as tagInterval
        '''
        ...
        last_interval = 1
        print k
        # use the read mapping position to allocate
        # it to an interval.  Only count reads that
        # are clipped using the CIGAR string

        if clip == "soft":
            pattern = re.compile(r"([0-9]*)([S])")
        elif clip == "hard":
            pattern = re.compile(r"([0-9]*)([H])")
        elif clip == "both":
            pattern = re.compile(r"([0-9]*)([SH])")
        else:
            raise ValueError("No clipping option provided")

        for record in results:
            if (record.start <= interval):
                reads += 1
                last_interval = interval
                if re.search(pattern, record.cigarstring):
                    counter += 1
                else:
                    pass
            else:
            ...
            # only yield results if the proprotion
            # of reads in an interval exceed a threshold
            
            if (counter > 1 and interval > last_interval
...
def main(argv=None):
    ...
    parser.add_option("--clip", dest="clip", type="choice",
                      choices=("hard", "soft", "both"),
                      help="either hard, soft or both [default=%default]")
    ...
    elif (options.condition == "cat" and options.clip):
        options.stdout.write('track type=bedGraph name="%s'
                             'description=stdout" '
                             'visibility=full priority=20\n'
                             % (options.condition))
