...
def getRangesFromExons( exons, both_strands = False, contig_sizes = None ):
    ...
    for key, ee in exons.items():
        ...
        k = "%s:%s" % (contig, strand)
        if k not in filter_ranges: filter_ranges[k] = []
...
def FilterEliminateOverlappingTranscripts( exons, filter_exons, eliminated_predictions, contig_sizes, options ):
    ...
    for k, ee in exon_ranges.items():
        ...
        if k not in filter_ranges: continue
...
def EliminateSuboptimalPredictions( data, 
    ...
    for d in data:
        ...
        if rep_quality in options.quality_keep_suboptimal: continue
...
def EliminateExonSwoppers( data, 
    ...
    for d in data:
        ...
        if rep_quality in options.quality_keep_exon_swopper: continue
        if rep_id in eliminated_predictions: continue
...
def EliminateGeneSpanners( data, eliminated_predictions, exons, options ):
    ...
    def ProcessChunk( chunk, eliminated_predictions, exons ):
        ...
        for x in range(0, len(chunk)-1):
            ...
            if xquality in options.quality_keep_gene_spanners: continue
            for y in range(x+1, len(chunk)):
                yfrom, yto, yid, yquality = chunk[y]
                # print xid, yid, xfrom < yfrom, xto > yto, Exons.CheckOverlap(exons[xid], exons[yid] ), xquality, yquality 
                if xfrom < yfrom and \
                       xto > yto and \
...
def EliminateRedundantEntriesByRange( rep, data,
    ...
    entries = []
    for d in data:
        if d.transcript_id == rep_id: continue
        if d.transcript_id in eliminated_predictions: continue
        if d.contig != rep_contig: continue
        if d.strand != rep_strand: continue
        if filter_quality and d.mQuality in filter_quality: continue
...
def EliminateRedundantEntriesByOverlap( rep,
    ...
    if options.loglevel >= 3:
        options.stdlog.write( "# eliminating transcripts for %s out of %i\n" % (rep.transcript_id, len(data)) )
        options.stdlog.flush()

    entries = []
    for mem in data[1:]:

        if mem.gene_id != rep.gene_id: continue
        if mem.mQuality in filter_quality: continue
    ...
    if options.remove_spanning_predictions:
        ...
        for mem in data[1:]:
            ...
            if mem.contig != rep.contig: continue
            if min(mem.end, rep.end) - max(mem.start, rep.start) <= 0: continue
            if mem.mQuality in filter_quality: continue            
            if mem.mQuality not in options.quality_remove_dubious: continue
    ...
    if options.loglevel >= 3 and len(eliminated) > 0:
        options.stdlog.write( "# eliminated %i transcripts for %s\n" % (len(eliminated), rep_id) )
        options.stdlog.flush()
