...
Example::
   ...
   python gpipe/predictions2genes.py --help
...
Version: $Id: gpipe/predictions2genes.py 1799 2008-03-28 11:44:19Z andreas $
...
def ResolveExonOverlaps( gene_id, predictions ):
    ...
    if len(predictions) == 0: return gene_id
    
    for p in predictions:
        
        exons = Exons.Alignment2Exons( Genomics.String2Alignment(p.mAlignmentString),
                                       query_from = 0,
    ...
    for exon_from, exon_to, p in all_exons[1:]:
        ...
        if overlap and param_exon_identity:
            overlap = (exon_from == last_exon_from) and (exon_to == last_exon_to)
...
def main( argv = None ):
    ...
    for o,a in optlist:
        ...
        elif o in ( "-i", "--exon-identity" ):
            param_exon_identity = 1
    ...
    if param_tablename_predictions:
        ...
        if param_quality:
            statement = """SELECT p.* FROM %s
            WHERE 
            class IN ('%s')""" % (string.join(param_quality, "','"))
        else:
            statement = "SELECT p.* FROM %s AS p" % (param_tablename_predictions)
