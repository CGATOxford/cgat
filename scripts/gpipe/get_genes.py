...
def BuildLines( dbhandle, statement, genome_lengths, prefix = "", default_color = None ):
    ...
    for line in c.fetchall():
        ...
        if not genome_lengths.has_key(entry.mSbjctToken):
            ...
            genome_lengths[entry.mSbjctToken] = (len(forward_sequences[entry.mSbjctToken]), 0)
        ...
        if param_tablename_exons:
            ...
            if param_restrict_good_exons: statement += " AND is_ok = TRUE"
        ...
        if not exons:
            if entry.mMapPeptide2Genome:
                ...
                exons = [( "", "", 0, entry.mSbjctGenomeFrom, entry.mSbjctGenomeTo)]
        ...
        if param_format == "gff":
            ...
            if param_tablename_quality:
                ...
                if result:
                    ...
                    info_lines.append("is_best_prediction= %s" % str(result[1]))
                    info_lines.append("is_conserved= %s" % str(result[2]))
                    info_lines.append("is_partially_conserved= %s" % str(result[3]))
            ...
            if info_lines:
                ...
                url="http://fgu200.anat.ox.ac.uk:8080/ucsc_tracks/%s" % (name)
            ...
            if entry.mSbjctStrand == "+":
                ...
                for exon in exons:
                    lines.append( string.join( map(str, (entry.mSbjctToken,
                                                         "CPPipe",
                                                         "exon",
                                                         lgenome - exon[4] + offset, lgenome - exon[3] + offset,
                                                         ".",
                                                         entry.mSbjctStrand,
                                                         exon[2],
                                                         "%s%i_%s" % (prefix,entry.mPredictionId, entry.mQueryToken))),
                ...
                if entry.mSbjctStrand == "+":
                    ...
                    for exon in result:
                        lines.append( string.join( map(str, (entry.mSbjctToken,
                                                             "gpipe",
                                                             "exon",
                                                             lgenome - exon[2] + offset, lgenome - exon[1] + offset,
                                                             "100",
                                                             "-1",
                                                             ".",
                                                             exon[0],
...
if __name__ == '__main__':
    ...
    if param_filter_region:
        ...
        param_extra_conditions.append( ("g.gene_id = '%s' AND g.prediction_id = p.prediction_id") )
    ...
    if param_filter_query:
        param_extra_conditions.append( "query_token like '%%%s%%' %s" % (param_filter_query) )
        ids = [""]
    ...
    tables = string.join( map( lambda x: "%s AS %s" % x, param_extra_tables), ",")
    ...
    for id in ids:
        ...
        # build SQL statement
        statement = "SELECT p.* FROM %s WHERE %s ORDER BY export_sbjct_genome_from" % (tables, conditions)
        if "%s" in statement: statement = statement % id

        if param_loglevel >= 3: print "# SQL statement=%s" % statement
    ...
    if param_format == "ucsc":
        ...
        lines = [ "browser position %s:%i-%i" % (sbjct_token, sbjct_from, sbjct_to)] + lines

        name = "%s_%s-%i-%i" % (param_tablename_predictions, sbjct_token, sbjct_from, sbjct_to)
        filename = "%s/%s" % (param_ucsc_dir, name)
