...
class Predictor(E.Experiment):
    ...
    def __call__( self,
        ...
        try:
            result = self.mParser.Parse( lines, peptide_sequence, genomic_sequence )
        ...
        if bracket_from: result.ShiftGenomicRegion( bracket_from )
        if peptide_from: result.ShiftQueryRegion( peptide_from )
...
class TranscriptPredictor(E.Experiment):
    ...
    def RunIncremental( self, bracket_from, bracket_to, bracket_from_end, bracket_to_end ):
        ...
        while 1:
            ...
            if self.mLogLevel >= 1:
                print "# INCREMENTAL: key=%s, iteration=%i, sensivity=%i, hits=%i, score=%5.2f, coverage=%5.2f, time=%i" % \
                      (self.mKey, niterations, self.mSensitivityLevel,
                       result.GetNumMatches(), best_entry.score, best_entry.mQueryCoverage,
                       time.time() - t1)
            ...
            if last_mode != "brackets" and \
                ...
                # stop extension, if peptide is (almost) complete at either terminus
    ...
    def RunRecursive( self,
        ...
        if self.mDoRefinement:
            ...
            if not result:
                ...
                result.Add( e )
            result = self.RunRefinement( result, bracket_from_end, bracket_to_end)
        ...
        if result and result.GetNumMatches() > 0 and self.mDoRecursive:
            ...
            if match_from > bracket_from:
                new_result = self.RunRecursive( bracket_from, match_from, bracket_from_end, match_from, level + 1 )
                if new_result:
                    result.Combine(new_result)

            # run right prediction
            if match_to < bracket_to:
                new_result = self.RunRecursive( match_to, bracket_to, match_to, bracket_to_end, level + 1)
                if new_result:
                    result.Combine(new_result)
    ...
    def RunInitialize( self ):
        ...
        self.mForwardSequences, self.mReverseSequences = Genomics.ReadGenomicSequences( open(self.mFilenameGenome, "r"))

        # read peptide sequence from peptide sequences
        self.mPeptideSequences = Genomics.ReadPeptideSequences( open(self.mFilenamePeptides, "r") )
    ...
    def RunRefinement( self, result, bracket_from_end, bracket_to_end ):
        ...
        for p in result:
            ...
            for xp in tmp_result:
                ...
                overlap = min(xp.mQueryTo, p.mQueryTo) - max(xp.mQueryFrom, p.mQueryFrom)
                goverlap = min(xp.mSbjctGenomeTo, p.mSbjctGenomeTo) - max(xp.mSbjctGenomeFrom, p.mSbjctGenomeFrom)
        ...
        new_prediction = self.RefinePrediction( new_p, bracket_from_end, bracket_to_end )
        if new_prediction: new_result.Add(new_prediction)
    ...
    def RunProbeStep( self, level,
        ...
        if result and result.GetNumMatches() > 0:
            ...
            best_entry = result.GetBestMatch()
            self.mSensitivityLevelStart = max(self.mSensitivityLevelStart, self.mSensitivityLevel)
            ...
            increment = self.mBracketIncrements[min(level, len(self.mBracketIncrements)-1)]

            # run left prediction, if no internal match

            self.mMaxPeptideTo = max(self.mMaxPeptideTo, match_peptide_to)
            self.mMinPeptideFrom = min(self.mMinPeptideFrom, match_peptide_from)
            
            if match_peptide_from <= self.mMinPeptideFrom:
                new_result = self.RunProbeStep( level + 1,
                                                bracket_from - increment , match_genome_from,
            ...
            if match_peptide_to >= self.mMaxPeptideTo:                
                new_result = self.RunProbeStep( level + 1, 
                                                match_genome_to, bracket_to + increment, match_genome_to, bracket_to_end,
    ...
    def RunProbe( self, bracket_from, bracket_to, bracket_from_end, bracket_to_end ):
        ...
        if result:
            ...
            coverage = 100 * (self.mMaxPeptideTo - self.mMinPeptideFrom ) / self.mQueryLength
        else:
            coverage = 0 
...
class TranscriptPredictorTwoStep( TranscriptPredictor ):
    ...
    def RefinePrediction( self, prediction, bracket_from_end, bracket_to_end ):
        ...
        prediction_length = prediction.mSbjctGenomeTo - prediction.mSbjctGenomeFrom
        ...
        result = self.mRefinementPredictor(self.mQueryToken, self.mPeptideSequences[self.mQueryToken],
                                           self.mSbjctToken, self.mForwardSequences[self.mSbjctToken],
...
class TranscriptPredictorTwoStepEG( TranscriptPredictorTwoStep ):
    ...
    def __init__(self, filename_genome, filename_peptides ):
        TranscriptPredictorTwoStep.__init__( self, filename_genome, filename_peptides )
...
class TranscriptPredictorTwoStepEE( TranscriptPredictorTwoStep ):
    ...
    def __init__(self, filename_genome, filename_peptides,
        ...
        self.mRefinementOptions = "--exhaustive --subopt FALSE --score '%s' " % str(min_score)
...
def EvaluatePrediction( prediction, query_exons, query_sequence ):
    ...
    for e in exons: e.mQueryToken = "prediction"
    ...
    comparison = Exons.CompareGeneStructures(
        ...
        threshold_min_pide = prediction.mPercentIdentity * global_options.quality_threshold_pide / 100,
    ...
    ## two exons in result: 
        ...
        ## accept if both exons are identical
            if comparison.mNumIdenticalExons == 2:
                ...
                is_ok = True
    ## single exon in result: accept, if query is single exon and coverage above threshold
    else:
...
def main( argv = None ):
    ...
    parser.add_option("-e", "--exit-identical", dest="exit_if_identical", action="store_true" )
    ...
    parser.add_option("--border-refinement", dest="border_refinement", type="int")
    ...
    if global_options.bracket_increments:
        global_options.bracket_increments = map(int, global_options.bracket_increments.split( "," ))
            
    if global_options.method == "twostep_eg":
        predictor = TranscriptPredictorTwoStepEG( global_options.filename_peptides, global_options.filename_genome )
    elif global_options.method == "twostep_ee":
        predictor = TranscriptPredictorTwoStepEE( global_options.filename_peptides, global_options.filename_genome, global_options.min_score )
    else:
    ...
    if global_options.input_format == "pairs":
        ...
        peptide_sequences = Genomics.ReadPeptideSequences( open(global_options.filename_peptides, "r") )
        forward_sequences = Genomics.ReadGenomicSequences( open(global_options.filename_genome, "r"), do_reverse = 0 )  
    ...
    ## the remaining entries are skipped and the next list is processed.
        ...
        ## get genomes in input set (so that for single genome files, unnecessary entries can be skipped).
        ...
        if len(args) == 1 and args[0] == "-":
            ...
            for d in data:
                ...
                (sbjct_from, sbjct_to, min_bracket_from, min_bracket_to, region_id, region_nr, region_max_nr) = \
                             map(int, (sbjct_from, sbjct_to, min_bracket_from, min_bracket_to, region_id, region_nr, region_max_nr))

                if sbjct_sequence == "":
                    if last_sbjct_sequence == None:
                        try:
                            sbjct_sequence = fasta.getSequence( sbjct_token, sbjct_strand, sbjct_from, sbjct_to )
                ...
                ## be 1 due to duplicated key removal in gpipe/assignments2pairs.py
                ...
                if result and query_token in exons:
                    ...
                    for r in result:
                        skip = skip or EvaluatePrediction( r, exons[query_token], query_sequence )
    ...
    ## process default format
        ...
        ##--------------------------------------------------------------------
        ## two arguments: a single prediction with one peptide file and one genome file.
        if len(args) == 2:
            global_options.filename_peptides, global_options.filename_genome = args
            ...
            if global_options.use_header:
                ...
                if len(data) == 1:
                    ...
                    map(int, (sbjct_from, sbjct_to, min_bracket_from, min_bracket_to))
