...
Example::
   ...
   python gpipe/gbrowser_clone_devel.py --help
...
data_monodelphis = (("Monodelphis", "mono_p1"), )

data_platypus = (("Platypus", "plat1"), )
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: gpipe/gbrowser_clone_devel.py 2781 2009-09-10 11:33:14Z andreas $", usage=globals()["__doc__"])

    parser.add_option("-t", "--target-dir", dest="target", type="string",
                      help="target directory.", metavar="FILE")

    parser.add_option("-s", "--set", dest="set", type="choice",
                      choices=("flies", "platypus", "monodelphis", "worms"),
                      help="database set to clone [%default]")
    ...
    if options.set == "flies":
        data = data_flies
    elif options.set == "platypus":
        data = data_platypus
    elif options.set == "monodelphis":
        data = data_monodelphis
    elif options.set == "worms":
        data = data_worms
    ...
    for name, schema in data:
        ...
        cc = dbhandle.cursor()
        for table in tables:
            cc.execute("ALTER TABLE %s.%s RENAME AS %s.%s" %
                       (old_name, table, new_name, table))
        ...
        for table in tables:
            ...
            cc.execute("INSERT INTO %s.%s SELECT * FROM %s.%s" %
                       (old_name, table, new_name, table))
