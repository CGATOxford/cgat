...
parser = E.OptionParser( version = "%prog version: $Id: gpipe/genes2quality.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    if options.method == "geneinfo":
        ...
        if best:
            ...
            for quality, id, index in qualities:
                if quality == best: s.append( (index,id) )
...
def main( argv = None ):
    ...
    if do_filter_ds and options.loglevel >= 1:
        options.stdlog.write( "# filtering values by ds <= %f" % options.filter_ds )

    for this_gene, this_id, this_quality, this_score in rows[1:]:

        if this_gene == "0": continue
