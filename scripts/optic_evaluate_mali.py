...
def WriteGeneStructureCorrespondence( mali, identifiers, exons, param_master_pattern, gap_char = "-" , prefix = "" ):
    ...
    for x in range(len(identifiers)):
        ...
        for y in range(len(identifiers)):
            ...
            if param_loglevel >= 5:
                print str(alignlib.AlignmentFormatEmissions( map_cmp2ref ))
        ...
        if best != None:
            if param_loglevel >= 2:
                print "# assigning %s to %s: pid=%5.2f, cov=%5.2f, cmp=%5.2f" % (id, identifiers[best_m],
                                                                                 matrix_identity[x][best_m],
                                                                                 matrix_coverage[x][best_m],
...
def WriteBootstrap( mali,
    ...
    pruned_reference_tree = evaluate_bootstrap.GetPrunedReferenceTree( mask, present_orgs, param_reference_tree )
    
    evaluate_bootstrap.AnalyseMask( mask, patterns, norgs, pruned_reference_tree, map_id2org, param_min_report_support )
...
if __name__ == '__main__':
    ...
    if param_filename_reference_tree:
        param_reference_tree = "".join( map(lambda x: x[:-1], open(param_filename_reference_tree, "r").readlines()) )

    ## read and process reference tree
    if param_reference_tree:
        rx_species = re.compile(param_pattern_species)
        
        reference_tree, map_taxon2id = evaluate_bootstrap.ParseTree( param_reference_tree, rx_species )
        if param_loglevel >1:
        ...
        if os.path.exists( param_filename_bootstrap ):
            ...
            while 1:
                ...
                if re.match("Species in order:", line):
                    ...
                    map_id2org, map_otu2id = evaluate_bootstrap.ReadMapId2Org( infile, rx_species, map_taxon2id )
                    continue
    ...
    for key, identifiers in components.items():
        ...
        for k, v in clusters.items():
            ...
            if len(patterns):
                WriteBootstrap( ungapped_mali, patterns, map_id2org, map_otu2id, reference_tree, prefix=prefix )
