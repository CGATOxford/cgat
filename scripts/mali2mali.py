...
def removeUnalignedPairs(mali, options):
    ...
    delete_rows = [ids[x]
                   for x in set(range(len(ids))).difference(set(take_rows))]

    for id in delete_rows:
        mali.deleteEntry(id)

    E.info("ninput=%i, nremoved=%i, noutput=%i" %
    ...
    if annotation_type.upper() in ("SLR-CODON", "SLR-AA"):
        ...
        if options.loglevel >= 2:
            options.stdlog.write(
                "# Warning: only positive sites of > %f significance for Bayes Empirical Bayes\n" % threshold)
        ...
        for x in ("1", "0", "3", "2", "7", "8"):
            ...
            found = False
            for site in sites:
                if site.mProbability < threshold:
                    continue
                found = True
