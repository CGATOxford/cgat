...
def main(argv=None):
    ...
    if options.filter_by_trees:
        ...
        for filename in options.filter_by_trees:
            ...
            if options.loglevel >= 1:
                options.stdlog.write(
                    "# read %i trees for filtering from %s\n" % (len(trees), filename))
    ...
    for tree in nexus.trees:
        ...
        if options.filter_simple_orthologs:
            ntaxa = len(tree.get_terminals())
            nspecies = len(
                set(map(lambda x: extract_species(tree.node(x).data.taxon), tree.get_terminals())))
            if nspecies != ntaxa:
                if options.loglevel >= 2:
        ...
        if options.filter_by_monophyly:
            ...
            terminals = set(
                map(lambda x: tree.node(x).data.taxon, tree.get_terminals()))

            for t in monophyly_taxa:
                if t not in terminals:
                    if options.loglevel >= 2:
                        options.stdlog.write(
                            "taxon %s not in tree %s\n" % (t, tree.name))
                    nskipped += 1
        ...
        if options.method == "split":
            ...
            if not os.path.exists(output_filename):
                ...
                if options.loglevel >= 1:
                    options.stdlog.write(
                        "# skipping because output for tree %s already exists: %s\n" % (id, output_filename))
                nskipped += 1
                continue
