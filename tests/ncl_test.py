...
import unittest
import random, tempfile, shutil, os
from ncl import *
...
class TestNCLSimple(unittest.TestCase):
    ...
    def buildIndex( self, l ):
        index = self.mIndex()
        for start, end, value in self.l: index.add( start, end )
        return index
    ...
    def testPointIntervals( self ):
        ...
        for x in xrange(0,len(bits)):
            ...
            self.assertEqual( r > 0, bits[x], "invalid return at position %i (expected %i, got %i)" % (x, bits[x], r) )
...
class TestNCL(TestNCLSimple):
    ...
    def buildIndex( self, l ):
        index = self.mIndex()
        for start, end, value in self.l: index.add( start, end, value )
        return index
