...
#
#   $Id: analyze_genetrees.py 2781 2009-09-10 11:33:14Z andreas $
#
...
def printCounts( heights_per_species, relheights_per_species,
    ...
    printHeightsPerSpecies( heights_per_species, "species_heights", options, prefix_header, prefix_row )
    printHeightsPerSpecies( relheights_per_species, "species_relheights", options, prefix_header, prefix_row )    

    printHeightsPerTree( heights_per_tree, "tree_heights", options, prefix_header, prefix_row )
    printHeightsPerTree( relheights_per_tree, "tree_relheights", options, prefix_header, prefix_row )
...
if __name__ == "__main__":
    ...
    for gene_tree in gene_nexus.trees:
        ...
        if len(otus) <= 1:
            nfiltered += 1
            if options.loglevel >= 1:
                options.stdlog.write("# tree %s: empty after filtering - skipped.\n" % gene_tree.name )
            continue
        ...
        if options.outgroup_species:
            ...
            if options.skip_without_outgroups and not set(this_species_list).intersection( options.outgroup_species):
                ...
                if options.loglevel >= 1:
                    options.stdlog.write("# tree %s: no outgroups - skipped.\n" % gene_tree.name )
                continue
        ...
        analysis_set, gene_set, pseudogene_set, other_set = TreeReconciliation.getAnalysisSets( gene_tree, extract_quality, options )
        ...
        ## update total counts
        #########################################################################
        TreeReconciliation.appendCounts( total_heights_per_species, heights_per_species )
        TreeReconciliation.appendCounts( total_relheights_per_species, relheights_per_species )        

        TreeReconciliation.appendCounts( total_heights_per_tree, heights_per_tree )
        TreeReconciliation.appendCounts( total_relheights_per_tree, relheights_per_tree )        
