'''
PipelineTransfacMatch.py - Classes and functions used in pipeline_trasfacmatch.py
=================================================================================
'''
        ...
        for line in inf.readlines():
            if line.find("Inspecting sequence ID") != -1:
                ...
                if line.strip() == "":
                    ...
                    continue
                self.matrix_id, position_strand, self.core_score, self.matrix_score, self.sequence = [
...
def frequencyMetrics(database, match_table, outfile):
    ...
    for seq_id, tfs in tf_max.iteritems():
        ...
        # output the results
        outf.write(
            "\t".join(map(str, [seq_id, len(tf_number[seq_id]), max(result.values())])) + "\n")
    outf.close()
...
def testSignificanceOfMatrices(background, foreground, database, match_table, outfile):
    '''
    ...
    outf = open(outfile, "w")
    outf.write(
        "matrix_id\toddsRatio\t95CI_low\t95_CI_hi\tpvalue\tnforeground\tnbackground\ttforeground\ttbackground\tqvalue\n")
    ...
    for tf in all_tfs:
        ...
        results.append(
            [tf, OR, ci_low, ci_hi, pvalue, nforeground, nbackground, tforeground, tbackground])
...
def matchBackgroundForCpGComposition(gc_load_files, background, foreground, database, outfile):
    '''
    ...
    # NB this is specific to this pipeline
    fasta_file = open(glob.glob("fasta.dir/*.fasta")[0])
