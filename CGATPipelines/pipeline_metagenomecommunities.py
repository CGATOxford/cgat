...
fast. Metaphlan is likely to perform better where the samples are derived from
human (e.g gut) as the database is significantly overrepresented for]
human derived taxa.
...
A third approach is to use a sensitive alignment algorithm. At the time
of writing this, a new approach to perform sensitive blastx-like alignments
was developed by the Huson lab. It is called Diamond and is 16000 times
faster than blast - a requirement for large datasets. The pipeline utilises
diamond in cases where it is expected that there is high divergence between
sequences derived from the sample and the NCBI nr database. Following alignment
with diamond, the pipeline will attempt to assign each read to a taxa
using the lcammaper tool (lowest common ancestor (LCA)) also developed by the Huson lab. 
...
To detect differences in abundance of taxa or genes, we utilise the metagenomeSeq R package
from bioconductor. This package utilises a zero-inflated gaussian micture model to 
compensate for undersampling of taxa/genes bewteen samples - which may 
cause overestimation of differences due to differences in library size.
...
+--------------------+-------------------+------------------------------------------------+
|diamond             |                   |Sensitive alignment algorithm                   |
+--------------------+-------------------+------------------------------------------------+
|lcamapper           |                   |Community profiling + fuctional profiling       |
+--------------------+-------------------+------------------------------------------------+
|metaphlan           |                   |taxanomic relative abundance estimator          |
+--------------------+-------------------+------------------------------------------------+
|kraken              |                   |megablast taxanomic assignment of reads         |
+--------------------+-------------------+------------------------------------------------+
|metagenomeSeq       |                   |differential abundance estimation tool          |
+--------------------+-------------------+------------------------------------------------+
...
def preprocessReads(infile, outfile):
    ...
    if infile.endswith(".fastq.gz"):
        ...
        statement = '''fastq-to-fasta.py %(infile)s 2> %(outfile)s.log | gzip > %(outfile)s'''
        P.run()
...
def mapReadsWithMetaphlan(infile, outfile):
    ...
    statement = '''zcat %(infile)s | metaphlan.py %(infile)s 
                                     ...
                                     --bt2_ps %(options)s
                                     --no_map 
                                     --bowtie2out %(outfile)s &> %(outfile)s.log'''
    P.run()
...
@transform(mapReadsWithMetaphlan, regex("(\S+)/(\S+).bt2out.txt"), r"metaphlan.dir/\2.readmap")
def buildMetaphlanReadmap(infile, outfile):
    ...
    blastn searching
    '''
    statement = '''metaphlan.py -t reads_map --input_type bowtie2out %(infile)s
...
@transform(mapReadsWithMetaphlan, regex("(\S+)/(\S+).bt2out.txt"), r"metaphlan.dir/\2.relab")
...
@transform(classifyReadsWithKraken, suffix(".classified.tsv.gz"), ".counts.tsv.gz")
...
def buildKrakenLevelCounts(infiles, outfiles):
    ...
    for infile in infiles:
        ...
        for level in levels:
            outname = "counts.dir/" + track + ".%s.kraken.counts.tsv.gz" % level
            outf = IOTools.openFile(outname, "w")
            outf.write("taxa\tcount\n")
            for data in cc.execute("""SELECT taxon, count FROM %s 
                                      WHERE taxon_level == '%s'""" % (tablename, level)).fetchall():
...
def mergeKrakenCountsAcrossSamples(infiles, outfiles):
    ...
    levels = [
        ...
        "species"]
    for level in levels:
        prefixes = glob.glob("counts.dir/*.%(level)s.kraken.counts.tsv.gz" % locals())
        prefixes = ",".join([P.snip(os.path.basename(x), ".%(level)s.kraken.counts.tsv.gz" % locals()) for x in prefixes])

        outname = os.path.join("counts.dir", level + ".kraken.aggregated.counts.tsv.gz")
...
@transform(buildLcaCounts, suffix(".diamond.counts.tsv.gz"), ".counts.diamond.load")
...
def buildLcaLevelCounts(infiles, outfiles):
    ...
    for infile in infiles:
        ...
        for level in levels:
            outname = "counts.dir/" + track + ".%s.diamond.counts.tsv.gz" % level
            outf = IOTools.openFile(outname, "w")
            outf.write("taxa\tcount\n")
            for data in cc.execute("""SELECT taxa, count FROM %s WHERE level == '%s'""" % (tablename, level)).fetchall():
                outf.write("\t".join(map(str, data)) + "\n")
            outf.close()
...
def mergeLcaCountsAcrossSamples(infiles, outfiles):
    ...
    levels = [
        ...
        "species"]
    for level in levels:
        prefixes = glob.glob("counts.dir/*.%(level)s.diamond.counts.tsv.gz" % locals())
        prefixes = ",".join([P.snip(os.path.basename(x), ".%(level)s.diamond.counts.tsv.gz" % locals()) for x in prefixes])

        outname = os.path.join("counts.dir", level + ".diamond.aggregated.counts.tsv.gz")
...
@transform(COUNT_DATA, regex("(\S+)/(\S+).counts.tsv.gz"), r"diversity.dir/\2.rarefaction.pdf")
def runRarefactionAnalysis(infile, outfile):
    ...
    '''
    f, to, step = PARAMS.get("rarefaction_from"), PARAMS.get("rarefaction_to"), PARAMS.get("rarefaction_step")
...
@transform(COUNT_DATA, regex("(\S+)/(\S+).counts.tsv.gz"), r"diversity.dir/\2.richness.sig")
...
@jobs_limit(1, "R")
@follows(mkdir("diversity.dir"))
@transform(COUNT_DATA, regex("(\S+)/(\S+).counts.tsv.gz"), r"diversity.dir/\2.diversity.pdf")
...
@jobs_limit(1, "R")
@follows(mkdir("diversity.dir"))
@transform(COUNT_DATA, regex("(\S+)/(\S+).counts.tsv.gz"), r"diversity.dir/\2.diversity.sig")
...
def mergeDiamondGeneCounts(infiles, outfile):
    ...
    prefixes = [
        P.snip(os.path.basename(x), ".genes.counts.tsv.gz") for x in glob.glob("genes.dir/*.genes.counts.tsv.gz")]
...
def countAlignments(infiles, outfile):
    ...
    track = os.path.basename(re.match("(.*-R[0-9]).(.*.counts.load)", infile).groups()[0])
    table = P.toTable(infile)
    ...
    nreads = cc.execute("""SELECT total_reads 
                           ...
                           WHERE track == '%(track)s'""" % locals()).fetchone()[0]

    outf = open(outfile, "w")
    outf.write("total_reads\taligned_reads\tpaligned_reads\n")
    outf.write("\t".join(map(str, [nreads, alignments, (float(alignments)/nreads)*100])) + "\n")
    outf.close()
...
@transform(PARAMS.get("genes_annotation"), regex("(\S+)/(\S+).txt"), r"annotations.dir/\2.load")
...
def buildForegroundGeneset(infile, outfile):
    ...
    result = {}
    groups = set()
    for group in  cc.execute("""SELECT group1, group2 FROM %(table)s""" % locals()).fetchall():
        groups.add(group)
    ...
    for group in groups:
        ...
        for data in cc.execute("""SELECT taxa, %(p_type)s, logFC FROM %(table)s
                                  WHERE group1 == '%(group1)s' and group2 == '%(group2)s'""" % locals()).fetchall():
...
def buildBackgroundGeneset(infile, outfile):
    ...
    outf = open(outfile, "w")
    outf.write("gene_id\n")
    for data in cc.execute("""SELECT DISTINCT taxa FROM %(table)s""" % locals()).fetchall():
        outf.write(data[0] + "\n")
    outf.close()
...
###################################################################
###################################################################
#[mergeKrakenCountsAcrossSamples,
