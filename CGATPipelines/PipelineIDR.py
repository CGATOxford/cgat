...
class macs2IDRPeaks( callerIDRPeaks ):
    ...
    def getRunStatement( self, infile, outfile, controlfile ):
        ...
        if not re.search( "-g\s|--gsize", 
            ...
            statement.append( "--gsize %s" % self.PARAMS_PEAKCALLER[ "macs2_options_genome_prefix" ][:2] )
        ...
        if self.PARAMS_PEAKCALLER[ "macs2_options_fdr" ]:
            if self.PARAMS_PEAKCALLER[ "macs2_options_pvalue" ]:
                ...
                threshold = "--qvalue " + str( self.PARAMS_PEAKCALLER[ "macs2_options_fdr" ] )
        elif self.PARAMS_PEAKCALLER[ "macs2_options_pvalue" ]:
            threshold = "--pvalue=" + str( self.PARAMS_PEAKCALLER[ "macs2_options_pvalue" ] )
        else:
    ...
    def postProcess( self, infile, outfile, controlfile ):
        ...
        statement = [ ( "sort -k %(sort_column)s %(outfile_stub)s_peaks.narrowPeak"
                      " | head -n %(npeaks)s"
                      " | gzip"
    ...
    def getRunStatement( self, infile, outfile, controlfile ):
        ...
        if self.PARAMS_PEAKCALLER[ "spp_options_npeaks" ]:
            if self.PARAMS_PEAKCALLER[ "spp_options_fdr" ]:
                ...
                threshold = "-npeaks=" + str( self.PARAMS_PEAKCALLER[ "spp_options_npeaks" ] )
        elif self.PARAMS_PEAKCALLER[ "spp_options_fdr" ]:
                threshold = "-fdr=" + str( self.PARAMS_PEAKCALLER[ "spp_options_fdr" ] )
        else:
            raise Exception( "Must specify a value for either"
...
def findNPeaks( infiles, outfile, params ):
    ...
    outf.write( "tissue\texperiment\tidr_comp\tsample_1\tsample_2\tn_peaks\n" )
    ...
    for inf in infiles:
        ...
        experiment = "_".join( [ inf_name.split("-")[0], inf_name.split("-")[1] ] )
