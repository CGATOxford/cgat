...
This pipeline is a wrapper for the Irreproducibility Discovery Pipeline outlined
by Anshul Kundaje. (see https://sites.google.com/site/anshulkundaje/projects/idr)
...
vvcarried out against the same control (i.e. pseudoreplication of the control file
does not take place), and peak calling for the pooled pseudoreplicates is carried
out against a pooled control. 
...
and iii) paired pooled pseudoreplicates for each EXPERIMENT. These steps provide
IDR analysis on origanl replicates, IDR analysis on self-pseudoreplicates, and 
IDR analysis on pooled-pseudoreplicates, respectively. 

Second stage
------------

4) Following IDR analysis, a second stage of the pipeline (generatePeakSets) may
be run. Libraries that fail in terms of 'max_numPeaks_Rep' or 'self-consistency' 
may be specified for exclusion in the config file. Subsequently, all non-excluded
bamfiles are pooled per EXPERIMENT so that both a conservative peak set and 
...
he sphinxreport report requires a :file:`conf.py` and :file:`sphinxreport.ini` file 
...
Example data is available at http://www.cgat.org/~andreas/sample_data/pipeline_IDR.tgz.
...
def get_peak_caller_parameters( peak_caller_id ):
    ...
    (an attempt to keep access to PARAMS out of the associated pipeline script).
    """
    caller_parameters = {}
...
def splitBamfiles( infile, sentinal ):
    ...
    P.submit( "/ifs/devel/jethro/PipelineIDR",
              "splitBam", 
              params, 
              infile,
              outfile )
...
def callPeaksOnIndividualReplicates( infile, outfile ):
    ...
    PARAMS_PEAKCALLER = get_peak_caller_parameters( PARAMS[ "options_peak_caller" ] )
...
@merge( "./peakfiles_individual_replicates/*.narrowPeak.gz", 
        "./peakfiles_individual_replicates/npeak_summary_individual_replicates.tsv" )
def summarizePeaksForIndividualReplicates( infiles, outfile ):
    outf = IOTools.openFile( outfile, "w" )
    outf.write( "Sample_id\tTissue\tCondition\tReplicate\tn_peaks\n" )
...
@merge( "./peakfiles_pseudoreplicates/*.narrowPeak.gz", 
        "./peakfiles_pseudoreplicates/npeak_summary_pseudoreplicates.tsv" )
def summarizePeaksForPseudoreplicates( infiles, outfile ):
    outf = IOTools.openFile( outfile, "w" )
    outf.write( "Sample_id\tTissue\tCondition\tPseudoreplicate\tn_peaks\n" )
    IDR.countPeaks( infiles, outf )
...
@merge( "./peakfiles_pooled_pseudoreplicates/*.narrowPeak.gz", 
        "./peakfiles_pooled_pseudoreplicates/npeak_summary_pooled_pseudoreplicates.tsv" )
...
def loadPeakCallingSummary(): pass
...
@collate( "./peakfiles_pseudoreplicates/*.regionPeak.gz", 
          regex( "./peakfiles_pseudoreplicates/(.+)(_00|_01)_VS_(.+).regionPeak.gz" ),
...
@collate( "./peakfiles_pooled_pseudoreplicates/*.regionPeak.gz", 
          regex( r"./peakfiles_pooled_pseudoreplicates/(.+)(_00|_01)_VS_(.+).regionPeak.gz" ),
...
def generatePeakSets( infile, outfiles ):
    ...
    if PARAMS[ "idr_options_ranking_measure" ] == "signal.value":
        ...
        raise ValueError( "Unrecognised ranking_measure %s don't know which column"
