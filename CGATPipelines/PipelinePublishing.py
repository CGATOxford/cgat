...
def publish_tracks( export_files, prefix = "", 
    ...
    *prefix* will be added to each track.
    ...
    if project_id == None: project_id = P.getProjectId()
    if project_name == None: project_name = P.getProjectName()
    ...
    if not os.path.exists( trackdir ): os.mkdir( trackdir )
    ...
    E.info( "data hub has been created at http://www.cgat.org/downloads/%(project_id)s/ucsc/hub.txt" % locals())
