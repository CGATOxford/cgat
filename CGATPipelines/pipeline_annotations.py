...
# only_proteincoding = PARAMS["ensembl_only_proteincoding"] )
    ...
    PipelineDatabase.importFromIterator(outfile, tablename, data, columns=columns, indices=("gene_id", "transcript_id", "protein_id", "gene_name", "transcript_name", "uniprot_id")
                                        )
...
if 0:
    ...
    def importUCSCEncodeTracks(infile, outfile):
        ...
        cc = dbhandle.cursor()
        tables = set(
            [x[0] for x in cc.execute("SELECT name FROM sqlite_master WHERE type='table'")])
        cc.close()
    ...
    def exportUCSCEncodeTracks(infile, outfile):
        ...
        for tablename in getUCSCTracks():
            ...
            cc = dbhandle.cursor()
            statement = "SELECT chrom, chrostart, chroend FROM %s ORDER by chrom, chrostart" % (
                tablename)
            cc.execute(statement)
...
############################################################
############################################################
# BED feature files for genes, TSS intervals, 5'/3' flanks and intergenic
# regions
    ...
    def buildGWASTracks(infile, outfile):
        ...
        for disease, pp in tracks.iteritems():
            ...
            for contig, positions in pp.iteritems():
                ...
                regions = [(max(0, x - extension), min(contigsize, x + extension))
                           for x in positions]
