...
def transcriptGtfToTab( infile, outfile ):
    ...
    statement = '''cat %(infile)s | python %(scriptsdir)s/gtf2tab.py -f --log=%(outfile)s.log | awk '{if (NR==1) {print $0"\\tgene_biotype\\tgene_name"} else {print $0"\\tprotein_coding\\tunknown"}}' > %(outfile)s'''
    P.run()
