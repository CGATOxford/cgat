    ...
    def addFilter( self, statement, track, slice ):
        ...
        if slice == "all" or slice == None:
            ...
            if ":" in slice:
                ...
                if "workspace" in o and workspace not in o["workspace"]: return None
                if "subset" in o and subset not in o["subset"]: return None
                if "slice" in o and xslice not in o["slice"]: return None
                if "track" in o and xtrack not in o["track"]: return None
...
class Annotator( AnnotatorSlicer, TrackerSQL ):
    ...
    tablename = None
    columns = ("over", "category", "fold", "pvalue", "qvalue", "observed", "expected")
    order = "pvalue"
    fdr = 0.10
...
class AnnotatorPower( TrackerSQL ):
    ...
    def __call__(self, track, slice = None ):
        ...
        if not self.tablename: raise NotImplementedError("table not specified")
        if not self.select: raise NotImplementedError("invalid use of base class - only use derived classes")
    ...
    def __call__(self, track, slice = None ):
        ...
        if slice:
            ...
            if ":" in slice:
                ...
                categories = slice.split("-")
                category_sql = "AND category IN ('%s')" % ("','".join(categories))
