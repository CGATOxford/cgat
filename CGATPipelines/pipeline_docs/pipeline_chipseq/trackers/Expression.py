...
class Correlations(DefaultTracker):
    ...
    def __call__(self, track, slice=None):
        field = self.mField
        data = self.getValues(
            "SELECT %(field)s FROM %(track)s AS e ORDER BY cluster_id" % locals())
...
class ExpressionReplicateCorrelation(DefaultTracker):
    ...
    def __call__(self, track, slice=None):
        ...
        if track.endswith("_levels"):
            track = track[:-len("_levels")]
...
class ExpressionFullCorrelation(DefaultTracker):
    ...
    def getSubset(self, subset=None):
        ...
        all_tracks = self.get(
            "SELECT DISTINCT track1, replicate1 FROM %s ORDER BY track1,replicate1" % self.tablename)
        if not subset:
...
class ExpressionDifferencesSummaryPerGroup(ExpressionTracker):
    ...
    def __call__(self, track, slice=None):
        ...
        result["tested"] = self.getValue(
            "SELECT COUNT(distinct gene_id) FROM %(track)s AS a, probeset2transcript AS e WHERE e.cluster_id = a.cluster_id AND e.source = '%(slice)s'" % locals())
        result["called"] = self.getValue(
            "SELECT COUNT(distinct gene_id) FROM %(track)s AS a, probeset2transcript AS e WHERE %(cutoff)s AND e.cluster_id = a.cluster_id AND e.source = '%(slice)s'" % locals())
        result["upregulated"] = self.getValue(
            "SELECT COUNT(distinct gene_id) FROM %(track)s AS a, probeset2transcript AS e WHERE %(cutoff)s AND fold > 1 AND e.cluster_id = a.cluster_id AND e.source = '%(slice)s'" % locals())
        result["downregulated"] = self.getValue(
            "SELECT COUNT(distinct gene_id) FROM %(track)s AS a, probeset2transcript AS e WHERE %(cutoff)s AND fold < 1 AND e.cluster_id = a.cluster_id AND e.source = '%(slice)s'" % locals())
        return result
...
class ExpressionDifferencesSummary(ExpressionTracker):
    ...
    def __call__(self, track, slice=None):
        ...
        cutoff = "a.%s %s" % (self.mCutoffField, self.mCutoffThreshold)
        result["tested"] = self.getValue(
            "SELECT COUNT(DISTINCT e.gene_id) FROM %(track)s AS a, probeset2transcript AS e WHERE a.cluster_id = e.cluster_id AND source = '%(slice)s'" % locals())
        result["called"] = self.getValue(
            "SELECT COUNT(DISTINCT e.gene_id) FROM %(track)s AS a, probeset2transcript AS e WHERE a.cluster_id = e.cluster_id AND %(cutoff)s AND source = '%(slice)s'" % locals())
        result["upregulated"] = self.getValue(
            "SELECT COUNT(DISTINCT e.gene_id) FROM %(track)s AS a, probeset2transcript AS e WHERE a.cluster_id = e.cluster_id AND %(cutoff)s AND fold > 1 AND source = '%(slice)s'" % locals())
        result["downregulated"] = self.getValue(
            "SELECT COUNT(DISTINCT e.gene_id) FROM %(track)s AS a, probeset2transcript AS e WHERE a.cluster_id = e.cluster_id AND %(cutoff)s AND fold < 1 AND source = '%(slice)s'" % locals())
        if self.mWithResultsDir:
            ...
            if os.path.exists(resultdir):
                result[
                    "pdf"] = "`pdf <%(resultdir)s/%(foreground)s.sam.expdiffsam.pdf>`_" % locals()
        return result
...
class DifferentiallyExpressedGenesFoldChangeCategories(ExpressionTracker):
    ...
    def __call__(self, track, slice=None):
        ...
        r["intermediate"] = self.getValue(
            statement % ("%(t1)f and %(t2)f" % locals()))
        r[">%s fold up" % self.mThreshold] = self.getValue(
            statement % ("fold > %(t2)f" % locals()))
...
class GeneList(ExpressionTracker):
    ...
    def __call__(self, track, slice=None):
        ...
        if self.mMaxEntries:
            statement += " LIMIT %i" % self.mMaxEntries
...
class CandidateGenesPeakval(CandidateGenesMotifs):
    mColumnsVariable = (
        "nprobes", "nintervals", "exp-pvalue", "min-peakval", "max-peakval", "description")
    ...
    def getSQLStatement(self, track, slice=None):
        ...
        peakvals = self.getValues(
            "SELECT peakval FROM %(master)s_intervals" % (locals()))
...
class DifferentiallyExpressedGenes(GeneList):
    ...
    mColumnsVariable = (
        "nprobes", "min-fold", "max-fold", "exp-pvalue", "description")
    mMaxEntries = None
...
class SummarySAM(ExpressionDifferencesSummary, PairTrackerSAM):
    pass


class SummaryTTest(ExpressionDifferencesSummary, PairTrackerTTest):
    pass


class SummaryTTestQValue(ExpressionDifferencesSummary, PairTrackerTTestQValue):
    pass


class OverlapSAM(ExpressionDifferencesOverlap, PairTrackerSAM):
    pass


class OverlapTTest(ExpressionDifferencesOverlap, PairTrackerTTest):
    pass


class FoldChangeAll(DifferentiallyExpressedGenesFoldChange, PairTrackerAll):
    pass


class FoldChangeCategories(DifferentiallyExpressedGenesFoldChangeCategories, PairTrackerAll):
    pass


class FoldChangeSAM(DifferentiallyExpressedGenesFoldChange, PairTrackerSAM):
    pass


class FoldChangeTTest(DifferentiallyExpressedGenesFoldChange, PairTrackerTTest):
    pass


class FoldChange95CISAM(DifferentiallyExpressedGenesFoldChange, PairTrackerSAM):
    pass


class FoldChange95CITTest(DifferentiallyExpressedGenesFoldChange, PairTrackerTTest):
    pass


class DistanceToIntervalSAM(DifferentiallyExpressedGenesDistanceToInterval, PairTrackerSAM):
    pass


class DistanceToIntervalDistalSAM(DifferentiallyExpressedGenesDistanceToIntervalDistal, PairTrackerSAM):
    pass


class DistanceToIntervalTTest(DifferentiallyExpressedGenesDistanceToInterval, PairTrackerTTest):
    pass


class CandidateGenesMotifsTTest(CandidateGenesMotifs, PairTrackerTTest):
    pass


class CandidateGenesMotifsSAM(CandidateGenesMotifs, PairTrackerSAM):
    pass


class CandidateGenesPeakvalTTest(CandidateGenesPeakval, PairTrackerTTest):
    pass


class CandidateGenesPeakvalSAM(CandidateGenesPeakval, PairTrackerSAM):
    pass


class DifferentiallyExpressedGenesSAM(DifferentiallyExpressedGenes, PairTrackerSAM):
    pass


class DifferentiallyExpressedGenesTTest(DifferentiallyExpressedGenes, PairTrackerTTest):
    pass
...
class SummaryPerGroupSAM(ExpressionDifferencesSummaryPerGroup, PairTrackerSAM):
    pass


class SummaryPerGroupTTest(ExpressionDifferencesSummaryPerGroup, PairTrackerTTest):
    pass


class ExpressionIntervalCountsTTest (ExpressionIntervalCounts, PairTrackerTTest):
    pass


class ExpressionIntervalCountsSAM (ExpressionIntervalCounts, PairTrackerSAM):
    pass


class CandidateGenesOverlapTTest (CandidateGenesOverlap, PairTrackerTTest):
    pass


class CandidateGenesOverlapSAM (CandidateGenesOverlap, PairTrackerSAM):
    pass


class ExpressionNumMotifsSAM (ExpressionNumMotifs, PairTrackerSAM):
    pass
