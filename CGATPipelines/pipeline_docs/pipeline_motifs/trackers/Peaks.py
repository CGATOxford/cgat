...
class PeakShapeTracker( Tracker ):
    ...
    tracks = [ os.path.basename( x )[:-len(".peakshape.tsv.gz")] for x in glob.glob( os.path.join( DATADIR , "peakshapes", "*.peakshape.tsv.gz" )) ]
    slices = ["peak_height", "peak_width" ]
    
    scale = 1000

    def __call__(self, track, slice = None):
        fn = os.path.join( DATADIR, "peakshapes/%(track)s.peakshape.tsv.gz.matrix_%(slice)s.gz" % locals() )
