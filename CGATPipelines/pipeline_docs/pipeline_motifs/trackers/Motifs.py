...
class MemeChipRuns( IntervalTracker ):
    
    table = "memechip_summary"

    def __call__(self, track, slice = None ):

        memechip_dir = os.path.abspath( os.path.join( EXPORTDIR, "memechip") )
        if not os.path.exists( memechip_dir ): return None

        statement = '''SELECT track, masking, width, npeaks, path FROM %(table)s'''
        
        data = self.getAll(statement)
        
        data['path'] = [ "`memechip <%s.memechip/index.html>`_" % (os.path.join(memechip_dir,x)) for x in data['path'] ]
