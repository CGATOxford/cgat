...
class AveLength(Tracker):
    ...
    def getTracks(self, subset=None):
        tracks = [os.path.basename(x)[:-len(".gtf.gz")]
                  for x in glob.glob("gtfs/*_coding.gtf.gz")] + ["lncrna_final"] + ["refcoding"]
        return tracks
...
class TranscriptNumbers(Tracker):
    ...
    def getTracks(self, subset=None):
        ...
        tracks = [os.path.basename(glob.glob(
            "gtfs/*_coding.gtf.gz")[0])[:-len(".gtf.gz")], "lncrna_final", "refcoding"]
        return tracks
