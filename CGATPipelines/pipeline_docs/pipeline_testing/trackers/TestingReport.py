...
LogFileSummary = collections.namedtuple( "LogFileSummary", "info debug warning error" )
...
def summarizeLogFile( filename ):
    ...
    with open(filename) as f:
        for line in f:
            ...
            w = words[2]
            if w == "INFO": info += 1
            elif w == "DEBUG": debug += 1
            elif w == "WARNING": warning += 1
            elif w == "ERROR": error += 1
...
class ReportTable( TestingTracker ):
    ...
    def __call__(self, track):
        ...
        try:
            logfileresult = summarizeLogFile( os.path.join( track + ".dir", "sphinxreport.log" ) )
        except IOError:
            return

        report_file = os.path.join( track + ".dir", "report.log" )
        fn = os.path.abspath( os.path.join( track + ".dir", "report", "html", "pipeline.html" ) )
