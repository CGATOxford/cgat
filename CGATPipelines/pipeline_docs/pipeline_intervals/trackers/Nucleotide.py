...
class pGC(Annotations.AnnotationSlicer, IntervalTracker):
    ...
    def __call__(self, track, slice = None ):
        ...
        if not slice or slice == "all":
            data = self.getValues( """SELECT %(column)s FROM %(track)s_%(table)s AS d """ % locals() )
...
class TSS(Annotations.AnnotationSlicer, IntervalTracker):
    ...
    def getNormValues(self, track, table ):
        ...
        table2 = self.mTable2
        column = self.mColumn
        rawdata = self.getValues( """SELECT %(column)s FROM %(track)s_%(table)s AS d """ % locals() )
...
class GCvsCounts(Annotations.AnnotationSlicer, IntervalTracker):
    ...
    def __call__(self, track, slice = None ):
        ...
        if not slice or slice == "all":
            ...
            data = self.getAll( """SELECT %(column1)s,%(column2)s FROM %(track)s_%(table1)s AS t1, %(track)s_%(table2)s AS t2 WHERE t1.contig==t2.contig and t1.start==t2.start and t1.end==t2.end """ % locals() )

        return data #odict( ((self.mColumn, data),) )

#select avgval,pGC from macs_nuc as n, macs_intervals as i where n.end==i.end and n.start=i.start and n.contig==i.contig limit 1;
