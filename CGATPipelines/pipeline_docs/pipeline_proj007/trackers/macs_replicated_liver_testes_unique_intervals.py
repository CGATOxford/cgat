...
class replicatedUniqueIntervals( cpgTracker ):
    ...
    mPattern = "_replicated_liver_testes_unique_intervals$"

    def __call__(self, track, slice = None):
        data = self.getFirstRow( "SELECT COUNT(*) as number, round(AVG(end-start),0) as length FROM %(track)s_replicated_liver_testes_unique_intervals" % locals() )
...
class replicatedUniqueIntervalLengths( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getValues( "SELECT (end-start) FROM %(track)s_replicated_liver_testes_unique_intervals" % locals() )
        return { "length" : data }
...
class replicatedUniqueIntervalPeakValues( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getValues( '''SELECT i.peakval FROM %(track)s_replicated_liver_testes_unique_intervals u, %(track)s_replicated_intervals i
...
class replicatedUniqueIntervalAverageValues( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getValues( '''SELECT avgval FROM %(track)s_replicated_liver_testes_unique_intervals u, %(track)s_replicated_intervals i
                                  WHERE u.contig=i.contig
                                  AND u.start=i.start''' % locals() )
        return { "avgval" : data }
...
class replicatedUniqueIntervalFoldChange( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getValues( '''SELECT i.fold FROM %(track)s_replicated_liver_testes_unique_intervals u, %(track)s_replicated_intervals i
                                  WHERE u.contig=i.contig
                                  AND u.start=i.start''' % locals() )
...
class replicatedUniqueIntervalTSS( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        ...
        data = self.getValues( '''SELECT closest_dist FROM %(track)s_replicated_liver_testes_unique_intervals u, 
...
class replicatedUniqueIntervalCpGDensity( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getAll( '''SELECT pCpG FROM %(track)s_replicated_liver_testes_unique_intervals u, 
...
class replicatedUniqueIntervalCpGObsExp( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getAll( '''SELECT CpG_ObsExp FROM %(track)s_replicated_liver_testes_unique_intervals u, 
                               %(track)s_replicated_intervals i,%(track)s_replicated_capseq_composition c
                               WHERE u.contig=i.contig
                               AND u.start=i.start
                               AND c.gene_id=i.interval_id''' % locals() )
...
class replicatedUniqueIntervalCpGNumber( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getAll( '''SELECT nCpG FROM %(track)s_replicated_liver_testes_unique_intervals u, 
                               %(track)s_replicated_intervals i,%(track)s_replicated_capseq_composition c
                               WHERE u.contig=i.contig
                               AND u.start=i.start
                               AND c.gene_id=i.interval_id''' % locals() )
...
class replicatedUniqueIntervalGCContent( cpgTracker ):
    ...
    def __call__(self, track, slice = None):
        data = self.getAll( '''SELECT pGC FROM %(track)s_replicated_liver_testes_unique_intervals u, 
                               %(track)s_replicated_intervals i,%(track)s_replicated_capseq_composition c
                               WHERE u.contig=i.contig
                               AND u.start=i.start
                               AND c.gene_id=i.interval_id''' % locals() )
...
class replicatedUniqueIntervalTranscriptOverlap(featureOverlap):
    ...
    def __call__(self, track, slice = None):
        ...
        data = self.getValues( """ SELECT count(distinct gene_id) as intervals FROM (
                                   ...
                                   FROM %(track)s_replicated_%(ANNOTATIONS_NAME)s_overlap o, %(track)s_replicated_intervals r,%(track)s_replicated_liver_testes_unique_intervals u
                                   WHERE r.interval_id=o.gene_id
                                   AND r.contig=u.contig AND r.start=u.start)
