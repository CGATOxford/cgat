...
class replicatedUniqueIntervals(cpgTracker):
    ...
    def __call__(self, track, slice=None):
        data = self.getFirstRow(
            "SELECT COUNT(*) as number, round(AVG(end-start),0) as length FROM %(track)s_replicated_liver_testes_unique_intervals" % locals())
...
class replicatedUniqueIntervalLengths(cpgTracker):
    ...
    def __call__(self, track, slice=None):
        data = self.getValues(
            "SELECT (end-start) FROM %(track)s_replicated_liver_testes_unique_intervals" % locals())
        return {"length": data}
