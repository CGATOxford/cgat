...
class AnnotatorDistance( TrackerSQL ):
    ...
    mColumns = ("label", "fold", "observed", "expected", "cilower", "ciupper", "pvalue", "qvalue")
    mOrder = "fold"
...
class AnnotatorDistanceVolcano( TrackerSQL ):
    ...
    def getTracks( self ): return ["intronic", "intergenic"]
    def getSlices( self, subset = None ): return []

    def __call__(self, track, slice = None ):

        table = self.mTableName
        data = self.getAll( "SELECT CAST( observed AS FLOAT)/ expected, max( pvalue, 0.00001)  FROM %(table)s WHERE workspace = '%(track)s'" % locals() )
    ...
    def __call__(self, track, slice = None ):
        ...
        if slice:
            ...
            for pair in pairs:
                ...
                o[key] = value
            if "counter" in o and counter not in o["counter"]: return []
