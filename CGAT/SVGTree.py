    ...
    def getElements(self, x, y, map_node2height):
        ...
        for n in self.mTree.get_terminals():
            ...
            if cluster not in self.mMapCluster2Colour:
                self.mMapCluster2Colour[cluster] = COLOURS[
...
class BranchDecoratorHorizontalBranchLengthError(BranchDecoratorHorizontalBranchLength):
    ...
    def __init__(self, tree, error_tree, *args, **kwargs):
        BranchDecoratorHorizontalBranchLength.__init__(
            self, tree, *args, **kwargs)
...
class BranchDecoratorHorizontalBranchLengthWithKaks(BranchDecoratorHorizontalBranchLength):
    ...
    def getText(self, node_id):
        ...
        if d:
            text = " (%s)" % (self.mBranchKaksFormat %
                              self.mKaksTree.node(node_id).data.branchlength)
        else:
            text = ""
...
class SVGTree:
    ...
    def __init__(self, tree):
        ...
        self.mDecoratorHorizontalBranches = BranchDecoratorHorizontal(
            self.mTree)
    ...
    def calculateCanvasSize(self):
        ...
        self.mDataWidth = max(
            self.mNodeWidthsEnd) + max([self.mDecoratorExternalNodes.getWidth(x) for x in self.mTree.get_terminals()])
        ...
        ) + self.mDataWidth + self.mBorderWidth
...
# fill = "rgb(%i,%i,%i)" % WHITE) )
    ...
    def getFooterHeight(self):
        ...
        if "ruler" in self.mRulerElements:
            footer_height += 2 * self.mRulerTickSize + \
                1 + self.mSeparatorHeight
        if "scale" in self.mRulerElements:
            footer_height += 2 * self.mRulerTickSize + 1 + \
                self.mSeparatorHeight + self.mRulerFontSize
    ...
    def calculateCoordinates(self):
        ...
        def updateWidths(node_id):
            ...
            if d <= 0.0:
                d = 0.01
            right = self.mNodeWidthsStart[
                node_id] + int(d * self.mBranchScaleFactor)
        ...
        if rescale_width:
            ...
            self.mNodeWidthsStart = map(
                lambda x: int(x * f), self.mNodeWidthsStart)
            self.mNodeWidthsEnd = map(
                lambda x: int(x * f), self.mNodeWidthsEnd)
    ...
    def finalizePlot(self):
        ...
        for b in self.mBorderDecorators:
            self.addElements(
                b.getElements(x, self.getHeaderHeight(), self.mNodeHeights))
            x += b.getWidth()
...
if __name__ == "__main__":
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: SVGTree.py 2784 2009-09-10 11:41:14Z andreas $")
