...
def readTable(file,
    ...
    for l in lines:
        ...
        if truncate:
            if data[0] < min_val or data[0] > max_val:
                continue
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: plot_data.py 2782 2009-09-10 11:40:29Z andreas $")
    ...
    if options.layout:
        options.layout = tuple(map(int, options.layout.split(",")))
    if options.color:
        options.color -= 1
        ...
        for x in range(1, len(legend)):
            ...
            if options.scatter:
                if options.point_size:
                    lines.append(
                        pylab.scatter(xvals, yvals, options.point_size, c=colors, **plot_kwargs))
                else:
                    # lines.append ( pylab.scatter( xvals, yvals, c = colors )
                    # ) # , vmin=min(colors),vmax=max(colors), facetted=False))
                    lines.append(pylab.scatter(
                        xvals, yvals, c=colors, vmin=min(colors), vmax=max(colors), **plot_kwargs))
            else:
                if options.error_bars:
                    ...
                    assert len(xvals) == len(yvals), "unequal number of values: %i != %i" % (
                        len(xvals), len(yvals))
