...
def collect(infile, with_headers=False):
    ...
    for line in lines:
        ...
        try:
            (code, goid, scount, stotal, spercent, bcount, btotal, bpercent, pover,
             punder, goid, category, description) = line[:-1].split("\t")[:13]
        except ValueError:
    ...
    if options.pattern_filename:
        ...
        if options.titles:
            ...
            if len(titles) != len(input):
                raise "Number of titles and files different: %i != %i" % (
                    len(titles), len(input))
        else:
            titles = input
