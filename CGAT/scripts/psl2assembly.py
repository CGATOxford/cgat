...
class Builder(Processor):
    ...
    def buildPileUp(self, sbjct_id, sbjct_start, sbjct_end, matches, map_genome2transcript=None):
        ...
        genome = self.mGenomeFasta.getSequence(
            sbjct_id, "+", sbjct_start, sbjct_end)

        if map_genome2transcript:
            mapped = alignlib_lite.py_makeAlignmentBlocks()
            genome = "".join(
                [genome[x] for x in range(len(genome)) if map_genome2transcript.mapRowToCol(x) != -1])
        ...
        for match in matches:
            ...
            query = self.mQueriesFasta.getSequence(
                match.mQueryId, match.strand)
...
class BuilderTranscribedLocus(Builder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mOutFileStrand.write(
            "gene_id\tstrand\tnmatches\tnpos\tnneg\tnintrons\tnintrons_pos\tnintrons_neg\tnintrons_unknown\n")
    ...
    def guessStrand(self, matches, intron_counts=None):
        ...
        self.mOutFileStrand.write("%s\t%s\t%i\t%i\t%i\t%i\t%i\t%i\t%i\n" %
                                  (self.mOutputId,
                                   ...
                                   nnegative,
                                   nintrons_positive +
                                   nintrons_negative + nintrons_unknown,
                                   nintrons_positive,
                                   nintrons_negative,
                                   nintrons_unknown))
    ...
    def process(self, sbjct_id, sbjct_start, sbjct_end, matches):
        ...
        map_transcript2genome, sequence, strand = self.getTranscriptMap(
            sbjct_id, sbjct_start, sbjct_end, matches)
        ...
        if self.options.output_pileup:
            self.outputPileUp(
                sbjct_id, sbjct_start, sbjct_end, matches, match, sequence)
    ...
    def outputPileUp(self, sbjct_id, sbjct_start, sbjct_end, matches, match, sequence):
        ...
        mali, identifiers, seqs = self.buildPileUp(
            sbjct_id, sbjct_start, sbjct_end, matches)
...
class BuilderTranscriptSpliced(BuilderTranscribedLocus):
    ...
    def getTranscriptMap(self, sbjct_id, sbjct_start, sbjct_end, matches):
        ...
        genome = self.mGenomeFasta.getSequence(
            sbjct_id, "+", sbjct_start, sbjct_end).upper()
        ...
        def hasSpliceMotif(sequence):
            ...
            s = sequence.upper()
            r = string.translate(
                s.upper(), string.maketrans("ACGTacgt", "TGCAtgca"))[::-1]
    ...
    def getIntrons(self, is_exon):
        ...
        l = len(is_exon)
        start, end = 0, l - 1
        while start < l and is_exon[start] == 0:
            start += 1
        while end >= 0 and is_exon[end] == 0:
            end -= 1
...
class BuilderCoverage(Builder):
    ...
    def process(self, sbjct_id, sbjct_start, sbjct_end, matches):
        ...
        if self.mMapId2Coverage:
            ...
            extra_info = "%6.4f\t%6.4f" % (
                (float(c) / t), math.sqrt(pooled_variance))
...
class BuilderIndels(Builder):
    ...
    def process(self, sbjct_id, sbjct_start, sbjct_end, matches):
        ...
        columns = Mali.convertAlignlib2Mali(
            mali, identifiers, seqs).getColumns()
        ...
        for column in columns[1:-1]:
            ...
            if (is_gap and naligned >= min_support) or \
                ...
                naligned_right = min(
                    is_exon[g + 1:min(lgenome, g + self.mRegionSize + 1)])
        ...
        for key, ids in list(tails.items()):
            ...
            for name, motifs in list(self.mMotifs.items()):
                if direction == "+":
                    ...
                    pos = genome.find(
                        motifs[1], mapped, mapped + self.mMotifArea)
        ...
        for match in matches_with_introns:
            ...
            for q, s, b in zip(match.mQueryBlockStarts[1:], match.mSbjctBlockStarts[1:], match.mBlockSizes[1:]):
                if last_s - s >= self.mIntronSize:
                    ...
                    # coordinates
                    pos1, pos2 = last_s - 1 - \
                        match.mSbjctFrom, s - match.mSbjctFrom
                    for mm in matches_with_introns:
                        if match.mQueryToken == mm.mQueryToken:
                            continue
...
class FilterExonExtenders(Filter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mOutFile.write(
            "tName\ttStart\ttEnd\tqName\trName\tintron\tnOverlap\n")
        ...
        for match_without_introns in matches_without_introns:
            for match_with_introns in matches_with_introns:
                ...
                for intron_start, intron_end in match_with_introns.iterator_introns():
                    ...
                    if d > 0:
                        continue
                    intron_sequence = self.mGenomeFasta.getSequence(
                        sbjct_id, "+", intron_start, intron_end)
                    intron_type, prime5, prime3 = Genomics.GetIntronType(
                        intron_sequence, both_strands=True)
...
class FilterTranscriptMergers(Filter):
    ...
    # minimum avegare coverage in adjacent exons to consider intron for
    # deletion
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mOutFile.write(
            "tName\ttStart\ttEnd\tqName\tiStart\tiEnd\tiLen\tcExons\tcIntrons\n")
    ...
    def filter(self, sbjct_id, sbjct_start, sbjct_end, matches):
        ...
        for match in matches:
            ...
            for start, end in exons:
                ...
                if start - last_end < self.mMinIntronSize:
                    continue
...
class FilterDuplicates(Filter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mOutFile.write(
            "rep\tmem\tlRep\tlMem\tnoverlap\tdelta\tnreads\tPL\tPS\tP\n")

        if options.loglevel >= 1:
            options.stdlog.write(
                "# %s: obtaining length distribution\n" % self.mName)
            options.stdlog.flush()
    ...
    def filter(self, sbjct_id, sbjct_start, sbjct_end, matches):
        ...
        for x in range(len(matches)):
            ...
            for y in range(x + 1, len(matches)):
                ...
                if overlap < self.mMinAlignmentOverlap:
                    continue

                if mx.mSbjctFrom > my.mSbjctTo or mx.mSbjctTo < my.mSbjctFrom:
                    continue
                if abs(mx.mSbjctFrom - my.mSbjctFrom) > self.mMinDistance or abs(mx.mSbjctTo - my.mSbjctTo) > self.mMinDistance:
                    continue
                ...
                PL = scipy.stats.norm.cdf( min(lx, lx + delta) + 0.5, loc=self.mLengthMean, scale=self.mLengthStd ) - \
                    scipy.stats.norm.cdf(
                        max(0, lx - delta - 0.5), loc=self.mLengthMean, scale=self.mLengthStd)
...
def readMapId2Coverage(infile):
    ...
    for line in infile:
        ...
        data = line[:-1].split("\t")
        x = CoverageInfo()
        id, x.mNMatches, x.mNCovered, x.mMean, x.mStddev = data[0], int(
            data[5]), int(data[6]), float(data[10]), float(data[12])
...
def main(argv=None):
    ...
    for method in options.methods:
        if method == "region":
            ...
            methods.append(
                BuilderConsensus(genome_fasta, queries_fasta, options))
        elif method == "locus":
            methods.append(
                BuilderTranscribedLocus(genome_fasta, queries_fasta, options))
        elif method == "transcript":
            methods.append(
                BuilderTranscriptSpliced(genome_fasta, queries_fasta, options))
        elif method == "coverage":
            methods.append(
                BuilderCoverage(genome_fasta, queries_fasta, options))
        elif method == "sbjct_coverage":
            methods.append(
                BuilderSbjctCoverage(genome_fasta, queries_fasta, options))
        elif method == "indels":
    ...
    for f in options.filters:
        if f == "imperfect":
            post_filters.append(
                FilterImperfect(genome_fasta, queries_fasta, options))
        elif f == "exon-extenders":
            post_filters.append(
                FilterExonExtenders(genome_fasta, queries_fasta, options))
        elif f == "duplicates":
            pre_filters = [
                FilterDuplicates(genome_fasta, queries_fasta, options)] + pre_filters
        elif f == "transcript-mergers":
            pre_filters.append(
                FilterTranscriptMergers(genome_fasta, queries_fasta, options))
