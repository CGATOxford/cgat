...
+---------------+----------------------------------------------------------------------+
    ...
    def __init__(self, filename_junctions, *args, **kwargs):
        ...
        for line in infile:
            ...
            junctions.add(contig, intron_start, intron_start + self.mSize,
                          (strand, intron_start, intron_end, gene_id, transcript_id))
            junctions.add(contig, intron_end - self.mSize, intron_end,
                          (strand, intron_start, intron_end, gene_id, transcript_id))
            njunctions += 1
        infile.close()
...
+---------------+----------------------------------------------------------------------+
    ...
    mHeader = ["code", "reference_codon", "reference_aa",
               "variant_type", "variant_codon", "variant_aa"]
    ...
    def update(self, snp):
        ...
        if snp.pos >= lcontig:
            E.warn("snp is out of range: %s:%i => %s" %
                   (contig, lcontig, ";".join(map(str, snp))))
            self.mCode = "X"
            return
                    ...
                    for x in range(start, end):
                        if x in j:
                            ...
                            for junction_strand, next_start, frame in j[x]:
                                ...
                                if frame == 0:
                                    self.mReferenceCodons.add(
                                        self.mFasta.getSequence(contig, strand, start, end).upper())
                                    continue

                                codon = self.mFasta.getSequence( contig, strand, start, x + 1) + \
                                    self.mFasta.getSequence(
                                        contig, strand, next_start, next_start + end - (x + 1))
...
def main(argv=None):
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: snp2table.py 2861 2010-02-23 17:36:32Z andreas $", usage=globals()["__doc__"])
    ...
    if options.filename_annotations:
        modules.append(BaseAnnotatorCodon(
            options.filename_annotations, fasta=fasta, junctions=junctions))
    if options.filename_junctions:
        modules.append(
