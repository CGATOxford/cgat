...
def outputOntologyResults(all_results, go2infos, options):
    ...
    for testpair, data in all_results.items():
        ...
        for key, result in data.mResults.items():
            ...
            outfile.write(
                "\t".join((ontology, genelist, code, str(result), str(n))))
            if has_fdr:
                outfile.write("\t%e" % result.fdr)
            outfile.write("\n")
...
def main(argv=None):
    ...
    E.info("read %i gene list from %s" %
           (len(gene_lists), options.filename_genes))
    if len(gene_lists) == 0:
    ...
    for key, l in gene_lists.items():
        ...
        if options.strict:
            assert len(missing) == 0, \
                "%s: %i genes in foreground but not in background: %s" % (
