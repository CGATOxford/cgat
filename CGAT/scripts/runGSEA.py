...
title_font = {'fontname':'Ubuntu', 'size':'20','color':'darkblue', 'weight':'bold', 'verticalalignment':'bottom'} # Bottom vertical alignment for more space
axis_font = {'fontname':'Ubuntu', 'size':'16','weight':'bold'}
axis_font_h = {'fontname':'Ubuntu', 'size':'12','weight':'bold'}
title_font_h = {'fontname':'Ubuntu', 'size':'20', 'color':'darkblue', 'weight':'bold'}
title_font_ran = {'fontname':'Ubuntu', 'size':'16', 'color':'darkblue', 'weight':'bold'}
...
def fancy_dendrogram(*args, **kwargs):
    ...
    if not kwargs.get('no_plot', False):
        plt.title('Hierarchical Clustering Dendrogram (truncated)',**title_font)
        plt.xlabel('\nGene set index or (Cluster size)',fontsize=18,weight='bold')
...
def preprocess_geneset(file,m,n,id_exp):
    ...
    for item in lines:
        ...
        if len(F_ID) in range(m,n+1):
            included_list.append([item[0],item[1],len(Gene_ID),len(F_ID),F_ID])
            t=t+1
            INDI[c]=t
        else:
...
def generate_gen_set_report(EX,IN,m,n,Geneset_indicator):
       # Create gene set details
        ...
        with open("CGAT_Gene_set_details.tsv", 'w') as f:
            ...
            f.write("Gene set size filters (min=" + str(m) + ",max=" + str(n) + ") resulted in filtering out " + str(len(EX)) + "/" + str(C) + " gene sets." + "\n") 
            f.write("The remaining " + str(C-len(EX)) + " gene sets were used in the analysis." + "\n")
            f.write("List of gene sets used and their sizes (restricted to features in the specified dataset) are mentioned here:" + "\n")
            f.write("\n" + "NAME" + "\t" + "ORIGINAL SIZE" + "\t" + "AFTER RESTRICTING TO EXPRESSION DATASET" + "\t" + "STATUS" + "\n")
            for i in range(0,len(Geneset_indicator)):
                if(Geneset_indicator[i]<0):
                    f.write(EX[abs(Geneset_indicator[i])-1][0] + "\t" + str(EX[abs(Geneset_indicator[i])-1][2]) + "\t" + "\t" + " Rejected!" + "\n")
                else:
                    f.write(IN[Geneset_indicator[i]-1][0] + "\t" + str(IN[Geneset_indicator[i]-1][2]) + "\t" + str(IN[Geneset_indicator[i]-1][3]) + '  ' + "\n")
        f.close()
        return

    
def generate_leading_edge_m(I_U,I_D,f_U,f_D,IN_PRO,ch,hm,dict_new,dfp,ufp,matr,OE,OEI,ID_for_leading,store_leading,gene_set_a,STORE_GENE_LEADING_INFO):
    #####################################################################
    ...
    #######################################################################
            ...
            for i in FINAL_L: 
                ...
                indices = [dict_new[x] for x in inter]
                indices.sort()
                matr[indices]=1
                if(OE[AW]<0):
                ...
                store_leading.append([IN_PRO[AW][0],len(II[0]),len(v1[0]),list(v2),STORE_GENE_LEADING_INFO[0][AW]                                           ,STORE_GENE_LEADING_INFO[1][AW],CW])
                matr.fill(0)
                del indices
                del inter
                del II
...
def heatmap_plot(hh1,nl):
    fig, ax = plt.subplots()  
    cMap = ListedColormap(['white','palegreen','yellowgreen','limegreen','lawngreen', 'mediumseagreen', 'springgreen','darkgreen','c','red'])
    heatmap = ax.pcolor(hh1, cmap=cMap, alpha=0.8,edgecolor='k',clip_on=False,facecolor='w',lw=0.4)
    cbar = plt.colorbar(heatmap, orientation='vertical',pad=0.06)
    cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(),**axis_font_col)  # vertical colorbar
   ...
   # ax.set_xlabel("\nOverlap intensity between Leading Edge subsets",fontsize=18,weight='bold',color='darkblue')
    ax.set_title("Overlap intensity between Leading Edge subsets",**title_font)
    plt.ylabel('Gene sets',size=20,weight='bold',color='darkblue')
    # rotate the
    plt.xticks(rotation=90)

    plt.tight_layout()
    plt.savefig('Leading_Edge_Analysis/Leading_Edge_similarity_matrix_heatmap.jpeg',bbox_inches='tight')
    plt.close()
    return
...
def heatmap_leading_edge_subset(LM,j,SUBSET_dict,jac_f):
    ...
    for i in range(0,len(pairs)):
            ...
            if (len(F_ID)>0):
                ...
                store_leading_h1[h[0]][h[1]]= (LM[h[0]][2]/(LM[h[0]][2]+LM[h[1]][2]))
                jac_f.append((LM[h[0]][2]/(LM[h[0]][2]+LM[h[1]][2])))
            else:
                jac_f.append(0)
    o_clus.append(0)
    for i in range(0,len(store_leading_h1)):             # This section is for leading edge analysis.
        o_clus.append(np.argmax(store_leading_h1[i,:]))  # I am storing index to use it clustering
...
def heatmap_plot_assign(ASS_M,name_x,name_y,file_to_save,oi,cMap):
    fig, ax = plt.subplots()
    heatmap = ax.pcolor(ASS_M, cmap=cMap, alpha=0.8,edgecolor='k',lw=1,clip_on=False,facecolor="white")
    fig = plt.gcf()
    ...
    plt.xticks(rotation=90)
    plt.ylabel('Gene sets',size=20,weight='bold',color='darkblue')
    plt.xlabel('\nLeading Edge genes',size=20,weight='bold',color='darkblue')
    plt.tight_layout()
...
def leading_edge_clustering(SCB,SCB_2,name_x,name_y,hh):
    ...
    f2.close()
    ########################################################################################
    cMap = ListedColormap(['white','limegreen','lawngreen','darkgreen','dodgerblue','c','blue','orangered','tomato','red'])
    #cMap = ListedColormap(['white','palegreen','yellowgreen','limegreen','lawngreen', 'green', 'mediumseagreen','springgreen','darkgreen','red'])
    heatmap_plot_assign(FINAL_BOOLEAN_MA,name_l_x_clus,name_l_y_clus,'Leading_Edge_Analysis/Leading_Edge_heatmap_clustered.jpeg',1,cMap)
    return


def plot_enrichment_score(STORE_ENRICHMENT_SCORE,ORIGINAL_ES_INDEX,ORIGINAL_ES,IN_list,A_W,l_ID):
        plt.figure(figsize=(8, 6), dpi=80)
        plt.plot(STORE_ENRICHMENT_SCORE[A_W], color="firebrick", linewidth=3.5, linestyle="-")
        plt.plot(ORIGINAL_ES_INDEX[A_W],ORIGINAL_ES[A_W],'go',markersize=15, mfc='none', mew=2.5,label="ES")
        plt.legend(bbox_to_anchor=(0.97, 1),loc=2,prop=legend_properties,handletextpad=0)
        ...
        q2 = ''.join(['\nGene List Index','\n\nNumber of genes: ',str(l_ID),'(in list), ',str(q[3]),'(in gene set)'])
...
def plot_random_ES(store_permute,A_W,IN_list):
    ...
    n, bins, patches = plt.hist(Rans_plot,80,normed=False, color='magenta',alpha=0.9,histtype='bar')
    plt.xticks(fontsize = 12,weight='bold')
    plt.yticks(fontsize = 12,weight='bold')
    plt.xlabel('\nEnrichment Score',**axis_font)
    plt.ylabel('Frequency',**axis_font,labelpad=28)
   # plt.yticks(np.arange(0,max(n)+1, 1.0))
    q = IN_list[A_W]
...
def plot_dendrogram_for_cluster(ZZ):
    ...
    plt.tight_layout()
    plt.savefig("Leading_Edge_Analysis/Last_20_merged_cluster_dendrogram",bbox_inches='tight')
    plt.close()
    fig, ax = plt.subplots()
    ...
    plt.ylabel('Distance',fontsize=18,weight='bold',labelpad=28)
    plt.tight_layout()
    plt.savefig("Leading_Edge_Analysis/Hierarchical_cluster_dendrogram",bbox_inches='tight')
    plt.close()
    return
...
def main(argv=None):
    ...
    parser = EW.OptionParser(version="%prog version: $Id$",usage=globals()["__doc__"])

    parser.add_option("-f", "--filename", dest="file_name", type="string",
    ...
    parser.add_option("-m", "--minimum", dest="min_gene", type="int",
                      help="gene sets smaller than this are excluded from the analysis "
                      "[default=%default].")

    parser.add_option("-x", "--maximum", dest="max_gene",type="int",
    ...
    parser.add_option("-t", "--threshold", dest="fdr_user", type="float",
                      help="false discovery rate (significance threshold) for leading edge analysis")
        
    parser.add_option("-l", "--num_leading", dest="fdr_num", type="int",
    ...
    # Preprocess geneset
    EX_list,IN_list,Geneset_indicator = preprocess_geneset(options.geneset,options.min_gene,options.max_gene,ID)
    generate_gen_set_report(EX_list,IN_list,options.min_gene,options.max_gene,Geneset_indicator)

    # Store filtered geneset. Because IN_list contains annotation and description also,for each geneset.
    GG = [item[4:len(item)] for item in IN_list]


    # Store index of each id (from the ranked list of expression data as a dicitionary)
    ind_dict = dict((k,i) for i,k in enumerate(ID))


    # Calculate enrichment score for each geneset.
    # Create boolean array for total number of id in expression data and float array for Enrichment Score of options.iteration permutation.
    ...
    for i in GG: 
        ...
        indices.sort()
        S = len(i[0])
        E = calculate_enrichment_score(indices,temp,expression_value,S,temp_2)
        del indices
        del inter
        ...
        t0 = ORIGINAL_ES_INDEX[count]               # This section has been added by me for "Leading Edge Analysis".
        ...
        signal_l=(tag/100)*(1-(gene_l/100))*(len(ID)/(len(ID)-S))
    ...
    if(len(NES_UP_INDEX)>0):
            ...
            b,fdr_upregulated,w1,w2 = sm.multipletests(p_value_up[NES_UP_INDEX], alpha=0.05, method='fdr_bh',is_sorted=False, returnsorted=False)   

    # For downregulated
    if(len(NES_DOWN_INDEX)>0):
            p_value_down = nominal_p_nes[ORIGINAL_NES<0]
            b,fdr_downregulated,w1,w2 = sm.multipletests(p_value_down[NES_DOWN_INDEX], alpha=0.05, method='fdr_bh',is_sorted=False, returnsorted=False)   
    ...
    f.write("@ "+str(len(NES_UP_INDEX))+"/"+str(len(IN_list))+" gene sets are upregulated"+"\n")
    f.write("@ "+str(sum(fdr_upregulated<0.05))+" gene sets are significant at FDR < 5%"+"\n")
    f.write("@ "+str(sum(fdr_upregulated<0.01))+" gene sets are significant at FDR < 1%"+"\n")
    w1=nominal_p[ORIGINAL_NES>=0]
    w2=nominal_p[ORIGINAL_NES<0]
    f.write("@ "+str(sum(w1<0.01))+" gene sets are significantly enriched at nominal pvalue < 1%"+"\n")
    f.write("@ "+str(sum(w1<0.05))+" gene sets are significantly enriched at nominal pvalue < 5%"+"\n\n")
    f.write("Enrichment in phenotype (downregulated):"+"\n")
    f.write("@ "+str(len(NES_DOWN_INDEX))+"/"+str(len(IN_list))+" gene sets are upregulated"+"\n")
    f.write("@ "+str(sum(fdr_downregulated<0.05))+" gene sets are significant at FDR < 5%"+"\n")
    f.write("@ "+str(sum(fdr_downregulated<0.01))+" gene sets are significant at FDR < 1%"+"\n")
    f.write("@ "+str(sum(w2<0.01))+" gene sets are significantly enriched at nominal pvalue < 1%"+"\n")
    f.write("@ "+str(sum(w2<0.05))+" gene sets are significantly enriched at nominal pvalue < 5%"+"\n\n")
    f.close()
    ...
    for i in range(0,options.plot_no):
        if(len(NES_UP_INDEX)>0):
            ...
            plot_enrichment_score(STORE_ENRICHMENT_SCORE,ORIGINAL_ES_INDEX,ORIGINAL_ES,IN_list,AW,len(ID))
    ...
    plt.tight_layout()
    ax2 = plt.twinx()
    ax2.plot(NES_UP,fdr_upregulated,marker='s',color='firebrick',linestyle='')
    ax2.plot(NES_DOWN,fdr_downregulated,marker='s',color='firebrick',linestyle='')
    ...
    ax2.yaxis.set_tick_params('y', colors='firebrick',labelsize = 'x-large',width=20)
    ...
    n, bins, patches = plt.hist(ORIGINAL_NES, 50, normed=False, facecolor='darkgreen',alpha=0.9,histtype ='step',lw=4,color='darkgreen')
    plt.xticks(fontsize = 12,weight='bold')
    plt.yticks(fontsize = 12,weight='bold')
    ...
    if(len(NES_UP_INDEX)>0):
        with open(file_to_report_1, 'w') as f:
            ...
            for i in range(0,len(NES_UP_INDEX)):
                AW = up_for_plot[NES_UP_INDEX[i]]
                IT = IN_list[AW]
                f.write(IT[0] + "\t" + IT[1] + "\t" + str(IT[3]) + "\t" + str(ORIGINAL_ES[AW]) + "\t" + str(ORIGINAL_NES[AW]) + "\t" +                         str(nominal_p[AW]) + "\t" +  str(fdr_upregulated[i]) + "\t" + str(ORIGINAL_ES_INDEX[AW]) + "\t" +                         "tags=" + str(STORE_GENE_LEADING_INFO[0][AW]) + "%, list=" + str(STORE_GENE_LEADING_INFO[1][AW]) +                          "%" + "\n")
            f.close()
    ...
    if(len(NES_DOWN_INDEX)>0):
        with open(file_to_report_2, 'w') as f:
            ...
            for i in range(0,len(NES_DOWN_INDEX)):
                AW = down_for_plot[NES_DOWN_INDEX[i]]
                IT = IN_list[AW]
                f.write(IT[0] + "\t" + IT[1] + "\t" + str(IT[3]) + "\t" + str(ORIGINAL_ES[AW]) + "\t" + str(ORIGINAL_NES[AW]) +                         "\t" + str(nominal_p[AW]) + "\t" +  str(fdr_downregulated[i]) + "\t" + str(ORIGINAL_ES_INDEX[AW]) + "\t" +                         "tags=" + str(STORE_GENE_LEADING_INFO[0][AW]) + "%, list=" + str(STORE_GENE_LEADING_INFO[1][AW]) +                         "%" + "\n")
            f.close()


    # Write detail report of up and down regulated genes in to a single file, sorted by FDR values
    ...
    with open("CGAT_REPORT_FOR_fdr_sorted_up_down_expression_data.tsv", 'w') as f:
        ...
        for i in MERGE_INDEX: 
                ...
                if(i>=len(fdr_upregulated)):
                    ...
                    AW = up_for_plot[NES_UP_INDEX[i]]
                IT = IN_list[AW]
                f.write(IT[0] + "\t" + IT[1] + "\t" + str(IT[3]) + "\t" + str(ORIGINAL_ES[AW]) + "\t" + str(ORIGINAL_NES[AW]) +                         "\t" + str(nominal_p[AW]) + "\t" +  str(CW) + "\t" + str(ORIGINAL_ES_INDEX[AW]) + "\t" +                         "tags=" + str(STORE_GENE_LEADING_INFO[0][AW]) + "%, list=" + str(STORE_GENE_LEADING_INFO[1][AW]) +                         "%" + "\n")
        f.close()
    ...
    generate_leading_edge_m(NES_UP_INDEX,NES_DOWN_INDEX,fdr_upregulated,fdr_downregulated,IN_list,options.fdr_user,options.fdr_num,ind_dict,down_for_plot,up_for_plot,temp,ORIGINAL_ES                             ,ORIGINAL_ES_INDEX,ID_NEW,STORE_GENE_LEADING_MATRIX,GG,STORE_GENE_LEADING_INFO)
    ...
    f.write("There were "+str(options.fdr_num)+" gene sets used in the leading edge analysis (see below for details)"+"\n\n")
    f.write("Gene set"+"\t"+"Members"+"\t"+"Members in signal"+"\t"+"Tag%"+"\t"+"List%"+"\t"+"FDR-q value"+"\n")
    for i in STORE_GENE_LEADING_MATRIX:
        c=i
        f.write(c[0]+"\t"+str(c[1])+"\t"+str(c[2])+"\t"+str(c[4])+"%\t"+str(c[5])+"\t"+str(c[6])+"%\n")
    f.close()
    ...
    for j in SUBSET_dict_assign_matrix.keys():
        ...
        count=count+1
        name_l_x.append(j)
       # exp_for_clustering.append(expression_value[ind_dict[j]])
    ...
    STORE_UNCLUSTERED_BOOLEAN = np.zeros((len(STORE_GENE_LEADING_MATRIX),len(SUBSET_dict_assign_matrix)), dtype=np.int)
    STORE_CLUSTERED_BOOLEAN = np.zeros((len(STORE_GENE_LEADING_MATRIX),len(SUBSET_dict_assign_matrix)), dtype=np.float)
    count=0
    ...
    JAC_L,INTENSITY_FOR_CLUSTER = heatmap_leading_edge_subset(STORE_GENE_LEADING_MATRIX,options.fdr_num,SUBSET_dict,JAC_L)


    # HEATMAP OF UNCLUSTERED AND CLUSTERED ASSIGNMENT MATRIX
    cMap = ListedColormap(['white','red'])
    heatmap_plot_assign(STORE_UNCLUSTERED_BOOLEAN,name_l_x,name_l_y,'Leading_Edge_Analysis/Leading_Edge_heatmap_unclustered.jpeg',0,cMap)
    leading_edge_clustering(STORE_UNCLUSTERED_BOOLEAN,STORE_CLUSTERED_BOOLEAN,name_l_x,name_l_y,INTENSITY_FOR_CLUSTER)                                      
    ...
    plt.savefig('Leading_Edge_Analysis/Gene_in_leading_edge_subset.jpeg',bbox_inches='tight')
    plt.close()
    del temp_dict_k
    del temp_dict_v
    ...
    plt.figure(figsize=(11,6),dpi=80)
    n, bins, patches = plt.hist(JAC_L,70,normed=1, color='darkgreen',alpha=0.9)
    plt.xticks(fontsize = 12,weight='bold')
    plt.yticks(fontsize = 12,weight='bold')
    plt.xlabel('\nJacquard',**axis_font)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('Leading_Edge_Analysis/Jacquard_distribution.jpeg',bbox_inches='tight')
    plt.close()
