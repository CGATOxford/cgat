...
def CompareGeneStructures( xcmp_exons, ref_exons, 
    ...
    if ref_sequence and cmp_sequence:
        cmp_seq = alignlib.py_makeSequence( cmp_sequence)
        ref_seq = alignlib.py_makeSequence( ref_sequence)
    else:
    ...
    while e < len(cmp_exons) and r < len(ref_exons):
        ...
        if exon1.mPeptideFrom == 0 and exon1.mPeptideTo == 0:
            ...
            if cmp_seq and ref_seq and map_ref2cmp:
                ...
                tmp_ali = alignlib.py_makeAlignmentVector()

                xquery_from = max( exon2.mPeptideFrom / 3, exon1.mPeptideFrom / 3) 
                xquery_to = min(1 + exon2.mPeptideTo / 3, 1 + exon1.mPeptideTo / 3)

                alignlib.py_copyAlignment( tmp_ali, map_ref2cmp, xquery_from, xquery_to )

                percent_identity = alignlib.py_calculatePercentIdentity( tmp_ali,
                                                                      ref_seq,
                                                                      cmp_seq ) * 100
                percent_similarity = alignlib.py_calculatePercentSimilarity( tmp_ali ) * 100
...
def MapExons( exons, map_a2b ):
    ...
    long_map = alignlib.py_makeAlignmentVector()

    for x in range(map_a2b.getRowFrom(), map_a2b.getRowTo() + 1):
        y = map_a2b.mapRowToCol(x)
        if y:
            alignlib.py_addDiagonal2Alignment( long_map, 3 * (x - 1) + 1, 3 * x, 3 * (y - x) )
