        ...
        for line in infile:
            ...
            if line[0] == ">":
                ...
                if identifier in identifiers:
                    raise ValueError, "%s occurs more than once in %s and %s" % (
                ...
                outfile_index.write(
                    "%s\t%i\t%i\t" % (identifier, pos, outfile_fasta.tell()))
                lsequence = 0
            else:
                s = re.sub("\s", "", line[:-1])
                lsequence += len(s)
...
class IndexedFasta:
    ...
    def getSequence(self,
        ...
        if is_negative:
            p.reverse()
            p = MySequence(
                "c", string.translate(p[:], string.maketrans("ACGTacgt", "TGCAtgca")))
...
if __name__ == "__main__":
    ...
    parser = E.OptionParser(
        version="%prog version: $Id: GenomicIO.py 2781 2009-09-10 11:33:14Z andreas $", usage=globals()["__doc__"])
    ...
    if options.loglevel >= 1:
        options.stdlog.write(
            "# indexing the following files: %s\n" % (",".join(args[1:])))
