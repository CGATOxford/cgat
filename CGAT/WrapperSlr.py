...
class Slr:
    ...
    def parseLog(self, lines, result):
        ...
        result.mLogLikelihood = float(
            re.match("# lnL = (\S+)", lines[0]).groups()[0])
        del lines[0]
        ...
        if re.search("nan", lines[0]):
            ...
            try:
                result.mKappa, result.mOmega = map(
                    float, re.match("# Kappa\s+=\s+(\S+)\s*Omega\s+=\s+(\S+)", lines[0]).groups())
            except AttributeError:
                raise ParsingError(
                    "parsing error for kappa and omega", lines[0])
        ...
        result.mScaleFactor = float(re.match(
            "# Scaling tree to neutral evolution. Factor =\s*(\S+)", lines[0]).groups()[0])

        while not re.match("# Significance", lines[0]):
            del lines[0]
        del lines[0]
