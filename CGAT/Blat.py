...
class Match:
    ...
    def convertCoordinates( self ):
        ...
        if self.mSbjctStrand == "-":
            self.mSbjctFrom, self.mSbjctTo = self.mSbjctLength - self.mSbjctTo, self.mSbjctLength - self.mSbjctFrom
            query_starts = [ self.mQueryLength - x - y for x,y in zip( query_starts, block_sizes) ]
            sbjct_starts = [ self.mSbjctLength - x - y for x,y  in zip( sbjct_starts, block_sizes) ]
    ...
    def switchTargetStrand( self ):
        ...
        # invert alignment
        self.mQueryBlockStarts = [ self.mQueryLength - x - y for x,y  in zip( query_starts, block_sizes) ]
        self.mQueryBlockStarts.reverse()
        self.mSbjctBlockStarts = [ self.mSbjctLength - x - y for x,y  in zip( sbjct_starts, block_sizes) ]
    ...
    def fromTable( self, data ):
        ...
        try:
            ...
            raise ParsingError( "parsing error: %i fields" % len(data), "\t".join(data ))
        ...
        if self.mQueryLength != 0:
            self.mQueryCoverage = 100.0 * (self.mNMismatches + self.mNMatches) / self.mQueryLength
        else:
            self.mQueryCoverage = 0
        
        if self.mSbjctLength != 0:
            self.mSbjctCoverage = 100.0 * (self.mNMismatches + self.mNMatches) / self.mSbjctLength
        else:
    ...
    def fromMap(self, map_query2target, use_strand = None ):
        ...
        if self.mBlockSizes:
            ...
            self.mQueryBlockStarts = map(int, self.mQueryBlockStarts[:-1].split(",") )
            self.mSbjctBlockStarts = map(int, self.mSbjctBlockStarts[:-1].split(",") )
        else:
            self.mBlockSizes = []
            self.mQueryBlockStarts = []
            self.mSbjctBlockStarts = []
        ...
        if use_strand and self.strand == "-":
            self.mQueryFrom, self.mQueryTo = self.mQueryLength - self.mQueryTo, self.mQueryLength - self.mQueryFrom
...
class MatchPSLX( Match ):
    ...
    def fromPSL( self, other, query_sequence, sbjct_sequence ):
        ...
        if self.strand == "-":
            query_sequence = string.translate( query_sequence, string.maketrans("ACGTacgt", "TGCAtgca") )[::-1]
        ...
        for start_query, start_sbjct, size in other.getBlocks():
            self.mQuerySequence.append( query_sequence[start_query-offset_query:start_query-offset_query + size ] )
            self.mSbjctSequence.append( sbjct_sequence[start_sbjct-offset_sbjct:start_sbjct-offset_sbjct + size ] )
...
def iterator2( infile ):
    ...
    while 1:
        ...
        if line.startswith("psLayout version 3"):
            for x in range(4): infile.readline()
            continue
        match = Match()
        # match.fromTable( line[:-1].split() )
        yield match
...
def iterator_target_overlap( infile, merge_distance ):
    ...
    while 1:
        ...
        if match.mSbjctId != last_sbjct_id or match.mSbjctFrom >= (end + merge_distance):
            if last_sbjct_id: yield matches 
            matches = []
...
def iterator_query_overlap( infile, merge_distance ):
    ...
    while 1:
        ...
        if match.mQueryId != last_query_id or match.mQueryFrom >= (end + merge_distance):
            if last_query_id: yield matches 
            matches = []
