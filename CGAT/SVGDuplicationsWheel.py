...
class DuplicationPlot:
    ...
    def buildColourMap(self):
        ...
        if self.mMiddleColour:
            ...
            self.mColourIncrement = float(
                (self.mMaxValue - self.mMinValue)) / num_steps

        d = map(lambda x, y: (x - y) / num_steps,
                self.mStopColour, start_colour)
        for x in range(num_steps):
    ...
    def finalizePlot(self):
        ...
        self.mScaleY = self.mHeaderHeight + self.mSeparator + \
            self.mDataHeight + self.mSeparator
        self.mScaleHeight = 2 * \
            max(self.mScaleFontSize, self.mMaxBoxSize) + self.mSeparator
        self.mScaleWidth = 0
        ...
        self.mFooterX = self.mHeaderWidth + self.mDataWidth + \
            self.mScaleWidth + self.mFooterWidth + 3 * self.mSeparator
        self.mFooterY = self.mHeaderHeight + self.mDataHeight + \
            self.mScaleHeight + self.mFooterHeight + 3 * self.mSeparator

        # compute actual page width and height
        self.mPageWidth = self.mHeaderWidth + self.mDataWidth + \
            self.mScaleWidth + self.mFooterWidth + 3 * self.mSeparator
