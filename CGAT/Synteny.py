...
def ReadOrthologs(infile):
    ...
    for line in infile:
        ...
        ortholog_id, gene_id, contig, strand, first_res, last_res = line[
            :-1].split("\t")[:6]
...
class Block:
    ...
    def update(self):
        if self.mMembers1:
            ...
            self.mBreakers1 = self.cleanBreakers(
                self.mMembers1, self.mBreakers1)
        if self.mMembers2:
            ...
            self.mBreakers2 = self.cleanBreakers(
                self.mMembers2, self.mBreakers2)
...
def Blocks2Orthologs(blocks):
    ...
    for block in blocks:
        o1 = Ortholog(
            block.mBlockId, block.mBlockId, block.contig1, "+", block.mFrom1, block.mTo1)
        orthologs1.append(o1)
        o2 = Ortholog(
            block.mBlockId, block.mBlockId, block.contig2, "+", block.mFrom2, block.mTo2)
