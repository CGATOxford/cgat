...
def main( argv = sys.argv ):
    ...
    for line in infile:
        ...
        if re.search( "output generated by", msg ): 
            if is_start:
        ...
        elif re.search( "task.log_at_level.\d+Task=(\S+)", msg ):
            checked_task = re.search( "task.log_at_level.\d+Task=(\S+)", msg ).groups()[0]
            
        elif re.search( "Job=\[(\S+)->(\S+)\]Missingfile\[(\S+)\]", msg):
            started_infiles, started_job, missing = re.search( "Job=\[(\S+)->(\S+)\]Missingfile\[(\S+)\]", msg).groups()
