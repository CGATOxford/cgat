...
venn.diagram2 <- function (x, filename, height = 3000, width = 3000, resolution = 500, 
...
{                                                                                                                                                             
    ...
    if (length(x) == 1) {                                                             
        ...
        grob.list <- VennDiagram::draw.single.venn(length(x[[1]]),                                                                                            
        ...
        grob.list <- VennDiagram::draw.pairwise.venn(length(a),                                                                                               
            length(b), length(intersect), list.names, ind = FALSE,                                                                                            
            ...)                                                                                                                                              
        if (!is.null(sub)) {                                                                                                                                  
            grob.list <- add.title(gList = grob.list, x = sub,                                                                                                
                pos = sub.pos, fontface = sub.fontface, fontfamily = sub.fontfamily,                                                                          
        ...
        grob.list <- VennDiagram::draw.triple.venn(length(a),                                                                                                 
            length(b), length(c), length(nab), length(nbc), length(nac),                                                                                      
            length(nabc), list.names, ind = FALSE, list.order = 1:3,                                                                                          
            ...)                                                                                                                                              
        if (!is.null(sub)) {                                                                                                                                  
            grob.list <- add.title(gList = grob.list, x = sub,                                                                                                
                pos = sub.pos, fontface = sub.fontface, fontfamily = sub.fontfamily,                                                                          
