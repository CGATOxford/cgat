...
class layout(object):
    ...
    def __init__(self,region_start, region_finish, title=0, page_size="a4", default_graph_priority=["gene_model","read_possitions", "read_count", "splice_varient", "default"]):
        """instantiates layout plot for particular region as speicifed by user interface

        list_of_tracks = list of track objects (i.e graph types) that need to be ploted
        ...
        if title == 0:
            self.title = "Region " + str(region_start) + "to " + str(region_finish)
        else:
            self.title = title
    ...
    def addTracks(self, new_track_obj):
        """ adds track object to to list_of_tracks to be ploted - called by track plotter """
    ...
    def generatePlot(self):
        ...
        for number in range(number_of_tracks):
            axarr[number].set_xlim([self.region_start,self.region_finish])
        ...
        for track in self.list_of_track_objects:
            """graphs with predetermined priority will be ploted first, then those with default priorities will be plotted second"""
            ...
            if track.get_Priority != "default":
                ...
                #if no priority set - 
                track.set_Priority(self.default_graph_priority.index[track.get_Name()])
                graphs_no_priority.append(track)
            
                
        #sort list of proritised tracks for plotting onto subplot - NEED TO TEST THIS? 
        sorted_priority_list = sorted(graphs_with_predef_priority, key=lambda track: track.get_Priority)
        sorted_nopriority_list = sorted( graphs_no_priority, key=lambda track: track.get_Priority)
        
        #append tracks into final ordered list which dictates order they need to be plotted in
        ...
        for track in sorted_priority_list:
            ...
            axarr[subplot_no].add_collection(track.draw_regions())  # I think i need to call the draw otherwise they don't get made? 
    ...
    """
        ...
        while number < self.number_of_tracks:
            for track in self.list_of_track_objects:
                ...
                self.figure.add_subplot(number_of_tracks,1,number) #this needs to be updated somehow with number so subplot in right possition
                number += 1
   ...
   """     
    ...
    def showPlot(self):
        """ show plot on screen or write to different files"""
           #return plt.show(self.figure, block=False) #block = lets return to command line when figures are open in non-interative mode
        return plt.show(self.generatePlot(), block=False)
        
        
    def save_plot(self, file_name, dpi=None, facecolor="w", edgecolor="w", orientation="portrait", papertype=None, format=None):
        """ file_name= string containing file path to save file to
            """
        return plt.savefig(file_name, dpi, facecolor, edgecolor,orientation, papertype, format)  
