...
#
#   $Id: orthologs2transcripts.py 2774 2009-09-10 10:00:32Z andreas $
#
...
def ReadMapGene2Transcripts( infile, peptides, filter=None ):
    ...
    for line in infile:
        if line[0] == "#": continue
        try:
            gene_id, sbjct_from, sbjct_to, sbjct_strand, sbjct_token, prediction_id = line[:-1].split("\t")
        except ValueError:
            continue
        
        if filter and not filter.has_key( gene_id): continue
        if not map_gene2transcripts.has_key(gene_id): map_gene2transcripts[gene_id] = []
        map_gene2transcripts[gene_id].append( (prediction_id, sbjct_token, sbjct_strand, sbjct_from, sbjct_to) )

    for key in map_gene2transcripts.keys():
        map_gene2transcripts[key] = RemoveRedundantTranscripts( map_gene2transcripts[key], peptides )
...
def WriteExons( token1, peptide1, cds1, transcript1,
    ...
    e1, e2 = 0, 0
    while cds1[e1].mGenomeTo <= dna_map_a2b.getRowFrom(): e1 += 1
    while cds2[e2].mGenomeTo <= dna_map_a2b.getColFrom(): e2 += 1    
...
def GetOrthologTranscripts( transcripts1, peptides1, cds1,
    ...
    alis1 = []
    alis2 = []
    for i in range(len(seqs1)): alis1.append([])
    for i in range(len(seqs2)): alis2.append([])    
    ...
    if param_loglevel >= 3:
        ...
        for i in range( len(seqs1) ):
            for cd in cds1[transcripts1[i][0]]: print "#", str(cd)
        print "# versus"
        for i in range( len(seqs2) ):
            for cd in cds2[transcripts2[i][0]]: print "#", str(cd)
        sys.stdout.flush()
        
    weights = {}
    for i in range( len(seqs1) ):
        prediction_id1, sbjct_token1, sbjct_strand1, sbjct_from1, sbjct_to1 = transcripts1[i]
        ...
        for j in range( len(seqs2) ):
            ...
            if param_loglevel >= 3:
                print "# Starting alignment of pair (%i,%i) of lengths %s:%i and %s:%i" %\
                      (i,j,prediction_id1,seqs1[i].getLength(),prediction_id2,seqs2[j].getLength())
                sys.stdout.flush()
            ...
            coverage_a = 100.0 * (map_a2b.getRowTo() - map_a2b.getRowFrom() + 1) / seqs1[i].getLength()
            coverage_b = 100.0 * (map_a2b.getColTo() - map_a2b.getColFrom() + 1) / seqs2[j].getLength() 
            ...
            mapped_boundaries = map( lambda x: MyMap(map_a2b, x.mPeptideFrom / 3 + 1) , c1[1:])
            mapped_boundaries.sort()
            reference_boundaries = map( lambda x: x.mPeptideFrom / 3 + 1, c2[1:])
            reference_boundaries.sort()

            nmissed_cmp2ref = Exons.CountMissedBoundaries( mapped_boundaries, reference_boundaries, param_boundaries_max_slippage)
            nmissed_ref2cmp = Exons.CountMissedBoundaries( reference_boundaries, mapped_boundaries, param_boundaries_max_slippage)
            ...
            if (len(c1) == 1 and len(c2) == 1):
                ...
                ## allow for missed boundaries, if param_boundaries_allow_missed > 0
                if  min_nmissed == 0:
                    is_ok = 1
                else:
            ...
            for method in param_write_pairs:
                if method == "all":
                    print string.join( map(str, (
                        ...
                        prediction_id1, 
                        prediction_id2,
                        sbjct_token1, sbjct_strand1, sbjct_from1, sbjct_to1, seqs1[i].getLength(),
            ...
            if not weights.has_key( cc ): weights[cc] = []
    ...
    for w in ww:
        for i,j,map_a2b in weights[w]:
            if not assigned1.has_key(i) and not assigned2.has_key(j):
                ...
                assigned1[i] = 1
                assigned2[j] = 1                
        if len(assigned1) == len(transcripts1): break
        if len(assigned2) == len(transcripts2): break
...
def ReadTranscriptsAndCds( transcript_ids1, transcript_ids2 ):
    ...
    cds1 = Exons.ReadExonBoundaries( open(param_filename_cds1, "r"), filter= transcript_ids1, reset = True )
    cds2 = Exons.ReadExonBoundaries( open(param_filename_cds2, "r"), filter= transcript_ids2, reset = True )    
    ...
    if param_filename_transcripts1:
        if param_mode_genome1 == "indexed":
            transcripts1 = Genomics.ParseFasta2HashFromIndex( param_filename_transcripts1, filter = transcript_ids1 )
        else:
    ...
    if param_filename_transcripts2:
        if param_mode_genome2 == "indexed":
            transcripts2 = Genomics.ParseFasta2HashFromIndex( param_filename_transcripts2, filter = transcript_ids2 )
        else:
...
if __name__ == '__main__':
    ...
    peptides1 = Genomics.ReadPeptideSequences( open(param_filename_peptides1, "r"))
    peptides2 = Genomics.ReadPeptideSequences( open(param_filename_peptides2, "r"))    
    ...
    if param_mode == "genes":
        ...
        map_gene2transcripts1 = ReadMapGene2Transcripts( open(param_filename_map1, "r" ), peptides1, gene_ids1 )
        map_gene2transcripts2 = ReadMapGene2Transcripts( open(param_filename_map2, "r" ), peptides2, gene_ids2 )

        transcript_ids1 = {}
        for x in map_gene2transcripts1.values():
            for y in x: transcript_ids1[y[0]] = 1
        transcript_ids2 = {}
        for x in map_gene2transcripts2.values():
            for y in x: transcript_ids2[y[0]] = 1

        transcripts1,transcripts2,cds1,cds2 = ReadTranscriptsAndCds( transcript_ids1, transcript_ids2 )
        ...
        for gene_id1, gene_id2 in pairs:
            ...
            for transcript1, transcript2, weight, map_a2b in pairs:
                ...
                print string.join( map(str, ( "transcript",
                                              string.join(map(str, transcript1), "\t"),
                                              len(cds1[q1]), cds1[q1][0].mGenomeFrom, cds1[q1][-1].mGenomeTo,
                                              string.join(map(str, transcript2), "\t"),
                                              len(cds2[q2]), cds2[q2][0].mGenomeFrom, cds2[q2][-1].mGenomeTo,
                                              )), "\t")
                npairs += 1
                ...
                if map_a2b:
                    if q1 in transcripts1 and q2 in transcripts2:
                        nerrors, nskipped= WriteExons( q1, peptides1[q1], cds1[q1], transcripts1[q1],
                                                       q2, peptides2[q2], cds2[q2], transcripts2[q2],
                                                       map_a2b )

                total_nerrors += nerrors
                total_nskipped += nskipped

                if param_only_best: break
