        ...
        for x in range( self.mNExons ):
            ...
            if e.frame != 0:
                ...
                for y in range( 0, 3 - e.frame):
                    self.mSplitCodonsNext[ start - self.mIntronSize - y - 1 ] = start
        ...
        for x in range( self.mOffset, self.contigSize - self.mOffset, self.mIncrement):
            for y in range( 0, self.mExonSize):
                ...
                if base in self.mSplitCodonsPrev:
                    self.assertEqual( result.prev_exon_end, self.mSplitCodonsPrev[base])
                else:
                    self.assertEqual( result.prev_exon_end, None)
                if base in self.mSplitCodonsNext:
                    self.assertEqual( result.next_exon_start, self.mSplitCodonsNext[base])
                else:
    ...
    def testDeletion( self ):
        ...
        for l in range(1,7):
            for x in range(0,len(self.seq)):
                ...
                deleted_codons = max( 0, (deletion_codon_end - deletion_codon_start) // 3)
                ...
                # delete out-of-frame, multiple of 3
                    ...
                    self.assertEqual( r.noframe_codons, affected_codons - deleted_codons )
                    self.assertEqual( r.nwrong_frames, 1)
                    self.assertEqual( r.ncorrected_frames, 1)
                    
                # delete, but not multiple of 3
                else:
                    self.assertEqual( r.noframe_codons, self.ncodons - inframe - deleted_codons )
                    self.assertEqual( r.nwrong_frames, 1)
                    self.assertEqual( r.ncorrected_frames, 0)
        ...
        for l in range(1,7):
            for offset in range( 1, 5 ):
                for x in range(len(self.seq) -(l+offset)):
                    ...
                    ncodons = self.ncodons
                    # print "l=",l,"x=",x,"offest=",offset,"del=",todelete, "ins=",toinsert, "start=",codon_start, "end=", codon_end

                    naffected_codons = (codon_end - codon_start) // 3
                    if todelete % 3 == 0 and (x+todelete)// 3 != (x+todelete+offset) // 3:
                        # affected codons reduced, if offset includes full codons
                        naffected_codons -= (x+todelete+offset) // 3 - (x+todelete) // 3
                        #if offset > 3:
                        #    naffected_codons -= 1
