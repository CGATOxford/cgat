...
#
#   $Id: export_predictions.py 2781 2009-09-10 11:33:14Z andreas $
#
...
class Selector:
    ...
    def __init__(self, dbhandle, options ):
        ...
        self.mParameters = {}
        self.mParameters['tablename_predictions'] = self.mOptions.schema + "." + self.mOptions.tablename_predictions
        self.mParameters['tablename_genes'] = self.mOptions.schema + "." + self.mOptions.tablename_genes
        self.mParameters['tablename_redundant'] = self.mOptions.schema + "." + self.mOptions.tablename_redundant
        self.mParameters['tablename_contigs'] = self.mOptions.schema + "." + self.mOptions.tablename_contigs
        self.mParameters['tablename_exons'] = self.mOptions.schema + "." + self.mOptions.tablename_exons
        self.mParameters['tablename_quality'] = self.mOptions.schema + "." + self.mOptions.tablename_quality    
        self.mParameters['tablename_disruptions'] = self.mOptions.schema + "." + self.mOptions.tablename_disruptions
        
    def readFilter( self ):
        """read a list of transcripts/genes to be filtered."""
        if self.mOptions.filename_filter:
            self.mFilterList, errors = IOTools.ReadList( open(self.mOptions.filename_filter), map_function = int )
...
class SelectorDisruptions( Selector ):
    ...
    def getStatement( self, type ):
        ...
        if self.mOptions.loglevel >= 2:
            self.mOptions.stdlog.write( "statement for selecting disruptions:\n%s\n%s\n%s\n" % ("#" * 20, statement, "#"*20))
...
class SelectorMRNA(Selector):
    ...
    def getStatement( self ):
        ...
        if self.mOptions.loglevel >= 2:
            self.mOptions.stdlog.write( "statement for selecting mRNAs:\n%s\n%s\n%s\n" % ("#" * 20, statement, "#"*20))
...
class SelectorGenes(Selector):
    ...
    def getStatement( self ):
        ...
        if self.mOptions.loglevel >= 2:
            self.mOptions.stdlog.write( "statement for selecting genes:\n%s\n%s\n%s\n" % ("#" * 20, statement, "#"*20))
...
class SelectorCDS(Selector):
    ...
    def getStatement(self):
        ...
        if self.mOptions.loglevel >= 2:
            self.mOptions.stdlog.write( "statement for selecting cds:\n%s\n%s\n%s\n" % ("#" * 20, statement, "#"*20))
...
class SelectorPredictions(Selector):
    ...
    def getStatement(self):
        ...
        if self.mOptions.loglevel >= 2:
            self.mOptions.stdlog.write( "statement for selecting:\n%s\n%s\n%s\n" % ("#" * 20, statement, "#"*20))
    ...
    def getRows( self ):
        ...
        if self.mOptions.loglevel >= 2:
            self.mOptions.stdlog.write( "# retrieved %i predictions in %i genes.\n" % (len(predictions), len(genes)) )
...
class SelectorPredictionsFiltered(SelectorPredictions):
    def __init__(self,dbhandle,options):
        SelectorPredictions.__init__(self,dbhandle,options)
    def addTables(self): return ",%s.%s AS r" % (options.schema, options.tablename_redundant)        
    def addFilter(self): return """AND g.gene_id > 0
...
class SelectorPredictionsClean(SelectorPredictions):
    def __init__(self,dbhandle,options):
        SelectorPredictions.__init__(self,dbhandle,options)
    def addTables(self): return ",%s.%s AS r,%s.%s AS q" % (options.schema, options.tablename_redundant,
...
class SelectorPredictionsOrthologs(SelectorPredictions):
    def __init__(self,dbhandle,options):
        SelectorPredictions.__init__(self,dbhandle,options)
    def addTables(self): return ",%s AS oo" % options.tablename_orthologs
    def addFilter(self): return "AND g.gene_id > 0 AND oo.prediction_id1 = p.prediction_id AND oo.schema1='%s' " % (self.mOptions.schema)
...
class SelectorPredictionsMaxKs(SelectorPredictions):
    def __init__(self,dbhandle,options):
        SelectorPredictions.__init__(self,dbhandle,options)
    def addTables(self): return ",%s.%s AS k" % (options.schema, self.mOptions.tablename_kaks)
    def addFilter(self): return "AND g.gene_id > 0 AND k.prediction_id = p.prediction_id AND k.ds <= %f " % (self.mOptions.max_ks)
...
class SelectorPredictionsMaxKsQuality(SelectorPredictions):
    def __init__(self,dbhandle,options):
        SelectorPredictions.__init__(self,dbhandle,options)
    def addTables(self): return ",%s.%s AS k, %s.%s AS q" % (options.schema, self.mOptions.tablename_kaks,
...
class SelectorPredictionsFilterPredictions(SelectorPredictions):
    def __init__(self,dbhandle,options):
        SelectorPredictions.__init__(self,dbhandle,options)
        self.readFilter()        
    def addFilter(self): return """AND p.prediction_id IN ('%s')""" % ("','".join(map(str, self.mFilterList)))
...
class SelectorPredictionsFilterGenes(SelectorPredictions):
    ...
    def addFilter(self): return """AND g.gene_id IN ('%s')""" % ("','".join(map(str, self.mFilterList)))
...
class Exporter:
    ...
    def writeMRNATrack( self,
        ...
        for row in rr:
            ...
            if prediction_id not in map_prediction_id:
                map_prediction_id[prediction_id] = self.getMRNAIdentifier( current_id, map_gene_id[gene_id] )
    ...
    def writeContigs( self,
        ...
        if not self.mSelectorContigs: return
    ...
    def writeMap( self, outfile, category, map_id):
        ...
        outfile = open(self.mOptions.filename_pattern_map % (self.mOptions.assembly_id, category), "w")
        ids = map_id.keys()
        ids.sort()
        for id in ids:
            outfile.write( "%i\t%s\n" % (id, map_id[id]) )
...
def writeFasta( filenames, options ):
    ...
    for filename in filenames:
        if filename[-6:] == ".fasta": filename = filename[:-6]
        if filename[-3:] == ".fa": filename = filename[:-3]        
...
if __name__ == "__main__":
    ...
    parser = optparse.OptionParser( version = "%prog version: $Id: export_predictions.py 2781 2009-09-10 11:33:14Z andreas $")
    ...
    if options.set == "raw":
        ...
        selector_predictions = SelectorPredictionsMaxKsQuality( dbhandle, options )
    elif options.set == "filter-predictions":
        selector_predictions = SelectorPredictionsFilterPredictions( dbhandle, options )
    elif options.set == "filter-genes":
        selector_predictions = SelectorPredictionsFilterGenes( dbhandle, options )
    ...
    if options.filename_batch:
        ...
        for line in infile:
            if line[0] == "#": continue
            options.species, options.abbreviation, options.species_id, options.assembly_id, options.schema  = line[:-1].split("\t")[:5]
            filename = options.filename_pattern_output % options.assembly_id
            outfile = open( filename, "w" )
            
            if options.loglevel >= 1:
                options.stdlog.write( "# writing data from %s to %s\n" % (options.schema, filename) )
            ...
            map_gene_id, map_prediction_id = exporter.writeGFF( outfile, dbhandle, options )
