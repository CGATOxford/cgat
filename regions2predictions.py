...
#
#   $Id: regions2predictions.py 1799 2008-03-28 11:44:19Z andreas $
#
...
def CheckBenchmark( p, kept_p = None ):
    ...
    if not param_benchmarks.has_key( key ): return
...
def EvaluateBenchmark( predictions, loglevel = 2 ):
    ...
    if not param_benchmarks: return
    ...
    for p in predictions:
        ...
        if not param_benchmarks.has_key( key ): continue
...
def FilterSuboptimal( old_predictions,
    ...
    for p in old_predictions:
        ...
        if p.mQueryCoverage / x.mQueryCoverage < min_relative_coverage:
            ...
            if param_benchmarks: CheckBenchmark( p )
            removed_predictions.append( p )
            continue
            ...
            if last_prediction.mQueryCoverage > p.mQueryCoverage:
                ...
                if param_loglevel >= 2:
                    print "# OVERLAP: kept %s(%s-%i), removed: %s" % (str(p.mPredictionId),
                                                                      ...
                                                                      p.mSbjctGenomeFrom,
                                                                      str(last_prediction))
                if param_benchmarks: CheckBenchmark( last_prediction )            
...
if __name__ == '__main__':
    ...
    if param_filename_benchmark:
        param_benchmarks = ReadBenchmarkingRegions( open(param_filename_benchmark, "r") )
        if param_loglevel >= 1:
    ...
    if param_filter_overlaps:
        noverlaps = FilterOverlaps( old_predictions, new_predictions, removed_predictions )
        if param_conserve_memory:
    ...
    if param_max_matches > 0:    
        nmax_matches = FilterMaxMatches( old_predictions, new_predictions, param_max_matches )
        if param_conserve_memory:
            ExchangeStreams( old_predictions, new_predictions )
        else:
            old_predictions = new_predictions
    ...
    if param_reactivate_missed:
        ...
        for p in removed_predictions:
            ...
            if accepted_transcripts.has_key( p.mQueryToken ): continue

            ## skip lower scoring segments
            if last_query_token == p.mQueryToken: continue
    ...
    if param_loglevel >= 1:
        print "# pairs: nread=%i, input=%i, bad=%i, overlaps=%i, max_matches=%i, conflicts=%i, suboptimal=%i, activated=%i, joined=%i" % \
              (nread, ninput, nbad, noverlaps, nmax_matches, nconflicts, nsuboptimal, nactivated, njoined)
