...
RUN wget --no-check-certificate https://raw.github.com/CGATOxford/cgat/master/install-CGAT-tools.sh && \
    mkdir /shared && \
    bash install-CGAT-tools.sh --cgat-scripts --location /shared

# Update PATH
ENV PATH /shared/conda-install/envs/cgat-scripts/bin:$PATH

# Add an entry point to the cgat command
ENTRYPOINT ["/shared/conda-install/envs/cgat-scripts/bin/cgat"]
